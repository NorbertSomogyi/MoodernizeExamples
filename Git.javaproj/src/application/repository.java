...
public class repository {
	...
	public void trace2_def_repo_fl(Object file, int line) {
		...
		for (; tgt_j; ) {
			if (ModernizedCProgram.tr2_dst_trace_want(generatedTr2_tgt)) {
				if (generatedTr2_tgt) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(file, line, ModernizedCProgram.repo);
				} 
			} 
		}
	}
	...
	public int print_config_from_gitmodules(Object key) {
		int ret;
		byte store_key;
		ret = /*Error: Function owner not recognized*/git_config_parse_key(key, store_key, ((Object)0));
		if (ret < 0) {
			return 1;
		} 
	...
	public void deduplicate_islands() {
		...
		(list) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (island_count)));
		{ 
	...
	public void load_delta_islands(int progress) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config(island_config_callback, ((Object)0));
		ModernizedCProgram.for_each_ref(find_island_for_ref, ((Object)0));
		r.deduplicate_islands();
		if (progress) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Marked %d islands, done.\n"), ModernizedCProgram.island_counter);
		} 
	}
	public void check_good_are_ancestors_of_bad(Object prefix, int no_checkout) {
		...
		if (!/*Error: Function owner not recognized*/stat(filename, st) && (((generatedSt_mode) & -1024) == /* Check if file BISECT_ANCESTORS_OK exists. */-1024)) {
			;
		} 
		...
		fd = /*Error: Function owner not recognized*/open(filename, -1024 | -1024 | 1, /* Create file BISECT_ANCESTORS_OK. */600);
		if (fd < 0) {
	...
	public int bisect_next_all(Object prefix, int no_checkout) {
		...
		if (!generatedCommits) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf(ModernizedCProgram._("%s was both %s and %s\n"), ModernizedCProgram.oid_to_hex(ModernizedCProgram.current_bad_oid), ModernizedCProgram.term_good, ModernizedCProgram.term_bad);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\bisect.c", 981, (true)));
		} 
		if (!all) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("No testable commit found.\nMaybe you started with bad path parameters?\n"));
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\bisect.c", 987, (true)));
		} 
		...
		if (ModernizedCProgram.oideq(bisect_rev, ModernizedCProgram.current_bad_oid)) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%s is the first %s commit\n", ModernizedCProgram.oid_to_hex(bisect_rev), ModernizedCProgram.term_bad);
			ModernizedCProgram.show_diff_tree(r, prefix, generatedItem);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\bisect.c", 998, (/* This means the bisection process succeeded. */true)));
		} 
		nr = all - reaches - 1;
		...
		steps_msg = ModernizedCProgram.xstrfmt(ModernizedCProgram.Q_("(roughly %d step)", "(roughly %d steps)", steps), steps/*
			 ...
			 * steps)" translation.
			 */);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf(ModernizedCProgram.Q_("Bisecting: %d revision left to test after this %s\n", "Bisecting: %d revisions left to test after this %s\n", nr), nr, steps_msg);
	...
	public int disambiguate_committish_only(Object oid, Object cb_data_unused) {
		...
		obj = /*Error: Function owner not recognized*/deref_tag(r, object.parse_object(r, oid), ((Object)0), /* We need to do this the hard way... */0);
	...
	public int repo_for_each_abbrev(Object prefix, Object fn, Object cb_data) {
		...
		int ret;
		if (ModernizedCProgram.init_object_disambiguation(r, prefix, /*Error: Function owner not recognized*/strlen(prefix), ds) < 0) {
			return -1;
		} 
	...
	public Object repo_find_unique_abbrev(Object oid, int len) {
		...
		bufno = (bufno + 1) % (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1));
	...
	public void diagnose_invalid_index_path(int stage, Object prefix, Object filename) {
		...
		int pos;
		int namelen = /*Error: Function owner not recognized*/strlen(filename);
		strbuf fullname = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		if (!prefix) {
			prefix = "";
		} 
		...
		if (pos < generatedCache_nr) {
			...
			if (((ce).getCe_namelen()) == namelen && !/*Error: Function owner not recognized*/memcmp(ce.getName(), filename, namelen)) {
		...
		if (pos < generatedCache_nr) {
			ce = generatedCache[pos];
			if (((ce).getCe_namelen()) == generatedLen && !/*Error: Function owner not recognized*/memcmp(ce.getName(), generatedBuf, generatedLen)) {
	...
	public Byte resolve_relative_path(Object rel) {
		if (!ModernizedCProgram.starts_with(rel, "./") && !ModernizedCProgram.starts_with(rel, "../")) {
			return ((Object)0);
		} 
		if (r != ModernizedCProgram.the_repository || !ModernizedCProgram.is_inside_work_tree()) {
			ModernizedCProgram.die("relative path syntax can't be used outside working tree.");
		} 
		return ModernizedCProgram.prefix_path(ModernizedCProgram.startup_info.getPrefix(), ModernizedCProgram.startup_info.getPrefix() ? /*Error: Function owner not recognized*/strlen(ModernizedCProgram.startup_info.getPrefix()) : 0, rel);
		strbuf sb = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		if (argc != 3) {
			ModernizedCProgram.die("submodule--helper relative-path takes exactly 2 arguments, got %d", argc);
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%s", sb.relative_path(argv[1], argv[2]));
		sb.strbuf_release();
		return 0;
	}
	...
	public void diffcore_break(int break_score) {
		...
		for (i = 0; i < generatedNr; i++) {
			...
			int score;
			if (((generatedMode) != 0) && ((generatedMode) != 0) && ModernizedCProgram.object_type(generatedMode) == object_type.OBJ_BLOB && ModernizedCProgram.object_type(generatedMode) == object_type.OBJ_BLOB && !/*Error: Function owner not recognized*/strcmp(generatedPath, generatedPath)) {
				if (ModernizedCProgram.should_break(r, generatedOne, generatedTwo, break_score, score/* Split this into delete and create */)) {
					...
					null_one = /*Error: Function owner not recognized*/alloc_filespec(generatedPath);
					dp = /*Error: Function owner not recognized*/diff_queue(outq, generatedOne, null_one);
					dp.setScore(score);
					dp.setBroken_pair(1);
					null_two = /*Error: Function owner not recognized*/alloc_filespec(generatedPath);
					dp = /*Error: Function owner not recognized*/diff_queue(outq, null_two, generatedTwo);
					dp.setScore(score);
					dp.setBroken_pair(1);
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/diff_free_filespec_blob(generatedOne);
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/diff_free_filespec_blob(generatedTwo);
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/diff_free_filespec_data(generatedOne);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/diff_free_filespec_data(generatedTwo);
	...
	public Object lookup_object_by_type(Object oid, object_type type) {
		...
		case object_type.OBJ_TAG:
				return /*Error: Function owner not recognized*/lookup_tag(r, oid);
		case object_type.OBJ_TREE:
				return tree.lookup_tree(r, oid);
		case object_type.OBJ_COMMIT:
				return /*Error: Function owner not recognized*/lookup_commit(r, oid);
	...
	public void prepare_repo_settings() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedSettings, -1, /*Error: sizeof expression not supported yet*/);
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "core.commitgraph", value)) {
			generatedSettings.setCore_commit_graph(value);
		} 
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "gc.writecommitgraph", value)) {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_int(r, "index.version", value)) {
			generatedSettings.setIndex_version(value);
		} 
		if (!/*Error: Function owner not recognized*/repo_config_get_maybe_bool(r, "core.untrackedcache", value)) {
			if (value == 0) {
		...
		}  else if (!/*Error: Function owner not recognized*/repo_config_get_string(r, "core.untrackedcache", strval)) {
			if (!/*Error: Function owner not recognized*/strcasecmp(strval, "keep")) {
				generatedSettings.setCore_untracked_cache(untracked_cache_setting.UNTRACKED_CACHE_KEEP);
			} 
			ModernizedCProgram.free(strval);
		} 
		if (!/*Error: Function owner not recognized*/repo_config_get_string(r, "fetch.negotiationalgorithm", strval)) {
			if (!/*Error: Function owner not recognized*/strcasecmp(strval, "skipping")) {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "pack.usesparse", value)) {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "feature.manyfiles", value) && value) {
			do {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "fetch.writecommitgraph", value)) {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(r, "feature.experimental", value) && value) {
			do {
	...
	public int write_archive(int argc, Object argv, Object prefix, Object name_hint, int remote) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config_get_bool("uploadarchive.allowunreachable", ModernizedCProgram.remote_allow_unreachable);
		...
		return /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(ar, args);
	}
	public Byte substitute_branch_name(Object string, Integer len) {
	...
	public int do_for_each_ref_helper(Object refname, Object oid, int flags, Object cb_data) {
		...
		return /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(refname, oid, flags, generatedCb_data);
	}
	public int for_each_replace_ref(Object fn, Object cb_data) {
		return r.do_for_each_repo_ref(ModernizedCProgram.git_replace_ref_base, fn, /*Error: Function owner not recognized*/strlen(ModernizedCProgram.git_replace_ref_base), DO_FOR_EACH_INCLUDE_BROKEN, cb_data);
	}
	public int is_submodule_active(Object path) {
		...
		if (!/*Error: Function owner not recognized*/repo_config_get_bool(ModernizedCProgram.repo, key, ret)) {
			ModernizedCProgram.free(key);
			return ret;
		} 
		ModernizedCProgram.free(key);
		sl = /*Error: Function owner not recognized*/repo_config_get_value_multi(ModernizedCProgram.repo, /* submodule.active is set */"submodule.active");
		if (sl) {
			...
			ret = ModernizedCProgram.match_pathspec(ModernizedCProgram.repo.getIndex(), ps, path, /*Error: Function owner not recognized*/strlen(path), 0, ((Object)0), 1);
			ModernizedCProgram.args.argv_array_clear();
			ps.clear_pathspec();
			return ret;
		} 
		key = ModernizedCProgram.xstrfmt("submodule.%s.url", module.getName());
		ret = !/*Error: Function owner not recognized*/repo_config_get_string(ModernizedCProgram.repo, key, value);
	...
	public repository open_submodule(Object path) {
		strbuf sb = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		repository out = ModernizedCProgram.xmalloc(/*Error: sizeof expression not supported yet*/);
	...
	public repository get_submodule_repo_for(Object sub) {
		repository ret = ModernizedCProgram.xmalloc(/*Error: sizeof expression not supported yet*/);
		...
		if (ret.repo_submodule_init(r, sub/*
				 ...
				 */)) {
			strbuf gitdir = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public void grow_object_hash() {
		...
		new_hash = ModernizedCProgram.xcalloc(new_hash_size, /*Error: Unsupported expression*/);
	...
	public int link_alt_odb_entry(Object entry, Object relative_base, int depth, Object normalized_objdir) {
		...
		ent = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
	...
	public void link_alt_odb_entries(Object alt, int sep, Object relative_base, int depth) {
		strbuf objdirbuf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf entry = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		if (!alt || !alt) {
			return /*Error: Unsupported expression*/;
		} 
		if (depth > 5) {
	...
	public int open_loose_object(Object oid, Object path) {
		...
		for (odb = generatedOdb; odb; odb = generatedNext) {
			...
			if (most_interesting_errno == 2) {
				most_interesting_errno = (/*Error: Function owner not recognized*/_errno());
			} 
		}
		(/*Error: Function owner not recognized*/_errno()) = most_interesting_errno;
		return -1;
	}
	...
	public Object read_object_file_extended(Object oid, object_type type, Long size, int lookup_replace) {
		...
		if ((/*Error: Function owner not recognized*/_errno()) && (/*Error: Function owner not recognized*/_errno()) != 2) {
	...
	public int get_index_format_default() {
		byte envversion = /*Error: Function owner not recognized*/getenv("GIT_INDEX_VERSION");
		byte endp;
		int version = 3;
		repo_settings generatedSettings = this.getSettings();
		int generatedIndex_version = generatedSettings.getIndex_version();
		...
		version = /*Error: Function owner not recognized*/strtoul(envversion, endp, 10);
	...
	public int repo_read_index_unmerged() {
		...
		for (i = 0; i < generatedCache_nr; i++) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedName, generatedName, ModernizedCProgram.len);
	...
	public int has_unstaged_changes(int ignore_submodules) {
		...
		return /*Error: Function owner not recognized*/diff_result_code(generatedDiffopt, result/**
	...
	public int require_clean_work_tree(Object action, Object hint, int ignore_submodules, int gently) {
		...
		if (err) {
			...
			if (!gently) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\wt-status.c", 2487, (true)));
			} 
		} 
		return err;
	}
	...
	public Object repo_logmsg_reencode(Object commit, Byte commit_encoding, Object output_encoding) {
		...
		byte use_encoding;
		byte encoding;
		byte msg = /*Error: Function owner not recognized*/repo_get_commit_buffer(r, commit, ((Object)0));
		byte out;
		...
		if (ModernizedCProgram.same_encoding(use_encoding, output_encoding/*
			...
			if (msg == /*Error: Function owner not recognized*/get_cached_commit_buffer(r, commit, ((Object)0))) {
				...
				if (out) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/repo_unuse_commit_buffer(r, commit, msg/*
	...
	public int is_index_unchanged() {
		...
		head_commit = /*Error: Function owner not recognized*/lookup_commit(r, head_oid/*
		...
		return ModernizedCProgram.oideq(cache_tree_oid, /*Error: Function owner not recognized*/get_commit_tree_oid(head_commit));
	}
	...
	public void print_commit_summary(Object prefix, Object oid, int flags) {
		rev_info rev = new rev_info();
		commit commit = new commit();
		strbuf format = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		byte head;
		pretty_print_context pctx = new pretty_print_context(0);
		strbuf author_ident = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf committer_ident = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		commit = /*Error: Function owner not recognized*/lookup_commit(r, oid);
		if (!commit) {
		...
		if (flags & (1 << 1)) {
			strbuf date = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (!/*Error: Function owner not recognized*/strcmp(head, "HEAD")) {
			head = ModernizedCProgram._("detached HEAD");
		} else {
				ModernizedCProgram.skip_prefix(head, "refs/heads/", head);
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("[%s%s ", head, (flags & (1 << 0)) ? ModernizedCProgram._(" (root-commit)") : "");
	...
	public int sequencer_get_last_command(replay_action action) {
		...
		if (buf.strbuf_read_file(todo_file, 0) < 0) {
			if ((/*Error: Function owner not recognized*/_errno()) == 2 || (/*Error: Function owner not recognized*/_errno()) == 20) {
				return -1;
			} else {
					return ();
			} 
		...
		byte[] generatedBuf = buf.getBuf();
		bol = generatedBuf + /*Error: Function owner not recognized*/strspn(generatedBuf, " \t\r\n");
	...
	public void sequencer_post_commit_cleanup(int verbose) {
		replay_opts opts = new replay_opts(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int need_cleanup = 0;
		if (ModernizedCProgram.file_exists(r.git_path_cherry_pick_head())) {
			if (!/*Error: Function owner not recognized*/unlink(r.git_path_cherry_pick_head()) && verbose) {
		...
		if (ModernizedCProgram.file_exists(r.git_path_revert_head())) {
			if (!/*Error: Function owner not recognized*/unlink(r.git_path_revert_head()) && verbose) {
	...
	public int create_seq_dir() {
		...
		if (/*Error: Function owner not recognized*/mkdir(ModernizedCProgram.git_path_seq_dir(), 777) < 0) {
			return ();
		} 
		return 0;
	}
	...
	public int do_exec(Object command_line) {
		...
		int dirty;
		int status;
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Executing: %s\n", command_line);
	...
	public int do_label(Object name, int len) {
		...
		strbuf ref_name = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public int stopped_at_head() {
		...
		if (ModernizedCProgram.repo_get_oid(ModernizedCProgram.the_repository, "HEAD", head) || !(commit = /*Error: Function owner not recognized*/lookup_commit(r, head)) || ModernizedCProgram.repo_parse_commit(ModernizedCProgram.the_repository, commit) || ModernizedCProgram.get_message(commit, message)) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Stopped at HEAD\n"));
		} else {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Stopped at %s\n"), generatedLabel);
	...
	public int check_todo_list_from_file() {
		todo_list old_todo = new todo_list(new todo_list(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/));
		todo_list new_todo = new todo_list(new todo_list(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/));
		int res = 0;
		...
		if (res) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._(ModernizedCProgram.edit_todo_list_advice));
		} 
	...
	public int register_replace_ref(Object refname, Object oid, int flag, Object cb_data) {
		byte slash = /*Error: Function owner not recognized*/strrchr(refname, /* Get sha1 from refname */(byte)'/');
		byte hash = slash ? slash + 1 : refname;
		replace_object repl_obj = ModernizedCProgram.xmalloc(/*Error: sizeof expression not supported yet*/);
	...
	public void prepare_replace_object() {
		...
		generatedObjects.setReplace_map(ModernizedCProgram.xmalloc(/*Error: sizeof expression not supported yet*/));
	...
	public void repo_set_gitdir(Object root, Object o) {
		...
		if (!ModernizedCProgram.repo.getObjects().getOdb()) {
			ModernizedCProgram.repo.getObjects().setOdb(ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/));
	...
	public int repo_init(Object gitdir, Object worktree) {
		repository_format format = new repository_format(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(ModernizedCProgram.repo, 0, /*Error: sizeof expression not supported yet*/);
	...
	public int repo_submodule_init(repository superproject, Object sub) {
		strbuf gitdir = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf worktree = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int ret = 0;
		if (!sub) {
			ret = -1;
			;
	...
	public void repo_clear() {
		...
		if (ModernizedCProgram.repo.getConfig()) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_configset_clear(ModernizedCProgram.repo.getConfig());
			do {
	...
	public int repo_read_index() {
		if (!ModernizedCProgram.repo.getIndex()) {
			ModernizedCProgram.repo.setIndex(ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/));
		} 
	...
	public int prepare_multi_pack_index_one(Object object_dir, int local) {
		...
		int env_value = -1;
		if (env_value < 0) {
			env_value = /*Error: Function owner not recognized*/git_env_bool("GIT_TEST_MULTI_PACK_INDEX", 0);
		} 
		if (!env_value && (/*Error: Function owner not recognized*/repo_config_get_bool(r, "core.multipackindex", config_value) || !config_value)) {
			return 0;
		} 
		...
		for (m_search = generatedMulti_pack_index; m_search; m_search = generatedNext) {
			if (!/*Error: Function owner not recognized*/strcmp(object_dir, generatedObject_dir)) {
				return 1;
			} 
		}
	...
	public int verify_midx_file(Object object_dir) {
		...
		for (i = 0; i < 255; i++) {
			uint32_t oid_fanout1 = /*Error: Function owner not recognized*/ntohl(generatedChunk_oid_fanout[i]);
			uint32_t oid_fanout2 = /*Error: Function owner not recognized*/ntohl(generatedChunk_oid_fanout[i + 1]);
		...
		progress/*
			 ...
			 * time.
			 */.stop_progress();
		(pairs) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedNum_objects)));
		...
		ModernizedCProgram.sane_qsort((pairs), (generatedNum_objects), /*Error: sizeof expression not supported yet*/, compare_pair_pos_vs_id);
		progress.stop_progress();
	...
	public int expire_midx_packs(Object object_dir) {
		...
		Object generatedNum_packs = m.getNum_packs();
		count = ModernizedCProgram.xcalloc(generatedNum_packs, /*Error: Unsupported expression*/);
		Object generatedNum_objects = m.getNum_objects();
		for (i = 0; i < generatedNum_objects; i++) {
	...
	public int midx_repack(Object object_dir, Object batch_size) {
		...
		strbuf base_name = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		multi_pack_index multi_pack_index = new multi_pack_index();
		multi_pack_index m = multi_pack_index.load_multi_pack_index(object_dir, 1);
		if (!m) {
			return 0;
		...
		Object generatedNum_packs = m.getNum_packs();
		include_pack = ModernizedCProgram.xcalloc(generatedNum_packs, /*Error: Unsupported expression*/);
		if (batch_size) {
	...
	public int register_shallow(Object oid) {
		commit_graft graft = ModernizedCProgram.xmalloc(/*Error: Unsupported expression*/);
		commit commit = /*Error: Function owner not recognized*/lookup_commit(ModernizedCProgram.the_repository, oid);
		...
		return /*Error: Function owner not recognized*/register_commit_graft(r, graft, 0);
	}
	public int is_repository_shallow() {
		...
		if (!path || (fp = /*Error: Function owner not recognized*/fopen(path, "r")) == ((Object)0)) {
	...
	public void prepare_packed_git_one(Byte objdir, int local) {
		...
		while (generatedM && /*Error: Function owner not recognized*/strcmp(generatedObject_dir, /* look for the multi-pack-index for this object directory */objdir)) {
		...
		ModernizedCProgram.report_pack_garbage(generatedGarbage);
	...
	public Object alloc_blob_node() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		blob b = generatedParsed_object_pool.alloc_node(/*Error: Unsupported expression*/);
	...
	public Object alloc_tree_node() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		tree t = generatedParsed_object_pool.alloc_node(/*Error: Unsupported expression*/);
	...
	public Object alloc_tag_node() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		tag t = generatedParsed_object_pool.alloc_node(/*Error: Unsupported expression*/);
		Object generatedObject = t.getObject();
	...
	public Object alloc_object_node() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		object obj = generatedParsed_object_pool.alloc_node(/*Error: Unsupported expression*/);
	...
	public Object alloc_commit_node() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		commit c = generatedParsed_object_pool.alloc_node(/*Error: Unsupported expression*/);
	...
	public void alloc_report() {
		parsed_object_pool generatedParsed_objects = this.getParsed_objects();
		Object generatedParsed_object_pool = generatedParsed_objects.getParsed_object_pool();
		"blob".report(generatedParsed_object_pool.getCount(), generatedParsed_object_pool.getCount() * /*Error: Unsupported expression*/ >> 10);
		"tree".report(generatedParsed_object_pool.getCount(), generatedParsed_object_pool.getCount() * /*Error: Unsupported expression*/ >> 10);
		"commit".report(generatedParsed_object_pool.getCount(), generatedParsed_object_pool.getCount() * /*Error: Unsupported expression*/ >> 10);
		"tag".report(generatedParsed_object_pool.getCount(), generatedParsed_object_pool.getCount() * /*Error: Unsupported expression*/ >> 10);
		"object".report(generatedParsed_object_pool.getCount(), generatedParsed_object_pool.getCount() * /*Error: Unsupported expression*/ >> 10);
	}
	public void create_branch(Object name, Object start_name, int force, int clobber_head_ok, int reflog, int quiet, branch_track track) {
		...
		if (ModernizedCProgram.repo_get_oid_mb(ModernizedCProgram.the_repository, start_name, oid)) {
			if (explicit_tracking) {
				if (ModernizedCProgram.advice_set_upstream_failure) {
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\branch.c", 277, (true)));
				} 
		...
		switch (oid.dwim_ref(start_name, /*Error: Function owner not recognized*/strlen(start_name), real_ref)) {
		case 0:
		...
		if ((commit = /*Error: Function owner not recognized*/lookup_commit_reference(r, oid)) == ((Object)0)) {
	...
	public void remove_merge_branch_state() {
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/unlink(r.git_path_merge_head());
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/unlink(r.git_path_merge_rr());
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/unlink(r.git_path_merge_msg());
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/unlink(r.git_path_merge_mode());
	}
	public void remove_branch_state(int verbose) {
		r.sequencer_post_commit_cleanup(verbose);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/unlink(r.git_path_squash_msg());
	...
	public int notes_cache_match_validity(Object ref, Object validity) {
		...
		commit = /*Error: Function owner not recognized*/lookup_commit_reference_gently(r, oid, 1);
		if (!commit) {
			return 0;
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(pretty_ctx, 0, /*Error: sizeof expression not supported yet*/);
		...
		ret = !/*Error: Function owner not recognized*/strcmp(generatedBuf, validity);
		msg.strbuf_release();
		return ret;
	}
