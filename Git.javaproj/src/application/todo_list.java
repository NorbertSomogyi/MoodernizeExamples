...
public class todo_list {
	...
	public int todo_list_check(todo_list new_todo) {
		...
		strbuf missing = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int res = 0;
		int i;
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Warning: some commits may have been dropped accidentally.\nDropped commits (newer to older):\n"));
		byte[] generatedBuf = missing.getBuf();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fputs(generatedBuf, (_iob[/* Make the list user-friendly and display */2]));
		missing.strbuf_release();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("To avoid this message, use \"drop\" to explicitly remove a commit.\n\nUse 'git config rebase.missingCommitsCheck' to change the level of warnings.\nThe possible behaviours are: ignore, warn, error.\n\n"));
		return res;
	}
	...
	public int todo_list_rearrange_squash() {
		...
		(next) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedNr)));
		(tail) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedNr)));
		(subjects) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedNr)));
		todo_item[] generatedItems = this.getItems();
		commit generatedCommit = item.getCommit();
		...
		for (i = 0; i < generatedNr; i++) {
			...
			commit_buffer = /*Error: Function owner not recognized*/repo_get_commit_buffer(ModernizedCProgram.the_repository, generatedCommit, ((Object)0));
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/find_commit_subject(commit_buffer, subject);
			ModernizedCProgram.buf.format_subject(subject, " ");
			subject = subjects[i] = ModernizedCProgram.buf.strbuf_detach(subject_len);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/repo_unuse_commit_buffer(ModernizedCProgram.the_repository, generatedCommit, commit_buffer);
			if ((ModernizedCProgram.skip_prefix(subject, "fixup! ", p) || ModernizedCProgram.skip_prefix(subject, "squash! ", p))) {
				...
				if (entry) {
					i2 = generatedI;
				}  else if (!/*Error: Function owner not recognized*/strchr(p, (byte)' ') && (commit2 = /*Error: Function owner not recognized*/lookup_commit_reference_by_name(p)) && todo_item.commit_todo_item_at(commit_todo, commit2)) {
			...
			if (i2 >= 0) {
				...
				do {
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy((Object)generatedSubject, (subject), flex_array_len_);
				} while (0);
				entry.setI(i);
				...
				while (cur >= 0) {
					do {
						if ((nr + 1) > alloc) {
							...
							(items) = ModernizedCProgram.xrealloc((items), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (alloc)));
						} 
					} while (0);
