...
public class lock_file {
	...
	public int lock_file_timeout(Object path, int flags, long timeout_ms) {
		...
		while (1) {
			...
			wait_ms = (750 + .rand() % 500) * backoff_ms / /* back off for between 0.75*backoff_ms and 1.25*backoff_ms */1000;
	...
	public int hold_lock_file_for_update_timeout(Object path, int flags, long timeout_ms) {
		...
		if (fd < 0) {
			if (flags & 1) {
				ModernizedCProgram.unable_to_lock_die(path, (._errno()));
			} 
			if (flags & 4) {
				strbuf buf = new strbuf(, , );
				ModernizedCProgram.buf.unable_to_lock_message(path, (._errno()));
				();
