...
public class transport {
	...
	public void set_option(Object name, Object value) {
		...
		string_list_item string_list_item = new string_list_item();
		if (!/*Error: Function owner not recognized*/strcmp(name, "verbosity")) {
			byte end;
			int v = /*Error: Function owner not recognized*/strtol(value, end, 10);
			if (value == end || end) {
				return -1;
			} 
			ModernizedCProgram.options.setVerbosity(v);
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "progress")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
				ModernizedCProgram.options.setProgress(1);
			}  else if (!/*Error: Function owner not recognized*/strcmp(value, "false")) {
			...
			byte end;
			long v = /*Error: Function owner not recognized*/strtoul(value, end, 10);
			if (value == end || end) {
				return -1;
			} 
			...
			string_list_item.string_list_append(ModernizedCProgram.options.getDeepen_not(), value);
			return 0;
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "dry-run")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "check-connectivity")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "cas")) {
			...
			string_list_item.string_list_append(ModernizedCProgram.cas_options, generatedBuf);
			val.strbuf_release();
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "cloning")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "update-shallow")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "pushcert")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
				ModernizedCProgram.options.setPush_cert(2);
			}  else if (!/*Error: Function owner not recognized*/strcmp(value, "false")) {
				ModernizedCProgram.options.setPush_cert(0);
			}  else if (!/*Error: Function owner not recognized*/strcmp(value, "if-asked")) {
				ModernizedCProgram.options.setPush_cert(1);
			} else {
					return -1;
			} 
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "atomic")) {
			if (!/*Error: Function owner not recognized*/strcmp(value, "true")) {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, "push-option")) {
			if (value != (byte)'"') {
				string_list_item.string_list_append(ModernizedCProgram.options.getPush_options(), value);
			} else {
			...
			return 0;
		}  else if (!/*Error: Function owner not recognized*/strcmp(name, /* LIBCURL_VERSION_NUM >= 0x070a08 */"from-promisor")) {
	...
	public void do_take_over() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fclose(generatedOut);
	...
	public int disconnect_helper() {
		...
		if (generatedHelper) {
			if (ModernizedCProgram.debug) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Debug: Disconnecting.\n");
			} 
	...
	public int set_helper_option(Object name, Object value) {
		...
		if (!/*Error: Function owner not recognized*/strcmp(name, "deepen-not")) {
		...
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); i++) {
			if (!/*Error: Function owner not recognized*/strcmp(name, ModernizedCProgram.unsupported_options[i])) {
				return 1;
			} 
		}
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); i++) {
			if (!/*Error: Function owner not recognized*/strcmp(name, ModernizedCProgram.boolean_options[i])) {
				is_bool = 1;
				break;
			} 
		}
	...
	public void standard_options() {
		...
		ModernizedCProgram.xsnprintf(buf, /*Error: sizeof expression not supported yet*/, "%d", v + 1);
	...
	public int process_connect_service(Object name, Object exec) {
		Object generatedData = this.getData();
		helper_data data = generatedData;
		strbuf cmdbuf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int ret = 0/*
			 * Handle --upload-pack and friends. This is fire and forget...
			 * just warn if it fails.
			 */;
		if (/*Error: Function owner not recognized*/strcmp(name, exec)) {
		...
		}  else if (generatedStateless_connect && (ModernizedCProgram.get_protocol_version_config() == protocol_version.protocol_v2) && !/*Error: Function owner not recognized*/strcmp("git-upload-pack", name)) {
	...
	public int connect_helper(Object name, Object exec, int[] fd) {
		Object generatedData = this.getData();
		helper_data data = generatedData;
		child_process child_process = new child_process();
	...
	public int transport_helper_init(Object name) {
		helper_data data = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
		data.setName(name);
		ModernizedCProgram.transport_check_allowed(name);
		if (/*Error: Function owner not recognized*/getenv("GIT_TRANSPORT_HELPER_DEBUG")) {
	...
	public void update_remote_refs(Object refs, Object mapped_refs, Object remote_head_points_at, Object branch_top, Object msg, int check_connectivity, int check_refs_only) {
		...
		if (remote_head_points_at && !ModernizedCProgram.option_bare) {
			strbuf head_ref = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public int connect_git(Object name, Object executable, int[] fd) {
		Object generatedData = this.getData();
		git_transport_data data = generatedData;
		Object generatedFd = data.getFd();
		Object generatedUrl = this.getUrl();
	...
	public transport transport_get(remote remote, Object url) {
		byte helper;
		transport ret = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
		ret.setProgress(/*Error: Function owner not recognized*/isatty(2));
		if (!remote) {
		...
		if (helper) {
		...
		}  else if (ModernizedCProgram.url_is_local_not_ssh(url) && ModernizedCProgram.is_file(url) && /*Error: Function owner not recognized*/is_bundle(url, 1)) {
			bundle_transport_data data = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
			...
			ret.setSmart_options(((Object)0));
				 ...
				 */)) {
			git_transport_data data = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
			ret.setData(data);
	...
	public int transport_set_option(Object name, Object value) {
		...
		if (generatedVtable.getSet_option()) {
			protocol_reports = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(transport, name, value);
		} 
	...
	public void transport_set_verbosity(int verbosity, int force_progress) {
		...
		if (force_progress >= 0) {
			this.setProgress(!!force_progress);
				...
				this.setProgress(verbosity >= 0 && /*Error: Function owner not recognized*/isatty(2));
		} 
	}
	public Object transport_get_remote_refs(Object ref_prefixes) {
		...
		if (!generatedGot_remote_refs) {
			this.setRemote_refs(/*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(transport, 0, ref_prefixes));
	...
	public int transport_connect(Object name, Object exec, Integer fd) {
		...
		if (generatedVtable.getConnect()) {
			return /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(transport, name, exec, fd);
		} else {
				ModernizedCProgram.die(ModernizedCProgram._("operation not supported by protocol"));
		} 
	}
	public int transport_disconnect() {
		...
		if (generatedVtable.getDisconnect()) {
			ret = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(transport);
		} 
