...
public class pack_midx_entry {
	...
	public pack_midx_entry get_sorted_entries(multi_pack_index m, pack_info info, Object nr_packs, Object nr_objects) {
		...
		(entries_by_fanout) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (alloc_fanout)));
		(deduplicated_entries) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (alloc_objects)));
		nr_objects = 0;
		...
		for (cur_fanout = 0; cur_fanout < 256; cur_fanout++) {
			...
			if (m) {
				...
				if (cur_fanout) {
					start = .ntohl(generatedChunk_oid_fanout[cur_fanout - 1]);
				} 
				end = .ntohl(generatedChunk_oid_fanout[cur_fanout]);
				for (cur_object = start; cur_object < end; cur_object++) {
					do {
						if ((nr_fanout + 1) > alloc_fanout) {
							...
							(entries_by_fanout) = ModernizedCProgram.xrealloc((entries_by_fanout), ModernizedCProgram.st_mult(, (alloc_fanout)));
						} 
					} while (0);
			...
			ModernizedCProgram.sane_qsort((entries_by_fanout), (nr_fanout), , midx_oid_compare/*
			...
			for (cur_object = 0; cur_object < nr_fanout; cur_object++) {
				...
				do {
					if ((nr_objects + 1) > alloc_objects) {
						...
						(deduplicated_entries) = ModernizedCProgram.xrealloc((deduplicated_entries), ModernizedCProgram.st_mult(, (alloc_objects)));
					} 
				} while (0);
				.memcpy(deduplicated_entries[nr_objects], entries_by_fanout[cur_object], );
				(nr_objects)++;
			}
		}
