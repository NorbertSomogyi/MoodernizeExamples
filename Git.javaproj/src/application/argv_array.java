...
public class argv_array {
	...
	public void argv_array_push_nodup(Object value) {
		...
		do {
			if ((generatedArgc + 2) > generatedAlloc) {
				...
				(generatedArgv) = ModernizedCProgram.xrealloc((generatedArgv), ModernizedCProgram.st_mult(, (generatedAlloc)));
			} 
		} while (0);
	...
	public void prepare_submodule_repo_env_no_git_dir() {
		...
		for (var = ModernizedCProgram.local_repo_env; var; var++) {
			if (.strcmp(var, "GIT_CONFIG_PARAMETERS")) {
	...
	public void parse_argv(Object arg, Object service) {
		while (arg) {
			byte expanded = ModernizedCProgram.strip_escapes(arg, service, arg);
			if (expanded) {
				out.argv_array_push(expanded);
			} 
			ModernizedCProgram.free(expanded);
		}
		int i;
		for (i = 1; i < ModernizedCProgram.global_argc; i++) {
			byte a = ModernizedCProgram.global_argv[i];
			if (a != (byte)'-' || !.strcmp(a, "--")) {
				break;
			} 
			if (!ModernizedCProgram.skip_prefix(a, "--", a)) {
				ModernizedCProgram.die("unknown option %s", a);
			} 
			if (ModernizedCProgram.parse_one_option(a)) {
				continue;
			} 
			if (ModernizedCProgram.parse_one_feature(a, 0)) {
				continue;
			} 
			if (ModernizedCProgram.skip_prefix(a, "cat-blob-fd=", a)) {
				ModernizedCProgram.option_cat_blob_fd(a);
				continue;
			} 
			ModernizedCProgram.die("unknown option --%s", a);
		}
		if (i != ModernizedCProgram.global_argc) {
			ModernizedCProgram.usage(ModernizedCProgram.fast_import_usage);
		} 
		ModernizedCProgram.seen_data_command = 1;
		if (ModernizedCProgram.import_marks_file) {
			ModernizedCProgram.read_marks();
		} 
	}
	...
	public void refspec_ref_prefixes(Object rs) {
		...
		for (i = 0; i < rs.getNr(); i++) {
			...
			if (ModernizedCProgram.prefix) {
				if (item.getPattern()) {
					byte glob = .strchr(ModernizedCProgram.prefix, (byte)'*');
	...
	public int sq_dequote_to_argv_internal(byte arg, Object argv, int nr, int alloc) {
		byte next = arg;
		if (!arg) {
			return 0;
		} 
			...
			if (argv) {
				do {
					if ((nr + 1) > alloc) {
						...
						(argv) = ModernizedCProgram.xrealloc((argv), ModernizedCProgram.st_mult(, (alloc)));
					} 
				} while (0);
				(argv)[(nr)++] = dequoted;
			} 
