...
public class dir_entry {
	...
	private dir_entry parent;
	private int nr;
	private int namelen;
	private Object name;
	
	public dir_entry(hashmap_entry ent, dir_entry parent, int nr, int namelen, Object name) {
		setEnt(ent);
		setParent(parent);
		setNr(nr);
		setNamelen(namelen);
	...
	public dir_entry hash_dir_entry(index_state istate, cache_entry ce, int namelen) {
		...
		hashmap_entry generatedEnt = dir.getEnt();
		hashmap generatedDir_hash = istate.getDir_hash();
		dir_entry dir_entry = new dir_entry();
		if (!dir) {
			do {
				size_t flex_array_len_ = (namelen);
				(dir) = ModernizedCProgram.xcalloc(1, ModernizedCProgram.st_add(ModernizedCProgram.st_add((/*Error: sizeof expression not supported yet*/), (flex_array_len_)), (true)));
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy((Object)generatedName, (generatedName), flex_array_len_);
	...
	public dir_entry hash_dir_entry_with_parent_and_prefix(index_state istate, strbuf prefix) {
		...
		(((parent != ((Object)0)) ^ (/*Error: Function owner not recognized*/strchr(generatedBuf, (byte)'/') == ((Object)0))) ? (Object)0 : /*Error: Function owner not recognized*/_assert("(parent != NULL) ^ (strchr(prefix->buf, '/') == NULL)", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\name-hash.c", 273));
		hashmap_entry generatedEnt = this.getEnt();
		int generatedHash = generatedEnt.getHash();
		int generatedNamelen = this.getNamelen();
		Object generatedLen = prefix.getLen();
		if (parent) {
			hash = ModernizedCProgram.memihash_cont(generatedHash, generatedBuf + generatedNamelen, generatedLen - generatedNamelen);
		} else {
		...
		if (!dir) {
			do {
				...
				(dir) = ModernizedCProgram.xcalloc(1, ModernizedCProgram.st_add(ModernizedCProgram.st_add((/*Error: sizeof expression not supported yet*/), (flex_array_len_)), (true)));
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy((Object)generatedName, (generatedBuf), flex_array_len_);
			} while (0);
			generatedEnt.hashmap_entry_init(hash);
			...
			if (parent) {
				...
				lock_nr = ModernizedCProgram.compute_dir_lock_nr(generatedDir_hash, generatedHash);
				ModernizedCProgram.lock_dir_mutex(lock_nr);
				generatedNr++;
			} 
		} 
	...
	public dir_entry dir_entry_new(Object pathname, int len) {
		...
		do {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy((Object)generatedName, (pathname), flex_array_len_);
		} while (0);
		ent.setLen(len);
		return ent;
	}
	...
	public dir_entry dir_add_ignored(dir_struct dir, index_state istate, Object pathname, int len) {
		...
		do {
			if ((generatedIgnored_nr + 1) > generatedIgnored_alloc) {
				...
				(generatedIgnored) = ModernizedCProgram.xrealloc((generatedIgnored), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedIgnored_alloc)));
			} 
		} while (0);
		dir_entry dir_entry = new dir_entry();
	...
	public void show_dir_entry(Object istate, Object tag) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fputs(tag, (_iob[1]));
	...
	public dir_entry getParent() {
		return parent;
	}
	public void setParent(dir_entry newParent) {
		parent = newParent;
	}
	public int getNr() {
		return nr;
