...
public class combine_diff_path {
	...
	public combine_diff_path path_appendnew(int nparent, Object base, Object path, int pathlen, int mode, Object oid) {
		...
		size_t alloclen = ModernizedCProgram.st_add(ModernizedCProgram.st_add(ModernizedCProgram.st_add((()), (((len)))), ((true))), (ModernizedCProgram.st_mult(, (nparent))));
		...
		.memcpy(generatedPath, base.getBuf(), base.getLen());
		.memcpy(generatedPath + base.getLen(), path, pathlen);
		generatedPath[len] = 0;
		p.setMode(mode);
	...
	public combine_diff_path emit_path(strbuf base, diff_options opt, int nparent, tree_desc t, tree_desc tp, int imin) {
		...
		int recurse = 0;
		int emitthis = 1;
		((t || tp) ? (Object)0 : ._assert("t || tp", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\tree-diff.c", /* at least something has to be valid */192));
		...
		if (emitthis) {
			...
			if (generatedPathchange) {
				ModernizedCProgram.keep = .UNRECOGNIZEDFUNCTIONNAME(opt, p/*
		...
		if (recurse) {
			...
			do {
				if ((nparent) <= 2) {
					(parents_oid) = (ModernizedCProgram.xmalloc((nparent) * ));
				} else {
						((parents_oid)) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (nparent)));
				} 
			} while (0);
			for (i = 0; i < nparent; ++i) {
	...
	public combine_diff_path ll_diff_tree_paths(Object oid, Object parents_oid, int nparent, strbuf base, diff_options opt) {
		...
		do {
			if ((nparent) <= 2) {
				(tp) = (ModernizedCProgram.xmalloc((nparent) * ));
			} else {
					((tp)) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (nparent)));
			} 
		} while (0);
		do {
			if ((nparent) <= 2) {
				(tptree) = (ModernizedCProgram.xmalloc((nparent) * ));
			} else {
					((tptree)) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (nparent)));
			} 
		} while (0/*
	...
	public combine_diff_path intersect_paths(int n, int num_parent, int combined_all_paths) {
		...
		if (!n) {
			for (i = 0; i < generatedNr; i++) {
				int len;
				byte path;
				if (.diff_unmodified_pair(generatedQueue[i])) {
					continue;
				} 
				ModernizedCProgram.path = generatedQueue[i].getTwo().getPath();
				ModernizedCProgram.len = .strlen(ModernizedCProgram.path);
				p = ModernizedCProgram.xmalloc(ModernizedCProgram.st_add(ModernizedCProgram.st_add(ModernizedCProgram.st_add((()), (((ModernizedCProgram.len)))), ((true))), (ModernizedCProgram.st_mult(, (num_parent)))));
				p.setPath((byte)(generatedParent[num_parent]));
				.memcpy(generatedPath, ModernizedCProgram.path, ModernizedCProgram.len);
				generatedPath[ModernizedCProgram.len] = 0;
				p.setNext(((Object)0));
				.memset(generatedParent, 0,  * num_parent);
