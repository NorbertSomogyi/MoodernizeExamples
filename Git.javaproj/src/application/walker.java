...
public class walker {
	...
	public int loop() {
		...
		while (ModernizedCProgram.process_queue) {
			...
			if (!(generatedFlags & (-1024 << 2))) {
				if (.UNRECOGNIZEDFUNCTIONNAME(walker, generatedHash)) {
			...
			if (ModernizedCProgram.process_object(walker, obj)) {
				return -1;
			} 
		}
		return 0;
	...
	public int walker_fetch(int targets, Byte target, Object write_ref, Object write_ref_log_details) {
		strbuf refname = new strbuf(, , );
		strbuf err = new strbuf(, , );
		ref_transaction transaction = ((Object)0);
		object_id oids = ModernizedCProgram.xmalloc(targets * );
		byte msg = ((Object)0);
		int i;
		int ret = -1;
		...
		if (!generatedGet_recover) {
			...
			.commit_list_sort_by_date(ModernizedCProgram.complete);
		} 
		object object = new object();
		for (i = 0; i < targets; i++) {
	...
	public void walker_free() {
		.UNRECOGNIZEDFUNCTIONNAME(walker);
	...
	public void prefetch(Byte sha1) {
		...
		newreq = ModernizedCProgram.xmalloc();
	...
	public void fetch_alternates(Object base) {
		...
		int generatedGet_verbosely = this.getGet_verbosely();
		if (generatedGet_verbosely) {
			.fprintf((_iob[2]), "Getting alternates list for %s\n", base);
		} 
		...
		.curl_easy_setopt(generatedCurl, CURLOPT_FILE, buffer);
		.curl_easy_setopt(generatedCurl, CURLOPT_WRITEFUNCTION, ModernizedCProgram.fwrite_buffer);
		byte generatedBuf = url.getBuf();
		.curl_easy_setopt(generatedCurl, CURLOPT_URL, generatedBuf);
	...
	public int fetch_object(Byte hash) {
		...
		ModernizedCProgram.normalize_curl_result(generatedCurl_result, generatedHttp_code, generatedErrorstr, );
	...
	public walker get_http_walker(Object url) {
		byte s;
		walker_data data = ModernizedCProgram.xmalloc();
		walker walker = ModernizedCProgram.xmalloc();
		alt_base generatedAlt = data.getAlt();
		data.setAlt(ModernizedCProgram.xmalloc());
		generatedAlt.setBase(ModernizedCProgram.xstrdup(url));
		Byte generatedBase = generatedAlt.getBase();
		for (s = generatedBase + .strlen(generatedBase) - 1; s == (byte)'/'; --s) {
			s = 0;
		}
