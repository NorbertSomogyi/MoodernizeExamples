...
public class strbuf {
	...
	public void add_branch(Object key, Object branchname, Object remotename, int mirror) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config_set_multivar(key, generatedBuf, "^$", 0);
	}
	public int do_command() {
		...
		if (generatedLen == 0) {
			if (batch_cmd) {
				...
				for (item = generatedItems; item && item < generatedItems + generatedNr; ++item) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedString);
				}
		...
		if (batch_cmd) {
			...
			string_list_item.string_list_append(batchlines, generatedBuf);
			return 0;
		} 
		for (p = ModernizedCProgram.input_command_list; p.getName(); p++) {
			if (ModernizedCProgram.starts_with(generatedBuf, p.getName()) && (/*Error: Function owner not recognized*/strlen(p.getName()) == generatedLen || generatedBuf[/*Error: Function owner not recognized*/strlen(p.getName())] == (byte)' ')) {
				if (p.getBatchable()) {
					batch_cmd = p;
					string_list_item.string_list_append(batchlines, generatedBuf);
					return 0;
				} 
				return /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedBuf);
			} 
		}
	...
	public void wrap_in_html() {
		...
		byte pre_open = "<pre>\n";
		byte pre_close = "</pre>\n";
		byte body = /*Error: Function owner not recognized*/strstr(ModernizedCProgram.msg.getBuf(), "\n\n");
		if (!body) {
	...
	public int count_messages() {
		...
		while (1) {
			if (ModernizedCProgram.starts_with(p, "From ")) {
				p = /*Error: Function owner not recognized*/strstr(p + 5, "\nFrom: ");
				if (!p) {
					break;
				} 
				p = /*Error: Function owner not recognized*/strstr(p + 7, "\nDate: ");
				if (!p) {
					break;
				} 
				p = /*Error: Function owner not recognized*/strstr(p + 7, "\nSubject: ");
	...
	public int split_msg(strbuf msg, Integer ofs) {
		byte p;
		byte data;
		...
		p = /*Error: Function owner not recognized*/strchr(data, (byte)'\n');
		if (p) {
			p++;
			len -= p - data;
			ofs += p - data;
		...
		p = /*Error: Function owner not recognized*/strstr(data, "\nFrom ");
		if (p) {
			len = p[1] - data;
		} 
	...
	public int show_one_reflog_ent(Object fn, Object cb_data) {
		...
		byte p = generatedBuf;
		Object generatedLen = this.getLen();
		if (!generatedLen || generatedBuf[generatedLen - 1] != /* old SP new SP name <email> SP time TAB msg LF */(byte)'\n' || ooid.parse_oid_hex(p, p) || p++ != (byte)' ' || noid.parse_oid_hex(p, p) || p++ != (byte)' ' || !(email_end = /*Error: Function owner not recognized*/strchr(p, (byte)'>')) || email_end[1] != (byte)' ' || !(timestamp = /*Error: Function owner not recognized*/strtoumax(email_end + 2, message, 10)) || !message || message[0] != (byte)' ' || (message[1] != (byte)'+' && message[1] != (byte)'-') || !((ModernizedCProgram.sane_ctype[(byte)(message[2])] & (true)) != 0) || !((ModernizedCProgram.sane_ctype[(byte)(message[3])] & (true)) != 0) || !((ModernizedCProgram.sane_ctype[(byte)(message[4])] & (true)) != 0) || !((ModernizedCProgram.sane_ctype[(byte)(message[5])] & (true)) != 0)) {
			return /* corrupt? */0;
		} 
		email_end[1] = (byte)'\0';
		tz = /*Error: Function owner not recognized*/strtol(message + 1, ((Object)0), 10);
		if (message[6] != (byte)'\t') {
			message += 6;
		} else {
				message += 7;
		} 
		return /*Error: Function owner not recognized*/fn(ooid, noid, p, timestamp, tz, message, cb_data);
	}
	public int show_tree(Object oid, Object pathname, int mode, int stage, Object context) {
		...
		if (!(ModernizedCProgram.ls_options & 8)) {
			if (ModernizedCProgram.ls_options & 16) {
				...
				if (!/*Error: Function owner not recognized*/strcmp(type, ModernizedCProgram.blob_type)) {
					long size;
					if (ModernizedCProgram.the_repository.oid_object_info(oid, size) == object_type.OBJ_BAD) {
						ModernizedCProgram.xsnprintf(size_text, /*Error: sizeof expression not supported yet*/, "BAD");
					} else {
							ModernizedCProgram.xsnprintf(size_text, /*Error: sizeof expression not supported yet*/, "%llu", (uintmax_t)size);
					} 
				} else {
						ModernizedCProgram.xsnprintf(size_text, /*Error: sizeof expression not supported yet*/, "-");
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%06o %s %s %7s\t", mode, type, ModernizedCProgram.the_repository.repo_find_unique_abbrev(oid, ModernizedCProgram.abbrev), size_text);
			} else {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%06o %s %s\t", mode, type, ModernizedCProgram.the_repository.repo_find_unique_abbrev(oid, ModernizedCProgram.abbrev));
			} 
		} 
	...
	public void append_basename(Object dir, Object file) {
		byte tail = /*Error: Function owner not recognized*/strrchr(file, (byte)'/');
	...
	public void resolve_symlink() {
		int depth = 5;
		strbuf link = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public void unable_to_lock_message(Object path, int err) {
		if (err == 17) {
			ModernizedCProgram.buf.strbuf_addf(ModernizedCProgram._("Unable to create '%s.lock': %s.\n\nAnother git process seems to be running in this repository, e.g.\nan editor opened by 'git commit'. Please make sure all processes\nare terminated then try again. If it still fails, a git process\nmay have crashed in this repository earlier:\nremove the file manually to continue."), ModernizedCProgram.absolute_path(path), /*Error: Function owner not recognized*/strerror(err));
		} else {
				ModernizedCProgram.buf.strbuf_addf(ModernizedCProgram._("Unable to create '%s.lock': %s"), ModernizedCProgram.absolute_path(path), /*Error: Function owner not recognized*/strerror(err));
		} 
	}
	public int name_and_item_from_var(Object var, strbuf item) {
		...
		int subsection_len;
		int parse;
		parse = /*Error: Function owner not recognized*/parse_config_key(var, "submodule", subsection, subsection_len, key);
	...
	public void sq_quote_buf(Object src) {
		...
		while (src) {
			size_t len = /*Error: Function owner not recognized*/strcspn(src, "'!");
	...
	public void sq_quote_buf_pretty(Object src) {
		...
		for (p = src; p; p++) {
			if (!((ModernizedCProgram.sane_ctype[(byte)(p)] & (true)) != 0) && !((ModernizedCProgram.sane_ctype[(byte)(p)] & (true)) != 0) && !/*Error: Function owner not recognized*/strchr(ok_punct, p)) {
	...
	public Byte quote_path_relative(Object in, Object prefix) {
		...
		out.strbuf_setlen(0);
		ModernizedCProgram.quote_c_style_counted(rel, /*Error: Function owner not recognized*/strlen(rel), out, ((Object)0), 0);
		sb.strbuf_release();
		byte[] generatedBuf = this.getBuf();
		return generatedBuf;
	}
	public int unquote_c_style(Object quoted, Object endp) {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			len = /*Error: Function owner not recognized*/strcspn(quoted, "\"\\");
	...
	public void remove_subtree() {
		...
		while ((de = dirent.readdir(dir)) != ((Object)0)) {
			...
			if (/*Error: Function owner not recognized*/lstat(ModernizedCProgram.path.getBuf(), st)) {
			...
			}  else if (/*Error: Function owner not recognized*/unlink(ModernizedCProgram.path.getBuf())) {
				ModernizedCProgram.die_errno("cannot unlink '%s'", ModernizedCProgram.path.getBuf());
			} 
			ModernizedCProgram.path.strbuf_setlen(origlen);
		}
		dir.closedir();
		if (/*Error: Function owner not recognized*/rmdir(ModernizedCProgram.path.getBuf())) {
	...
	public int sign_buffer(strbuf signature, Object signing_key) {
		...
		ret |=  !/*Error: Function owner not recognized*/strstr(generatedBuf, "\n[GNUPG:] SIG_CREATED ");
	...
	public int read_patch_file(int fd) {
		...
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedBuf + generatedLen, 0, (true));
		return 0;
	}
	public Byte find_name_gnu(Object line, int p_value) {
		...
		for (cp = generatedBuf; p_value; p_value--) {
			cp = /*Error: Function owner not recognized*/strchr(cp, (byte)'/');
			if (!cp) {
				name.strbuf_release();
				return ((Object)0);
			} 
	...
	public Byte find_name_common(Object line, Object def, int p_value, Object end, int terminate) {
		...
		if (def) {
			int deflen = /*Error: Function owner not recognized*/strlen(def);
			if (deflen < len && !/*Error: Function owner not recognized*/strncmp(start, def, deflen)) {
	...
	public int interpret_empty_at(Object[] name, int namelen, int len) {
		...
		next = /*Error: Function owner not recognized*/memchr(name + len + 1, (byte)'@', namelen - len - /* make sure it's a single @, or @@{.*}, not @foo */1);
		if (next && next[1] != (byte)'{') {
			return -1;
		} 
		if (!next) {
	...
	public int strbuf_check_branch_ref(Object name) {
		...
		byte[] generatedBuf = this.getBuf();
		if (name == (byte)'-' || !/*Error: Function owner not recognized*/strcmp(generatedBuf, "refs/heads/HEAD")) {
			return -1;
		} 
	...
	public int encode_to_git(Object path, Object src, Object src_len, Object enc, int conv_flags) {
		...
		if (die_on_error && ModernizedCProgram.check_roundtrip(enc)) {
			...
			if (!re_src || src_len != re_src_len || /*Error: Function owner not recognized*/memcmp(src, re_src, src_len)) {
	...
	public int crlf_to_git(Object istate, Object path, Object src, Object len, crlf_action crlf_action, int conv_flags) {
		...
		if (((conv_flags & (1 << 1)) || ((conv_flags & (1 << 0)) && len))) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(new_stats, stats, /*Error: sizeof expression not supported yet*/);
	...
	public int crlf_to_worktree(Object src, Object len, crlf_action crlf_action) {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			byte nl = /*Error: Function owner not recognized*/memchr(src, (byte)'\n', len);
			if (!nl) {
				break;
			} 
	...
	public int apply_single_file_filter(Object path, Object src, Object len, int fd, Object cmd) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(async, 0, /*Error: sizeof expression not supported yet*/);
		...
		if (/*Error: Function owner not recognized*/close(generatedOut)) {
			err = ();
		} 
	...
	public int ident_to_git(Object src, Object len, int ident) {
		...
		dst = ModernizedCProgram.buf.getBuf();
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			dollar = /*Error: Function owner not recognized*/memchr(src, (byte)'$', len);
			if (!dollar) {
				break;
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(dst, src, dollar + 1 - src);
			dst += dollar + 1 - src;
			len -= dollar + 1 - src;
			src = dollar + 1;
			if (len > 3 && !/*Error: Function owner not recognized*/memcmp(src, "Id:", 3)) {
				dollar = /*Error: Function owner not recognized*/memchr(src + 3, (byte)'$', len - 3);
				if (!dollar) {
					break;
				} 
				if (/*Error: Function owner not recognized*/memchr(src + 3, (byte)'\n', dollar - src - 3/* Line break before the next dollar. */)) {
					continue;
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(dst, "Id$", 3);
				dst += 3;
				len -= dollar + 1 - src;
				src = dollar + 1;
			} 
		}
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(dst, src, len);
	...
	public int ident_to_worktree(Object[] src, Object len, int ident) {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			dollar = /*Error: Function owner not recognized*/memchr(src, (byte)'$', /* step 1: run to the next '$' */len);
			if (!dollar) {
				break;
			} 
			...
			len -= dollar + 1 - src;
			src = dollar + 1;
			if (len < 3 || /*Error: Function owner not recognized*/memcmp("Id", src, /* step 2: does it looks like a bit like Id:xxx$ or Id$ ? */2)) {
				continue;
			} 
			if (src[2] == /* step 3: skip over Id$ or Id:xxxxx$ */(byte)'$') {
				...
				spc = /*Error: Function owner not recognized*/memchr(src + 4, (byte)' ', dollar - src - 4);
	...
	public void convert_to_git_filter_fd(Object istate, Object path, int fd, int conv_flags) {
		...
		ca.convert_attrs(istate, path);
		convert_driver generatedDrv = ca.getDrv();
		((generatedDrv) ? (Object)0 : /*Error: Function owner not recognized*/_assert("ca.drv", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\convert.c", 1447));
		Object generatedClean = generatedDrv.getClean();
		Object generatedProcess = generatedDrv.getProcess();
		((generatedClean || generatedProcess) ? (Object)0 : /*Error: Function owner not recognized*/_assert("ca.drv->clean || ca.drv->process", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\convert.c", 1448));
		Object[] generatedName = generatedDrv.getName();
	...
	public int has_reserved_character(strbuf errbuf) {
		...
		byte c = generatedBuf;
		while (c) {
			if (c <= (byte)' ' || /*Error: Function owner not recognized*/strchr(ModernizedCProgram.RESERVED_NON_WS, c)) {
	...
	public int migrate_one(strbuf dst) {
		...
		Object generatedSt_mode = st.getSt_mode();
		if ((((generatedSt_mode) & -1024) == -1024)) {
			if (!/*Error: Function owner not recognized*/mkdir(generatedBuf, 777)) {
	...
	public int find_and_replace(Object needle, Object placeholder) {
		...
		int plen;
		int nlen;
		nlen = /*Error: Function owner not recognized*/strlen(needle);
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		if (ModernizedCProgram.ends_with(generatedBuf, needle)) {
			p = generatedBuf + generatedLen - nlen;
		} else {
				p = /*Error: Function owner not recognized*/strstr(generatedBuf, needle);
		} 
		if (!p) {
			return 0;
		} 
		...
		plen = /*Error: Function owner not recognized*/strlen(p);
		if (plen > nlen && p[nlen] != (byte)'/') {
			return 0;
		} 
		haystack.strbuf_splice(p - generatedBuf, nlen, placeholder, /*Error: Function owner not recognized*/strlen(placeholder));
		return 1;
	}
	public void print_compact(Object remote, Object local) {
		strbuf r = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf l = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		if (!/*Error: Function owner not recognized*/strcmp(remote, local)) {
	...
	public void format_display(byte code, Object summary, Object error, Object remote, Object local, int summary_width) {
		int width = (summary_width + /*Error: Function owner not recognized*/strlen(summary) - ModernizedCProgram.gettext_width(summary));
	...
	public void ws_fix_copy(Object[] src, int len, int ws_rule, Integer error_count) {
		int i;
	...
	public void set_reflog_message(Object action, Object rev) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/setenv("GIT_REFLOG_ACTION", generatedBuf, 0);
		msg/**
	...
	public void datestamp() {
		...
		offset = ModernizedCProgram.tm_to_time_t(/*Error: Function owner not recognized*/localtime(now)) - now;
		offset /= 60;
		out/*
	...
	public void format_subst(Object commit, Object src, Object len) {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			byte b;
			byte c;
			b = /*Error: Function owner not recognized*/memmem(src, len, "$Format:", 8);
			if (!b) {
				break;
			} 
			c = /*Error: Function owner not recognized*/memchr(b + 8, (byte)'$', (src + len) - b - 8);
			if (!c) {
				break;
			} 
			fmt.strbuf_setlen(0);
	...
	public int queue_or_write_archive_entry(Object oid, Object filename, int mode, int stage, Object context) {
		...
		while (generatedBottom && !(generatedLen >= generatedLen && !/*Error: Function owner not recognized*/strncmp(generatedBuf, generatedPath, generatedLen))) {
	...
	public int check_refname_component(Object[] refname, Integer flags) {
		byte cp;
		byte last = (byte)'\0';
		...
		byte[] generatedBuf = this.getBuf();
		for (cp = refname; /*Error: Unsupported expression*/; cp++) {
			int ch = cp & 255;
		...
		if (cp - refname >= 5 && !/*Error: Function owner not recognized*/memcmp(cp - 5, ".lock", 5)) {
			if (!sanitized) {
				return -1;
			} 
	...
	public int check_or_sanitize_refname(Object[] refname, int flags) {
		...
		if (!/*Error: Function owner not recognized*/strcmp(refname, "@")) {
	...
	public int write_pseudoref(Object pseudoref, Object oid, Object old_oid) {
		...
		if (fd < 0) {
			ModernizedCProgram.err.strbuf_addf(ModernizedCProgram._("could not open '%s' for writing: %s"), filename, /*Error: Function owner not recognized*/strerror((/*Error: Function owner not recognized*/_errno())));
			;
		} 
		if (old_oid) {
	...
	public int refs_init_db() {
		...
		return /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(refs, ModernizedCProgram.err);
	}
		...
		if (generatedNr) {
			...
			do {
				if ((generatedOid_fetch_tasks_nr + 1) > generatedOid_fetch_tasks_alloc) {
					...
					(generatedOid_fetch_tasks) = ModernizedCProgram.xrealloc((generatedOid_fetch_tasks), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedOid_fetch_tasks_alloc)));
				} 
			} while (0);
	...
	public int show_tree_object(Object oid, Object pathname, int mode, int stage, Object context) {
		FILE file = context;
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf(file, "%s%s\n", pathname, (((mode) & -1024) == -1024) ? "/" : "");
		return 0;
	}
	public void add_branch_description(Object branch_name) {
		strbuf desc = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public int unlink_or_msg(Object file) {
		int rc = /*Error: Function owner not recognized*/unlink(file);
		((ModernizedCProgram.err) ? (Object)0 : /*Error: Function owner not recognized*/_assert("err", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\wrapper.c", 545));
		if (!rc || (/*Error: Function owner not recognized*/_errno()) == 2) {
			return 0;
		} 
		ModernizedCProgram.err.strbuf_addf("unable to unlink '%s': %s", file, /*Error: Function owner not recognized*/strerror((/*Error: Function owner not recognized*/_errno())));
		return -1;
	}
		...
		if (idx < generatedNr) {
			...
			do {
				if ((generatedFailed_clones_nr + 1) > generatedFailed_clones_alloc) {
					...
					(generatedFailed_clones) = ModernizedCProgram.xrealloc((generatedFailed_clones), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedFailed_clones_alloc)));
				} 
			} while (0);
	...
	public int prune_worktree(Object id) {
		...
		if (/*Error: Function owner not recognized*/stat(ModernizedCProgram.git_path("worktrees/%s/gitdir", id), st)) {
			reason.strbuf_addf(ModernizedCProgram._("Removing worktrees/%s: gitdir file does not exist"), id);
			return 1;
		} 
		fd = /*Error: Function owner not recognized*/open(ModernizedCProgram.git_path("worktrees/%s/gitdir", id), 0);
		if (fd < 0) {
			reason.strbuf_addf(ModernizedCProgram._("Removing worktrees/%s: unable to read gitdir file (%s)"), id, /*Error: Function owner not recognized*/strerror((/*Error: Function owner not recognized*/_errno())));
			return 1;
		} 
		...
		if (!ModernizedCProgram.file_exists(path)) {
			ModernizedCProgram.free(path);
			if (/*Error: Function owner not recognized*/stat(ModernizedCProgram.git_path("worktrees/%s/index", id), st) || generatedSt_mtime <= ModernizedCProgram.expire) {
	...
	public int test_finished(int result, Object cb, Object task_cb) {
		...
		string_list_item string_list_item = new string_list_item();
		if (result) {
			string_list_item.string_list_append(generatedFailed, name);
		} 
	...
	public void not_found(Object err) {
		...
		hdr.hdr_nocache();
		hdr.end_headers();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_start(params, err);
		if (err && err) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/vfprintf((_iob[2]), err, params);
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_end(params);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\http-backend.c", 139, (false)));
	}
	public void forbidden(Object err) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_end(params);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\http-backend.c", 155, (false)));
	}
	...
	public void send_local_file(Object the_type, Object name) {
		...
		int fd;
		stat sb = new stat();
		fd = /*Error: Function owner not recognized*/open(p, 0);
		if (fd < 0) {
			hdr.not_found("Cannot open '%s': %s", p, /*Error: Function owner not recognized*/strerror((/*Error: Function owner not recognized*/_errno())));
		} 
		if (/*Error: Function owner not recognized*/fstat(fd, sb) < 0) {
	...
	public void get_info_packs(Byte arg) {
		size_t objdirlen = /*Error: Function owner not recognized*/strlen(ModernizedCProgram.get_object_directory());
		strbuf buf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public void check_content_type(Object accepted_type) {
		byte actual_type = /*Error: Function owner not recognized*/getenv("CONTENT_TYPE");
		if (!actual_type) {
			actual_type = "";
		} 
		if (/*Error: Function owner not recognized*/strcmp(actual_type, accepted_type)) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\http-backend.c", 632, (false)));
		} 
	}
	...
	public int bad_request(Object c) {
		byte proto = /*Error: Function owner not recognized*/getenv("SERVER_PROTOCOL");
		if (proto && !/*Error: Function owner not recognized*/strcmp(proto, "HTTP/1.1")) {
			hdr.http_status(405, "Method Not Allowed");
			hdr.hdr_str("Allow", !/*Error: Function owner not recognized*/strcmp(c.getMethod(), "GET") ? "GET, HEAD" : c.getMethod());
		} else {
	...
	public void show_config_origin() {
		...
		ModernizedCProgram.buf.strbuf_addstr(/*Error: Function owner not recognized*/current_config_origin_type());
		ModernizedCProgram.buf.strbuf_addch((byte)':');
		if (ModernizedCProgram.end_null) {
			ModernizedCProgram.buf.strbuf_addstr(/*Error: Function owner not recognized*/current_config_name());
		} else {
				ModernizedCProgram.quote_c_style(/*Error: Function owner not recognized*/current_config_name(), ModernizedCProgram.buf, ((Object)0), 0);
		} 
	...
	public Byte compute_alternate_path(Object path) {
		...
		if (!/*Error: Function owner not recognized*/access(ModernizedCProgram.mkpath("%s/shallow", ref_git), 0)) {
			ModernizedCProgram.err.strbuf_addf(ModernizedCProgram._("reference repository '%s' is shallow"), path);
			seen_error = 1;
			;
		} 
		if (!/*Error: Function owner not recognized*/access(ModernizedCProgram.mkpath("%s/info/grafts", ref_git), 0)) {
	...
	public int create_tmpfile(Object filename) {
		...
		if (fd < 0 && dirlen && (/*Error: Function owner not recognized*/_errno()) == 2/*
			...
			if (/*Error: Function owner not recognized*/mkdir(generatedBuf, 777) && (/*Error: Function owner not recognized*/_errno()) != 17) {
				return -1;
			} 
			if (ModernizedCProgram.adjust_shared_perm(generatedBuf)) {
				return -1;
	...
	public int for_each_file_in_obj_subdir(int subdir_nr, Object obj_cb, Object cruft_cb, Object subdir_cb, Object data) {
		...
		while ((de = dirent.readdir(dir))) {
			...
			namelen = /*Error: Function owner not recognized*/strlen(generatedD_name);
			...
			if (namelen == ModernizedCProgram.the_repository.getHash_algo().getHexsz() - 2 && !ModernizedCProgram.hex_to_bytes(generatedHash + 1, generatedD_name, ModernizedCProgram.the_repository.getHash_algo().getRawsz() - 1)) {
				if (obj_cb) {
					r = /*Error: Function owner not recognized*/obj_cb(oid, ModernizedCProgram.path.getBuf(), data);
					if (r) {
						break;
					} 
				} 
			...
			if (cruft_cb) {
				r = /*Error: Function owner not recognized*/cruft_cb(generatedD_name, ModernizedCProgram.path.getBuf(), data);
				if (r) {
					break;
				} 
			} 
		...
		if (!r && subdir_cb) {
			r = /*Error: Function owner not recognized*/subdir_cb(subdir_nr, ModernizedCProgram.path.getBuf(), data);
		} 
		ModernizedCProgram.path.strbuf_setlen(origlen);
		return r;
	}
	...
	public Object get_repo_path_1(Integer is_bundle) {
		...
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); i++) {
			...
			if (/*Error: Function owner not recognized*/stat(ModernizedCProgram.path.getBuf(), st)) {
				continue;
			} 
			if ((((generatedSt_mode) & -1024) == -1024) && ModernizedCProgram.is_git_directory(ModernizedCProgram.path.getBuf())) {
				...
				byte dst;
				int len;
				int fd = /*Error: Function owner not recognized*/open(ModernizedCProgram.path.getBuf(), 0);
				if (fd < 0) {
					continue;
				} 
				ModernizedCProgram.len = ModernizedCProgram.read_in_full(fd, ModernizedCProgram.signature, 8);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/close(fd);
				if (ModernizedCProgram.len != 8 || /*Error: Function owner not recognized*/strncmp(ModernizedCProgram.signature, "gitdir: ", 8)) {
					continue;
				} 
		...
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); i++) {
			...
			if (!/*Error: Function owner not recognized*/stat(ModernizedCProgram.path.getBuf(), st) && (((generatedSt_mode) & -1024) == -1024)) {
				is_bundle = 1;
				return ModernizedCProgram.path.getBuf();
			} 
		}
	...
	public void copy_or_link_directory(strbuf dest, Object src_repo) {
		...
		while ((iter_status = iter.dir_iterator_advance()) == 0) {
			...
			if (/*Error: Function owner not recognized*/unlink(generatedBuf) && (/*Error: Function owner not recognized*/_errno()) != 2) {
	...
	public void write_refspec_config(Object src_ref_prefix, Object our_head_points_at, Object remote_head_points_at) {
		strbuf key = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf value = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		byte[] generatedBuf = this.getBuf();
		...
		if (ModernizedCProgram.option_mirror || !ModernizedCProgram.option_bare) {
			...
			if (generatedLen) {
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config_set_multivar(generatedBuf, generatedBuf, "^$", 0);
				key.strbuf_setlen(0);
				if (ModernizedCProgram.option_mirror) {
					key.strbuf_addf("remote.%s.mirror", ModernizedCProgram.option_origin);
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config_set(generatedBuf, "true");
					key.strbuf_setlen(0);
				} 
			} 
		} 
	...
	public void append_todo_help(int keep_empty, int command_count, Object shortrevisions, Object shortonto) {
		...
		ModernizedCProgram.buf.strbuf_add_commented_lines(msg, /*Error: Function owner not recognized*/strlen(msg));
	...
	public void strbuf_append_ext_header(Object keyword, Object value, Object valuelen) {
		...
		len = 1 + 1 + /*Error: Function owner not recognized*/strlen(keyword) + 1 + valuelen + /* "%u %s=%s\n" */1;
	...
	public void strbuf_append_ext_header_uint(Object keyword, Object value) {
		...
		int len;
		len = ModernizedCProgram.xsnprintf(buf, /*Error: sizeof expression not supported yet*/, "%llu", value);
	...
	public void get_html_page_path(Object page) {
		...
		Object generatedSt_mode = st.getSt_mode();
		if (!/*Error: Function owner not recognized*/strstr(ModernizedCProgram.html_path, /* Check that we have a git documentation directory. */"://")) {
			if (/*Error: Function owner not recognized*/stat(ModernizedCProgram.mkpath("%s/git.html", ModernizedCProgram.html_path), st) || !(((generatedSt_mode) & -1024) == -1024)) {
	...
	public strbuf get_pathname() {
		strbuf[] pathname_array = new strbuf[]{new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/), new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/), new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/), new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/)};
		int index;
		strbuf sb = pathname_array[index];
		index = (index + 1) % (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1));
		sb.strbuf_setlen(0);
		return sb;
	}
	...
	public void replace_dir(int len, Object[] newdir) {
		int newlen = /*Error: Function owner not recognized*/strlen(newdir);
	...
	public void adjust_git_path(Object repo, int git_dir_len) {
		...
		if (ModernizedCProgram.is_dir_file(base, "info", "grafts")) {
			ModernizedCProgram.buf.strbuf_splice(0, ModernizedCProgram.buf.getLen(), repo.getGraft_file(), /*Error: Function owner not recognized*/strlen(repo.getGraft_file()));
		}  else if (!/*Error: Function owner not recognized*/strcmp(base, "index")) {
			ModernizedCProgram.buf.strbuf_splice(0, ModernizedCProgram.buf.getLen(), repo.getIndex_file(), /*Error: Function owner not recognized*/strlen(repo.getIndex_file()));
	...
	public int do_submodule_path(Object path, Object fmt, Object args) {
		strbuf git_submodule_common_dir = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf git_submodule_dir = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int ret;
	...
	public Object relative_path(Object[] in, Object[] prefix) {
		int in_len = in ? /*Error: Function owner not recognized*/strlen(in) : 0;
		int prefix_len = prefix ? /*Error: Function owner not recognized*/strlen(prefix) : 0;
		int in_off = 0;
		int prefix_off = 0;
		int i = 0;
		int j = 0;
	...
	public void determine_author_info() {
		...
		name = ModernizedCProgram.xstrdup_or_null(/*Error: Function owner not recognized*/getenv("GIT_AUTHOR_NAME"));
		email = ModernizedCProgram.xstrdup_or_null(/*Error: Function owner not recognized*/getenv("GIT_AUTHOR_EMAIL"));
		date = ModernizedCProgram.xstrdup_or_null(/*Error: Function owner not recognized*/getenv("GIT_AUTHOR_DATE"));
		...
		if (ModernizedCProgram.author_message) {
			...
			size_t len = new size_t();
			byte a;
			a = /*Error: Function owner not recognized*/find_commit_header(ModernizedCProgram.author_message_buffer, "author", ModernizedCProgram.len);
			if (!a) {
			...
			if (generatedDate_begin) {
				strbuf date_buf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (ModernizedCProgram.force_author) {
			ident_split ident = new ident_split();
			if (ident.split_ident_line(ModernizedCProgram.force_author, /*Error: Function owner not recognized*/strlen(ModernizedCProgram.force_author)) < 0) {
	...
	public void strbuf_replace(Object a, Object b) {
		byte[] generatedBuf = this.getBuf();
		byte ptr = /*Error: Function owner not recognized*/strstr(generatedBuf, a);
		if (ptr) {
			sb.strbuf_splice(ptr - generatedBuf, /*Error: Function owner not recognized*/strlen(a), b, /*Error: Function owner not recognized*/strlen(b));
		} 
	}
	...
	public void format_trailer_info(Object info, Object opts) {
		...
		for (i = 0; i < info.getTrailer_nr(); i++) {
			...
			if (separator_pos >= 1) {
				...
				if (!opts.getFilter() || /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(tok, opts.getFilter_data())) {
					if (opts.getUnfold()) {
						val.unfold_value();
					} 
	...
	public void wt_status_append_cut_line() {
		...
		ModernizedCProgram.buf.strbuf_add_commented_lines(explanation, /*Error: Function owner not recognized*/strlen(explanation));
	}
	...
	public void perf_fmt_prepare(Object event_name, Object ctx, Object file, int line, Object repo, Object p_us_elapsed_absolute, Object p_us_elapsed_relative, Object category) {
		...
		if (!ModernizedCProgram.tr2env_perf_be_brief) {
			...
			if (file && file) {
				strbuf buf_fl = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public void add_rfc2047(Object line, Object len, Object encoding, rfc2047_type type) {
		...
		sb.strbuf_grow(len * 3 + /*Error: Function owner not recognized*/strlen(encoding) + 100);
		sb.strbuf_addf("=?%s?q?", encoding);
		line_len += /*Error: Function owner not recognized*/strlen(encoding) + /* 5 for =??q? */5;
		while (len/*
			...
			if (line_len + encoded_len + 2 > max_encoded_length) {
				...
				line_len = /*Error: Function owner not recognized*/strlen(encoding) + 5 + /* =??q? plus SP */1;
			} 
			for (i = 0; i < chrlen; i++) {
	...
	public Object format_commit_one(Object[] placeholder, Object context) {
		...
		case (byte)'C':
				if (ModernizedCProgram.starts_with(placeholder + 1, "(auto)")) {
					...
					return /* consumed 7 bytes, "C(auto)" */7;
						...
						int ret = ModernizedCProgram.parse_color(sb, placeholder, c);
						if (ret) {
							c.setAuto_color(0/*
		...
		case (byte)'w':
				if (placeholder[1] == (byte)'(') {
					...
					byte end = /*Error: Function owner not recognized*/strchr(start, (byte)')');
					if (!end) {
						return 0;
					} 
					if (end > start) {
						width = /*Error: Function owner not recognized*/strtoul(start, next, 10);
						if (next == (byte)',') {
							indent1 = /*Error: Function owner not recognized*/strtoul(next + 1, next, 10);
							if (next == (byte)',') {
								indent2 = /*Error: Function owner not recognized*/strtoul(next + 1, next, 10);
							} 
						} 
						if (next != (byte)')') {
							return 0;
		...
		case /* commit hash */(byte)'H':
				sb.strbuf_addstr(/*Error: Function owner not recognized*/diff_get_color(generatedAuto_color, color_diff.DIFF_COMMIT));
				sb.strbuf_addstr(ModernizedCProgram.oid_to_hex(commit.getObject().getOid()));
				sb.strbuf_addstr(/*Error: Function owner not recognized*/diff_get_color(generatedAuto_color, color_diff.DIFF_RESET));
				return 1;
		...
		case /* abbreviated tree hash */(byte)'t':
				sb.strbuf_add_unique_abbrev(/*Error: Function owner not recognized*/get_commit_tree_oid(commit), generatedPretty_ctx.getAbbrev());
				return 1;
		case (byte)'N':
		...
		case /* tree hash */(byte)'T':
				sb.strbuf_addstr(ModernizedCProgram.oid_to_hex(/*Error: Function owner not recognized*/get_commit_tree_oid(commit)));
				return 1;
		...
		if (placeholder[0] == (byte)'G') {
			if (!generatedResult) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/check_commit_signature(generatedCommit, (generatedSignature_check));
			} 
			switch (placeholder[1]) {
			case (byte)'S':
					if (generatedSigner) {
	...
	public void strbuf_add_tabexpand(int tabwidth, Object line, int linelen) {
		byte tab;
		while ((tab = /*Error: Function owner not recognized*/memchr(line, (byte)'\t', linelen)) != ((Object)0)) {
	...
	public int parse_data(Object limit, Object len_res) {
		...
		if (ModernizedCProgram.skip_prefix(data, "<<", data)) {
			...
			for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
				...
				if (term_len == ModernizedCProgram.command_buf.getLen() && !/*Error: Function owner not recognized*/strcmp(ModernizedCProgram.term, ModernizedCProgram.command_buf.getBuf())) {
					break;
				} 
			...
			ModernizedCProgram.free(ModernizedCProgram.term);
				...
				uintmax_t len = /*Error: Function owner not recognized*/strtoumax(data, ((Object)0), 10);
				size_t n = 0;
	...
	public int validate_raw_date(Object src) {
		...
		num = /*Error: Function owner not recognized*/strtoul(src, endp, 10);
		if ((/*Error: Function owner not recognized*/_errno()) || endp == src || endp != /* NEEDSWORK: perhaps check for reasonable values? */(byte)' ') {
			return -1;
		} 
		src = endp + 1;
		...
		num = /*Error: Function owner not recognized*/strtoul(src + 1, endp, 10);
		if ((/*Error: Function owner not recognized*/_errno()) || endp == src + 1 || endp || 1400 < num) {
			return -1;
		} 
	...
	public void trace_add_env(Object deltaenv) {
		...
		for (e = deltaenv; e && e; /* Last one wins, see run-command.c:prep_childenv() for context */e++) {
			strbuf key = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
			byte equals = /*Error: Function owner not recognized*/strchr(e, (byte)'=');
			if (equals) {
		...
		for (i = 0; i < generatedNr; /* "unset X Y...;" */i++) {
			...
			if (val || !/*Error: Function owner not recognized*/getenv(var)) {
				continue;
			} 
			if (!printed_unset) {
		...
		for (i = 0; i < generatedNr; /* ... followed by "A=B C=D ..." */i++) {
			...
			oldval = /*Error: Function owner not recognized*/getenv(var);
			if (oldval && !/*Error: Function owner not recognized*/strcmp(val, oldval)) {
				continue;
			} 
			dst.strbuf_addf(" %s=", var);
	...
	public int remove_dir_recurse(int flag, Integer kept_up) {
		DIR dir = new DIR();
		dirent e = new dirent();
		int ret = 0;
		...
		if (!dir) {
			...
			}  else if ((/*Error: Function owner not recognized*/_errno()) == 13 && !keep_toplevel/*
						 * An empty dir could be removable even if it
						 * is unreadable:
						 */) {
				return /*Error: Function owner not recognized*/rmdir(ModernizedCProgram.path.getBuf());
			} else {
					return -1;
			} 
		} 
		...
		while ((e = dirent.readdir(dir)) != ((Object)0)) {
			...
			}  else if (!only_empty && (!/*Error: Function owner not recognized*/unlink(ModernizedCProgram.path.getBuf()) || (/*Error: Function owner not recognized*/_errno()) == 2)) {
				continue;
			} 
		...
		if (!ret && !keep_toplevel && !kept_down) {
			ret = (!/*Error: Function owner not recognized*/rmdir(ModernizedCProgram.path.getBuf()) || (/*Error: Function owner not recognized*/_errno()) == 2) ? 0 : -1;
		}  else if (kept_up/*
	...
	public int parse_ref_filter_atom(Object format, Object atom, Object ep) {
		...
		for (i = 0; i < ModernizedCProgram.used_atom_cnt; /* Do we have the atom already used elsewhere? */i++) {
			int len = /*Error: Function owner not recognized*/strlen(used_atom[i].getName());
			if (ModernizedCProgram.len == ep - atom && !/*Error: Function owner not recognized*/memcmp(used_atom[i].getName(), atom, ModernizedCProgram.len)) {
				return i/*
		...
		arg = /*Error: Function owner not recognized*/memchr(sp, (byte)':', ep - sp);
		atom_len = (arg ? arg : ep) - sp;
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); /* Is the atom a valid one? */i++) {
			int len = /*Error: Function owner not recognized*/strlen(valid_atom[i].getName());
			if (ModernizedCProgram.len == atom_len && !/*Error: Function owner not recognized*/memcmp(valid_atom[i].getName(), sp, ModernizedCProgram.len)) {
				break;
			} 
		}
		...
		(used_atom) = ModernizedCProgram.xrealloc((used_atom), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (ModernizedCProgram.used_atom_cnt)));
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(used_atom[at].getU(), 0, /*Error: sizeof expression not supported yet*/);
		if (valid_atom[i].getParser() && /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(format, used_atom[at], arg, ModernizedCProgram.err)) {
			return -1;
		} 
		if (atom == (byte)'*') {
			ModernizedCProgram.need_tagged = 1;
		} 
		if (!/*Error: Function owner not recognized*/strcmp(valid_atom[i].getName(), "symref")) {
	...
	public void append_lines(Object buf, long size, int lines) {
		...
		for (i = 0; i < lines && sp < buf + size; i++) {
			...
			eol = /*Error: Function owner not recognized*/memchr(sp, (byte)'\n', size - (sp - buf));
	...
	public void strbuf_grow(Object extra) {
		...
		Object generatedLen = this.getLen();
		if (((true) > (-1024 >> ((8 * /*Error: Unsupported expression*/) - (8 * /*Error: sizeof expression not supported yet*/))) - (extra)) || ((extra + 1) > (-1024 >> ((8 * /*Error: Unsupported expression*/) - (8 * /*Error: sizeof expression not supported yet*/))) - (generatedLen))) {
		...
		do {
			if ((generatedLen + extra + 1) > generatedAlloc) {
				...
				(generatedBuf) = ModernizedCProgram.xrealloc((generatedBuf), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (generatedAlloc)));
			} 
		} while (0);
		if (new_buf) {
	...
	public void strbuf_ltrim() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(generatedBuf, b, generatedLen);
		generatedBuf[generatedLen] = (byte)'\0';
	}
	...
	public strbuf strbuf_split_buf(Object str, Object slen, int terminator, int max) {
		...
		while (slen) {
			int len = slen;
			if (max <= 0 || nr + 1 < max) {
				byte end = /*Error: Function owner not recognized*/memchr(str, terminator, slen);
				if (end) {
					ModernizedCProgram.len = end - str + 1;
				} 
			} 
			t = ModernizedCProgram.xmalloc(/*Error: Unsupported expression*/);
			...
			do {
				if ((nr + 2) > alloc) {
					...
					(ret) = ModernizedCProgram.xrealloc((ret), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (alloc)));
				} 
			} while (0);
			ret[nr++] = t;
	...
	public void strbuf_splice(Object pos, Object len, Object data, Object dlen) {
		if (((len) > (-1024 >> ((8 * /*Error: Unsupported expression*/) - (8 * /*Error: sizeof expression not supported yet*/))) - (pos))) {
			ModernizedCProgram.die("you want to use way too much memory");
		} 
		Object generatedLen = this.getLen();
		if (pos > generatedLen) {
		...
		byte[] generatedBuf = this.getBuf();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(generatedBuf + pos + dlen, generatedBuf + pos + len, generatedLen - pos - len);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedBuf + pos, data, dlen);
	...
	public void strbuf_vinsertf(Object pos, Object fmt, Object ap) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_copy(cp, ap);
		byte[] generatedBuf = this.getBuf();
		len = /*Error: Function owner not recognized*/vsnprintf(generatedBuf + generatedLen, 0, fmt, cp);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_end(cp);
		if (len < 0) {
		...
		if (((len) > (-1024 >> ((8 * /*Error: Unsupported expression*/) - (8 * /*Error: sizeof expression not supported yet*/))) - (generatedLen))) {
			ModernizedCProgram.die("you want to use way too much memory");
		} 
		sb.strbuf_grow(len);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(generatedBuf + pos + len, generatedBuf + pos, generatedLen - pos);
		save = generatedBuf[pos + /* vsnprintf() will append a NUL, overwriting one of our characters */len];
		len2 = /*Error: Function owner not recognized*/vsnprintf(generatedBuf + pos, len + 1, fmt, ap);
	...
	public void strbuf_add(Object data, Object len) {
		...
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedBuf + generatedLen, data, len);
		sb.strbuf_setlen(generatedLen + len);
	}
	public void strbuf_addbuf(Object sb2) {
		...
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedBuf + generatedLen, generatedBuf, generatedLen);
	...
	public void strbuf_addchars(int c, Object n) {
		...
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedBuf + generatedLen, c, n);
	...
	public void add_lines(Object prefix1, Object prefix2, Object[] buf, Object size) {
		while (size) {
			byte prefix;
			byte next = /*Error: Function owner not recognized*/memchr(buf, (byte)'\n', size);
	...
	public void strbuf_add_commented_lines(Object buf, Object size) {
		...
		if (prefix1[0] != ModernizedCProgram.comment_line_char) {
			ModernizedCProgram.xsnprintf(prefix1, /*Error: sizeof expression not supported yet*/, "%c ", ModernizedCProgram.comment_line_char);
			ModernizedCProgram.xsnprintf(prefix2, /*Error: sizeof expression not supported yet*/, "%c", ModernizedCProgram.comment_line_char);
		} 
	...
	public void strbuf_commented_addf(Object fmt) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_start(params, fmt);
	...
	public void strbuf_vaddf(Object fmt, Object ap) {
		...
		Object generatedLen = this.getLen();
		Object generatedAlloc = this.getAlloc();
		len = /*Error: Function owner not recognized*/vsnprintf(generatedBuf + generatedLen, generatedAlloc - generatedLen, fmt, cp);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_end(cp);
		if (len < 0) {
		...
		if (len > ModernizedCProgram.strbuf_avail(sb)) {
			sb.strbuf_grow(len);
			len = /*Error: Function owner not recognized*/vsnprintf(generatedBuf + generatedLen, generatedAlloc - generatedLen, fmt, ap);
			if (len > ModernizedCProgram.strbuf_avail(sb)) {
	...
	public void strbuf_expand(Object format, Object fn, Object context) {
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			...
			consumed = /*Error: Function owner not recognized*/fn(sb, format, context);
			if (consumed) {
				format += consumed;
			} else {
					sb.strbuf_addch((byte)'%');
	...
	public Object strbuf_expand_dict_cb(Object placeholder, Object context) {
		...
		for (; generatedPlaceholder && (len = /*Error: Function owner not recognized*/strlen(generatedPlaceholder)); e++) {
			if (!/*Error: Function owner not recognized*/strncmp(placeholder, generatedPlaceholder, len)) {
				if (generatedValue) {
	...
	public int strbuf_readlink(Object path, Object hint) {
		...
		while (hint < (2 * 260)) {
			...
			if (ModernizedCProgram.len < 0) {
				if ((/*Error: Function owner not recognized*/_errno()) != 34) {
					break;
				} 
	...
	public int strbuf_getcwd() {
		...
		byte[] generatedBuf = this.getBuf();
		for (; /*Error: Unsupported expression*/; guessed_len *= 2) {
			sb.strbuf_grow(guessed_len);
			if (/*Error: Function owner not recognized*/getcwd(generatedBuf, generatedAlloc)) {
				sb.strbuf_setlen(/*Error: Function owner not recognized*/strlen(generatedBuf));
				return 0/*
			...
			if ((/*Error: Function owner not recognized*/_errno()) == 13 && guessed_len < 260) {
				continue;
			} 
			if ((/*Error: Function owner not recognized*/_errno()) != 34) {
				break;
	...
	public void strbuf_addstr_xml_quoted(Object s) {
		while (s) {
			size_t len = /*Error: Function owner not recognized*/strcspn(s, "\"<>&");
	...
	public void strbuf_add_urlencode(Object s, Object len, Object allow_unencoded_fn) {
		...
		while (len--) {
			byte ch = s++;
			if (/*Error: Function owner not recognized*/allow_unencoded_fn(ch)) {
				sb.strbuf_addch(ch);
			} else {
	...
	public void strbuf_addstr_urlencode(Object s, Object allow_unencoded_fn) {
		sb.strbuf_add_urlencode(s, /*Error: Function owner not recognized*/strlen(s), allow_unencoded_fn);
	}
	...
	public void strbuf_add_absolute_path(Object path) {
		...
		if (!ModernizedCProgram.is_absolute_path(path)) {
			...
			byte pwd = /*Error: Function owner not recognized*/getenv("PWD");
			if (pwd && /*Error: Function owner not recognized*/strcmp(pwd, cwd) && !/*Error: Function owner not recognized*/stat(cwd, cwd_stat) && (generatedSt_dev || generatedSt_ino) && !/*Error: Function owner not recognized*/stat(pwd, pwd_stat) && generatedSt_dev == generatedSt_dev && generatedSt_ino == generatedSt_ino) {
	...
	public void strbuf_add_real_path(Object path) {
		Object generatedLen = this.getLen();
		if (generatedLen) {
			strbuf resolved = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public void strbuf_addftime(Object fmt, Object tm, int tz_offset, int suppress_tz_name) {
		strbuf munged_fmt = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		size_t hint = 128;
		size_t len = new size_t();
		if (!fmt) {
			return /*Error: Unsupported expression*/;
		...
		Object generatedLen = this.getLen();
		Object generatedAlloc = this.getAlloc();
		len = /*Error: Function owner not recognized*/strftime(generatedBuf + generatedLen, generatedAlloc - generatedLen, fmt, tm);
		if (!len/*
			...
			while (!len) {
				hint *= 2;
				sb.strbuf_grow(hint);
				len = /*Error: Function owner not recognized*/strftime(generatedBuf + generatedLen, generatedAlloc - generatedLen, generatedBuf, tm);
			}
	...
	public void strbuf_stripspace(int skip_comments) {
		...
		for (i = j = 0; i < generatedLen; ) {
			eol = /*Error: Function owner not recognized*/memchr(generatedBuf + i, (byte)'\n', generatedLen - i);
			len = eol ? eol - (generatedBuf + i) + 1 : generatedLen - i;
			if (skip_comments && len && generatedBuf[i] == ModernizedCProgram.comment_line_char) {
				newlen = 0;
				continue;
			} 
			newlen = generatedBuf + i.cleanup(len);
			if (/* Not just an empty line? */newlen) {
				if (empties > 0 && j > 0) {
					generatedBuf[j++] = (byte)'\n';
				} 
				empties = 0;
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove(generatedBuf + j, generatedBuf + i, newlen);
	...
	public int strbuf_normalize_path() {
		...
		dst.strbuf_setlen(/*Error: Function owner not recognized*/strlen(generatedBuf));
		src.strbuf_swap(dst);
		dst.strbuf_release();
		return 0;
	}
	...
	public int update_some(Object oid, Object pathname, int mode, int stage, Object context) {
		...
		Object generatedLen = this.getLen();
		len = generatedLen + /*Error: Function owner not recognized*/strlen(pathname);
		...
		byte[] generatedBuf = this.getBuf();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedName, generatedBuf, generatedLen);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedName + generatedLen, pathname, len - generatedLen);
	...
	public int parse_ls_response(Object[] response, Object mode) {
		byte tab;
		byte response_end;
		((response) ? (Object)0 : /*Error: Function owner not recognized*/_assert("response", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\fast_export.c", 266));
		response_end = response + /*Error: Function owner not recognized*/strlen(response);
		if (response == /* Missing. */(byte)'m') {
			(/*Error: Function owner not recognized*/_errno()) = 2;
			return -1;
		} 
		if (response_end - response < (int)/*Error: Function owner not recognized*/strlen(/* Mode. */"100644") || response[/*Error: Function owner not recognized*/strlen("100644")] != (byte)' ') {
		...
		if (response_end - response < (int)/*Error: Function owner not recognized*/strlen(/* ' blob ' or ' tree ' */" blob ") || (response[1] != (byte)'b' && response[1] != (byte)'t')) {
			ModernizedCProgram.die("unexpected ls response: not a tree or blob: %s", response);
		} 
		response += /*Error: Function owner not recognized*/strlen(" blob ");
		tab = /*Error: Function owner not recognized*/memchr(response, (byte)'\t', response_end - /* Dataref. */response);
		if (!tab) {
	...
	public int has_conforming_footer(strbuf sob, Object ignore_footer) {
		...
		for (i = 0; i < generatedTrailer_nr; i++) {
			if (sob && !/*Error: Function owner not recognized*/strncmp(generatedTrailers[i], generatedBuf, generatedLen)) {
				found_sob = 1;
	...
	public int update_head_with_reflog(Object old_head, Object new_head, Object action, Object msg) {
		...
		nl = /*Error: Function owner not recognized*/strchr(msg.getBuf(), (byte)'\n');
		if (nl) {
	...
	public void append_signoff(Object ignore_footer, int flag) {
		...
		strbuf sob = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		int has_footer;
		...
		Object generatedLen = this.getLen();
		byte[] generatedBuf = this.getBuf();
		if (generatedLen - ignore_footer == generatedLen && !/*Error: Function owner not recognized*/strncmp(generatedBuf, generatedBuf, generatedLen)) {
			has_footer = 3;
		} else {
		...
		if (!has_footer) {
			...
			if (append_newlines) {
				msgbuf.strbuf_splice(generatedLen - ignore_footer, 0, append_newlines, /*Error: Function owner not recognized*/strlen(append_newlines));
			} 
		} 
	...
	public strbuf diff_output_prefix_callback(diff_options opt, Object data) {
		...
		((opt) ? (Object)0 : /*Error: Function owner not recognized*/_assert("opt", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\graph.c", 236));
	...
	public int read_one(Object path) {
		...
		cp = buf;
		it.setEntry_count(/*Error: Function owner not recognized*/strtol(cp, ep, 10));
		if (cp == ep) {
			;
		} 
		cp = ep;
		subtree_nr = /*Error: Function owner not recognized*/strtol(cp, ep, 10);
		if (cp == ep) {
			;
		} 
		...
		it.setDown(ModernizedCProgram.xcalloc(generatedSubtree_alloc, /*Error: Unsupported expression*/));
	...
	public void redact_sensitive_header() {
		...
		if (ModernizedCProgram.skip_prefix(generatedBuf, "Authorization:", sensitive_header) || ModernizedCProgram.skip_prefix(generatedBuf, "Proxy-Authorization:", sensitive_header)) {
			...
			strbuf redacted_header = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
			byte cookie;
			...
			while (cookie) {
				byte equals;
				byte semicolon = /*Error: Function owner not recognized*/strstr(cookie, "; ");
				if (semicolon) {
					semicolon = 0;
				} 
				...
				if (semicolon/*
					...
					cookie = semicolon + /*Error: Function owner not recognized*/strlen("; ");
				} else {
						cookie = ((Object)0);
				} 
			}
	...
	public Object curlinfo_strbuf(Object curl, Object info) {
		byte ptr;
		 ret = new ();
		ModernizedCProgram.buf.strbuf_setlen(0);
		ret = /*Error: Function owner not recognized*/curl_easy_getinfo(curl, info, ptr);
		if (!ret && ptr) {
	...
	public int extract_param(Object raw, Object name) {
		...
		if (/*Error: Function owner not recognized*/strncasecmp(raw, name, len)) {
			return -1;
		} 
		raw += len;
		if (raw != (byte)'=') {
	...
	public void write_accept_language() {
		...
		int i;
		strbuf tag = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		do {
			...
			if (generatedLen) {
				num_langs++;
				(language_tags) = ModernizedCProgram.xrealloc((language_tags), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (num_langs)));
		...
		if (/* write Accept-Language header into buf */num_langs) {
			...
			(language_tags) = ModernizedCProgram.xrealloc((language_tags), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (num_langs + /* add '*' */1)));
			language_tags[num_langs++] = /* it's OK; this won't be freed */"*";
			for (; max_q < num_langs && decimal_places <= MAX_DECIMAL_PLACES; ) {
				;
			}
			ModernizedCProgram.xsnprintf(q_format, /*Error: sizeof expression not supported yet*/, ";q=0.%%0%dd", decimal_places);
	...
	public int update_url_from_redirect(Object asked, Object got) {
		...
		if (!/*Error: Function owner not recognized*/strcmp(asked, got.getBuf())) {
			return 0;
		} 
		byte[] generatedBuf = this.getBuf();
	...
	public void create_reflog_msg(Object oid) {
		...
		case object_type.OBJ_COMMIT:
				if ((buf = ModernizedCProgram.the_repository.repo_read_object_file(oid, object_type.type, size)) != ((Object)0)) {
					subject_len = /*Error: Function owner not recognized*/find_commit_subject(buf, subject_start);
					sb.strbuf_insert(generatedLen, subject_start, subject_len);
				} else {
						sb.strbuf_addstr("commit object");
				} 
				ModernizedCProgram.free(buf);
				if ((c = /*Error: Function owner not recognized*/lookup_commit_reference(ModernizedCProgram.the_repository, oid)) != ((Object)0)) {
	...
	public int add_mailname_host() {
		...
		strbuf mailnamebuf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (ModernizedCProgram.strbuf_getline(mailnamebuf, mailname) == (true)) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fclose(mailname);
			return -1;
		} 
	...
	public int canonical_name(Object host) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(hints, (byte)'\0', /*Error: sizeof expression not supported yet*/);
		hints.setAddrinfo(AI_CANONNAME);
		Object generatedAddrinfo = ai.getAddrinfo();
		if (!/*Error: Function owner not recognized*/getaddrinfo(host, ((Object)0), hints, ai)) {
			if (ai && generatedAddrinfo && /*Error: Function owner not recognized*/strchr(generatedAddrinfo, (byte)'.')) {
	...
	public void add_domainname(Integer is_bogus) {
		byte[] buf = new byte[256 + 1];
		if (ModernizedCProgram.xgethostname(buf, /*Error: sizeof expression not supported yet*/)) {
			ModernizedCProgram.warning_errno("cannot get host name");
			out.strbuf_addstr("(none)");
			is_bogus = 1;
			return /*Error: Unsupported expression*/;
		} 
		if (/*Error: Function owner not recognized*/strchr(buf, (byte)'.')) {
	...
	public void strbuf_addstr_without_crud(Object[] src) {
		...
		len = /*Error: Function owner not recognized*/strlen(/* Remove crud from the end.. */src);
		while (len > 0) {
			c = src[len - 1];
			if (!ModernizedCProgram.crud(c)) {
				break;
	...
	public void maybe_colorize_sideband(Object[] src, int n) {
		...
		for (i = 0; i < (/*Error: sizeof expression not supported yet*/ / /*Error: sizeof expression not supported yet*/ + (/*Error: Unsupported expression*/ - 1)); i++) {
			...
			int len = /*Error: Function owner not recognized*/strlen(generatedKeyword);
			if (n < ModernizedCProgram.len) {
				continue;
			} 
			if (!/*Error: Function owner not recognized*/strncasecmp(generatedKeyword, src, ModernizedCProgram.len) && (ModernizedCProgram.len == n || !((ModernizedCProgram.sane_ctype[(byte)(src[ModernizedCProgram.len])] & (-1024 | -1024)) != 0))) {
	...
	public int demultiplex_sideband(Object me, byte[] buf, int len, int die_on_error, sideband_type sideband_type) {
		...
		int band;
		if (!suffix) {
			if (/*Error: Function owner not recognized*/isatty(2) && !ModernizedCProgram.is_terminal_dumb()) {
				suffix = "\033[K";
			} else {
					suffix = "        ";
			} 
		...
		case 2:
				b = buf + 1/*
						 ...
						 */;
				while ((brk = /*Error: Function owner not recognized*/strpbrk(b, "\n\r"))) {
					int linelen = brk - b;
					if (!generatedLen) {
				...
				if (b) {
					...
					scratch.maybe_colorize_sideband(b, /*Error: Function owner not recognized*/strlen(b));
				} 
				return 0;
		default:
	...
	public void show_name(Object decoration) {
		if (ModernizedCProgram.decoration_flags == 1) {
			sb.strbuf_addstr(ModernizedCProgram.prettify_refname(decoration.getName()));
		} else {
				sb.strbuf_addstr(decoration.getName());
		} 
		byte name;
		object_id oid = obj.getOid();
		strbuf buf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		if (!name_only) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%s ", caller_name ? caller_name : ModernizedCProgram.oid_to_hex(oid));
		} 
		name = buf.get_rev_name(obj);
		if (name) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%s\n", name);
		}  else if (allow_undefined) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("undefined\n");
		}  else if (ModernizedCProgram.always) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf("%s\n", ModernizedCProgram.the_repository.repo_find_unique_abbrev(oid, ModernizedCProgram.default_abbrev));
		} else {
				ModernizedCProgram.die("cannot describe '%s'", ModernizedCProgram.oid_to_hex(oid));
		} 
		buf.strbuf_release();
	}
	public void format_decorations_extended(Object commit, int use_color, Object prefix, Object separator, Object suffix) {
		...
		byte color_commit = /*Error: Function owner not recognized*/diff_get_color(use_color, color_diff.DIFF_COMMIT);
	...
	public Object get_rev_name(Object o) {
		...
		if (!generatedGeneration) {
			return generatedTip_name;
				...
				int len = /*Error: Function owner not recognized*/strlen(generatedTip_name);
				if (ModernizedCProgram.len > 2 && !/*Error: Function owner not recognized*/strcmp(generatedTip_name + ModernizedCProgram.len - 2, "^0")) {
	...
	public void strbuf_swap(strbuf b) {
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/SWAP(a, b/**
	...
	public void strbuf_setlen(Object len) {
		...
		if (generatedBuf != ModernizedCProgram.strbuf_slopbuf) {
			generatedBuf[len] = (byte)'\0';
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/assert(!ModernizedCProgram.strbuf_slopbuf[0/**
	...
	public void strbuf_addstr(Object s) {
		sb.strbuf_add(s, /*Error: Function owner not recognized*/strlen(s/**
	...
	public strbuf strbuf_split_str(Object str, int terminator, int max) {
		strbuf strbuf = new strbuf();
		return strbuf.strbuf_split_buf(str, /*Error: Function owner not recognized*/strlen(str), terminator, max);
	}
	...
	public int append_normalized_escapes(Object from, Object from_len, Object esc_extra, Object esc_ok) {
		while (from_len) {
			...
			if ((byte)ch <= -1024 || (byte)ch >= -1024 || /*Error: Function owner not recognized*/strchr(" <>\"%{}|\\^`", ch) || (esc_extra && /*Error: Function owner not recognized*/strchr(esc_extra, ch)) || (was_esc && /*Error: Function owner not recognized*/strchr(esc_ok, ch))) {
	...
	public int write_shallow_commits_1(int use_pack_protocol, Object extra, int flags) {
		...
		data.setCount(0);
		data.setFlags(flags);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/for_each_commit_graft(write_one_shallow, data);
	...
	public int get_common_dir(Object gitdir) {
		byte git_env_common_dir = /*Error: Function owner not recognized*/getenv("GIT_COMMON_DIR");
	...
	public int get_common_dir_noenv(Object gitdir) {
		strbuf data = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
	...
	public int is_nonbare_repository_dir() {
		...
		((orig_path_len != 0) ? (Object)0 : /*Error: Function owner not recognized*/_assert("orig_path_len != 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\setup.c", 356));
		ModernizedCProgram.path.strbuf_complete((byte)'/');
	...
	public discovery_result setup_git_directory_gently_1(strbuf gitdir, int die_on_error) {
		byte env_ceiling_dirs = /*Error: Function owner not recognized*/getenv("GIT_CEILING_DIRECTORIES");
		...
		int one_filesystem = 1/*
			 ...
			 * validation.
			 */;
		gitdirenv = /*Error: Function owner not recognized*/getenv("GIT_DIR");
		if (gitdirenv) {
		...
		one_filesystem = !/*Error: Function owner not recognized*/git_env_bool("GIT_DISCOVERY_ACROSS_FILESYSTEM", 0);
		if (one_filesystem) {
	...
	public int discover_git_directory(strbuf gitdir) {
		...
		repository_format candidate = new repository_format(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (generatedLen < cwd_len && !ModernizedCProgram.is_absolute_path(generatedBuf + gitdir_offset)) {
			if (!/*Error: Function owner not recognized*/strcmp(".", generatedBuf + /* Avoid a trailing "/." */gitdir_offset)) {
	...
	public Byte url_decode_internal(Object query, int len, Object stop_at, int decode_plus) {
		...
		while (len) {
			...
			if (stop_at && /*Error: Function owner not recognized*/strchr(stop_at, c)) {
				q++;
				len--;
				break;
			} 
	...
	public int show_http_message(strbuf charset, strbuf msg) {
		...
		/*
			 ...
			 */
		if (/*Error: Function owner not recognized*/strcmp(ModernizedCProgram.type.getBuf(), "text/plain")) {
			return -1;
		} 
		...
		do {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "remote: %.*s\n", (int)(eol - p), p);
			p = eol + 1;
		} while (eol);
		return 0;
	}
	...
	public void parse_fetch() {
			...
			if (ModernizedCProgram.skip_prefix(ModernizedCProgram.buf.getBuf(), "fetch ", p)) {
				...
				do {
					if ((nr_heads + 1) > alloc_heads) {
						...
						(to_fetch) = ModernizedCProgram.xrealloc((to_fetch), ModernizedCProgram.st_mult(/*Error: sizeof expression not supported yet*/, (alloc_heads)));
					} 
				} while (0);
		...
		if (ModernizedCProgram.fetch(nr_heads, to_fetch)) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\remote-curl.c", 1157, (/* error already reported */true)));
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/free_refs(list_head);
	...
	public void parse_push() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fflush((_iob[1]));
		if (ret) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\remote-curl.c", 1275, (/* error already reported */true)));
		} 
	}
	...
	public int read_branch_desc(Object branch_name) {
		...
		byte[] generatedBuf = name.getBuf();
		if (/*Error: Function owner not recognized*/git_config_get_string(generatedBuf, v)) {
			name.strbuf_release();
			return -1;
		} 
	...
	public int validate_new_branchname(Object name, int force) {
		...
		byte[] generatedBuf = this.getBuf();
		if (!force) {
			ModernizedCProgram.die(ModernizedCProgram._("A branch named '%s' already exists."), generatedBuf + /*Error: Function owner not recognized*/strlen("refs/heads/"));
		} 
		head = ((Object)0).resolve_ref_unsafe("HEAD", 0, ((Object)0));
		if (!ModernizedCProgram.is_bare_repository() && head && !/*Error: Function owner not recognized*/strcmp(head, generatedBuf)) {
	...
	public void tr2_dst_write_line(Object dst) {
		...
		byte[] generatedBuf = this.getBuf();
		Object generatedLen = this.getLen();
		if (/*Error: Function owner not recognized*/write(fd, generatedBuf, generatedLen) >= 0) {
			return /*Error: Unsupported expression*/;
		} 
		if (ModernizedCProgram.tr2_dst_want_warning()) {
			ModernizedCProgram.warning("unable to write trace to '%s': %s", ModernizedCProgram.tr2_sysenv_display_name(dst.getSysenv_var()), /*Error: Function owner not recognized*/strerror((/*Error: Function owner not recognized*/_errno())));
		} 
	...
	public void merge_name(Object remote) {
		...
		strbuf buf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf bname = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		remote = generatedBuf;
		branch_head.oidclr();
		remote_head = /*Error: Function owner not recognized*/get_merge_parent(remote);
		if (!remote_head) {
			ModernizedCProgram.die(ModernizedCProgram._("'%s' does not point to a commit"), remote);
		} 
		if (branch_head.dwim_ref(remote, /*Error: Function owner not recognized*/strlen(remote), found_ref) > 0) {
		...
		if (len) {
			early = 1;
				...
				early = 0;
				ptr = /*Error: Function owner not recognized*/strrchr(remote, (byte)'~');
				if (ptr) {
					int seen_nonzero = 0;
					/* count ~ */len++;
		...
		object_id generatedOid = generatedObject.getOid();
		if (len) {
			strbuf truname = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		desc = /*Error: Function owner not recognized*/merge_remote_util(remote_head);
	...
	public void strbuf_utf8_replace(int pos, int width, Object subst) {
		strbuf sb_dst = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		byte[] generatedBuf = this.getBuf();
		...
		int subst_len = 0;
		if (subst) {
			subst_len = /*Error: Function owner not recognized*/strlen(subst);
		} 
		...
		while (src < end) {
			...
			while ((n = ModernizedCProgram.display_mode_esc_sequence_len(src))) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(dst, src, n);
				src += n;
				dst += n;
			}
			if (src >= end) {
			...
			if (n && w >= pos && w < pos + width) {
				if (subst) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(dst, subst, subst_len);
					dst += subst_len;
					subst = ((Object)0);
				} 
				w += n;
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(dst, old, src - old);
			dst += src - old;
			w += n;
		}
	...
	public void strbuf_utf8_align( position, int width, Object s) {
		int slen = /*Error: Function owner not recognized*/strlen(s);
	...
	public void handle_property(Object key_buf, Object type_set) {
		...
		switch (keylen + 1) {
		case /*Error: sizeof expression not supported yet*/:
				if (/*Error: Function owner not recognized*/memcmp(key, "svn:log", /*Error: sizeof expression not supported yet*/ - 1)) {
					break;
				} 
				if (!val) {
		...
		case /*Error: sizeof expression not supported yet*/:
		case /*Error: sizeof expression not supported yet*/:
				if (/*Error: Function owner not recognized*/memcmp(key, "svn:date", /*Error: sizeof expression not supported yet*/ - 1)) {
					break;
				} 
				if (!val) {
				...
				break;
		case /*Error: sizeof expression not supported yet*/:
				if (/*Error: Function owner not recognized*/memcmp(key, "svn:author", /*Error: sizeof expression not supported yet*/ - 1)) {
					break;
				} 
				if (!val) {
				...
				break;
		case /*Error: sizeof expression not supported yet*/:
				if (keylen == /*Error: Function owner not recognized*/strlen("svn:executable") && /*Error: Function owner not recognized*/memcmp(key, "svn:executable", /*Error: sizeof expression not supported yet*/ - 1)) {
					break;
				} 
				if (keylen == /*Error: Function owner not recognized*/strlen("svn:special") && /*Error: Function owner not recognized*/memcmp(key, "svn:special", /*Error: sizeof expression not supported yet*/ - 1)) {
					break;
				} 
				if (type_set) {
					if (!val) {
				...
				type_set = 1;
				node_ctx.setType(keylen == /*Error: Function owner not recognized*/strlen("svn:executable") ? (-1024 | 755) : S_IFLNK);
		}
	}
	...
	public void fmt_merge_msg_title(Object current_branch) {
		...
		for (i = 0; i < ModernizedCProgram.srcs.getNr(); i++) {
			...
			if (/*Error: Function owner not recognized*/strcmp(".", ModernizedCProgram.srcs.getItems()[i].getString())) {
				out.strbuf_addf(" of %s", ModernizedCProgram.srcs.getItems()[i].getString());
			} 
		}
		if (!/*Error: Function owner not recognized*/strcmp("master", current_branch)) {
			out.strbuf_addch((byte)'\n');
		} else {
	...
	public void fmt_tag_signature(strbuf sig, Object buf, long len) {
		byte tag_body = /*Error: Function owner not recognized*/strstr(buf, "\n\n");
		if (tag_body) {
			tag_body += 2;
	...
	public void fmt_merge_msg_sigs() {
		...
		int tag_number = 0;
		int first_tag = 0;
		strbuf tagbuf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		Object generatedLen = sig.getLen();
	...
	public int subprocess_read_status(int fd) {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			...
			if (pair[0] && pair[0].getLen() && pair[1]) {
				if (!/*Error: Function owner not recognized*/strcmp(pair[0].getBuf(), /* the last "status=<foo>" line wins */"status=")) {
	...
	public int remove_dirs(Object prefix, int force_flag, int dry_run, int quiet, Integer dir_gone) {
		DIR dir = new DIR();
		...
		if ((force_flag & 2) && ModernizedCProgram.path.is_nonbare_repository_dir()) {
			if (!quiet) {
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf(dry_run ? ModernizedCProgram._(ModernizedCProgram.msg_would_skip_git_dir) : ModernizedCProgram._(ModernizedCProgram.msg_skip_git_dir), generatedBuf);
			} 
			dir_gone = 0;
			;
		} 
		...
		if (!dir) {
			res = dry_run ? 0 : /*Error: Function owner not recognized*/rmdir(generatedBuf);
			if (res) {
		...
		while ((e = dirent.readdir(dir)) != ((Object)0)) {
			...
			ModernizedCProgram.path.strbuf_setlen(len);
			ModernizedCProgram.path.strbuf_addstr(generatedD_name);
			if (/*Error: Function owner not recognized*/lstat(generatedBuf, st)) {
				...
				if (gone) {
					quoted.quote_path_relative(generatedBuf, prefix);
					string_list_item.string_list_append(dels, generatedBuf);
				} else {
						dir_gone = 0;
				} 
				continue;
			} else {
					res = dry_run ? 0 : /*Error: Function owner not recognized*/unlink(generatedBuf);
					if (!res) {
						quoted.quote_path_relative(generatedBuf, prefix);
						string_list_item.string_list_append(dels, generatedBuf);
					} else {
		...
		if (!dir_gone && !quiet) {
			int i;
			for (i = 0; i < generatedNr; i++) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf(dry_run ? ModernizedCProgram._(ModernizedCProgram.msg_would_remove) : ModernizedCProgram._(ModernizedCProgram.msg_remove), generatedItems[i].getString());
			}
		} 
	...
	public int launch_specified_editor(Object editor, Object path, Object env) {
		...
		if (/*Error: Function owner not recognized*/strcmp(editor, ":")) {
			...
			int ret;
			int sig;
			int print_waiting_for_editor = ModernizedCProgram.advice_waiting_for_editor && /*Error: Function owner not recognized*/isatty(2);
			if (print_waiting_for_editor/*
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("hint: Waiting for your editor to close the file...%c"), ModernizedCProgram.term);
			...
			if (sig == 2 || sig == SIGQUIT) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/raise(sig);
			} 
			if (ret) {
				return ();
			} 
	...
	public int replace_parents(int argc, Object[][] argv) {
		strbuf new_parents = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		byte parent_start;
		byte parent_end;
		int i;
		...
		for (i = 0; i < argc; /* prepare new parents */i++) {
			...
			commit = /*Error: Function owner not recognized*/lookup_commit_reference(ModernizedCProgram.the_repository, oid);
			if (!commit) {
				new_parents.strbuf_release();
				return ();
			} 
	...
	public void expand_atom(Object atom, int len, Object vdata) {
		...
		Object generatedMark_query = data.getMark_query();
	...
	public Object expand_format(Object start, Object data) {
		...
		end = /*Error: Function owner not recognized*/strchr(start + 1, (byte)')');
		if (!end) {
	...
	public Byte strbuf_realpath(Object path, int die_on_error) {
		strbuf remaining = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf next = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		strbuf symlink = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		while (generatedLen > /* Iterate over the remaining path components */0) {
			...
			if (generatedLen == 0) {
				continue;
			}  else if (generatedLen == 1 && !/*Error: Function owner not recognized*/strcmp(generatedBuf, ".")) {
				continue;
			}  else if (generatedLen == 2 && !/*Error: Function owner not recognized*/strcmp(generatedBuf, "..")) {
			...
			if (/*Error: Function owner not recognized*/lstat(generatedBuf, st)) {
				if ((/*Error: Function owner not recognized*/_errno()) != 2 || generatedLen) {
					if (die_on_error) {
				...
				if (num_symlinks++ > 32) {
					(/*Error: Function owner not recognized*/_errno()) = ELOOP;
					if (die_on_error) {
