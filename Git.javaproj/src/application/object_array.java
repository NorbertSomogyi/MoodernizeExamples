...
public class object_array {
	...
	public int can_all_from_reach_with_flag(int with_flag, int assign_flag, Object min_commit_date, Object min_generation) {
		...
		int nr_commits;
		int result = 1;
		(list) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (ModernizedCProgram.from.getNr())));
		nr_commits = 0;
		...
		for (i = 0; i < ModernizedCProgram.from.getNr(); i++) {
			...
			from_one = .deref_tag(ModernizedCProgram.the_repository, from_one, "a from object", 0);
		...
		ModernizedCProgram.sane_qsort((list), (nr_commits), , compare_commits_by_gen);
			...
			while (stack) {
				...
				for (parent = generatedParents; parent; parent = generatedNext) {
					...
					if (!(generatedFlags & assign_flag)) {
						...
						.commit_list_insert(generatedItem, stack);
						break;
					} 
				}
				if (!parent) {
