...
public class commit {
	...
	public commit fake_working_tree_commit(repository r, diff_options opt, Object path, Object contents_from) {
		...
		msg.strbuf_addf("author %s\ncommitter %s\n\nVersion of %s from %s\n", ident, ident, path, (!contents_from ? path : (!.strcmp(contents_from, "-") ? "standard input" : contents_from)));
		...
		if (!contents_from || .strcmp("-", contents_from)) {
			...
			case -1024:
					if (generatedAllow_textconv && .textconv_object(r, read_from, mode, ModernizedCProgram.null_oid, 0, buf_ptr, buf_len)) {
	...
	public commit find_single_final(rev_info revs, Object name_p) {
		...
		for (i = 0; i < generatedNr; i++) {
			...
			obj = .deref_tag(generatedRepo, obj, ((Object)0), 0);
	...
	public commit dwim_reverse_initial(rev_info revs, Object name_p) {
		...
		head_commit = .lookup_commit_reference_gently(generatedRepo, head_oid, 1);
		if (!head_commit) {
			return ((Object)0);
		} 
		int generatedFlags = obj.getFlags();
		...
		case object_type.OBJ_TAG:
				{ 
					...
					while (tag && generatedObject.getType() == /* handle nested tags */object_type.OBJ_TAG) {
						...
						ModernizedCProgram.tag_refs.string_list_append(full_name).setUtil(tag);
	...
	public commit get_commit_reference(repository r, Object oid) {
		commit c = .lookup_commit_reference(r, oid);
		if (!c) {
		...
		r = .lookup_commit_reference(ModernizedCProgram.the_repository, revkey);
		if (!r) {
			ModernizedCProgram.die("Not a valid commit name %s", arg);
		} 
		return r;
	}
	public commit get_bad_and_good_commits(repository r, int rev_nr) {
		commit rev = new commit();
		int i;
		int n = 0;
		(rev) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (1 + ModernizedCProgram.good_revs.getNr())));
		commit commit = new commit();
		...
		for (; result; result = generatedNext) {
			...
			if (ModernizedCProgram.oideq(mb, ModernizedCProgram.current_bad_oid)) {
				...
				ModernizedCProgram.handle_skipped_merge_base(mb);
					...
					.printf(ModernizedCProgram._("Bisecting: a merge base must be tested\n"));
					.exit(ModernizedCProgram.trace2_cmd_exit_fl("E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\bisect.c", 823, (ModernizedCProgram.bisect_checkout(mb, no_checkout))));
			} 
		}
	...
	public void show_commit(Object data) {
		...
		if (generatedShow_timestamp) {
			.printf("%llu ", generatedDate);
		} 
		Object generatedHeader_prefix = info.getHeader_prefix();
		if (generatedHeader_prefix) {
			.fputs(generatedHeader_prefix, (_iob[1]));
		} 
		if (!generatedGraph) {
			.fputs(ModernizedCProgram.get_revision_mark(revs, commit), (_iob[1]));
		} 
		...
		if (generatedAbbrev_commit && generatedAbbrev) {
			.fputs(ModernizedCProgram.the_repository.repo_find_unique_abbrev(generatedOid, generatedAbbrev), (_iob[1]));
		} else {
				.fputs(ModernizedCProgram.oid_to_hex(generatedOid), (_iob[1]));
		} 
		...
		if (generatedPrint_parents) {
			...
			while (parents) {
				.printf(" %s", ModernizedCProgram.oid_to_hex(generatedOid));
		...
		if (generatedVerbose_header) {
			...
			if (ModernizedCProgram.buf.getLen()) {
				...
				.putchar(generatedHdr_termination);
			} else {
	...
	public void push_bitmapped_commit(Object reused) {
		if (ModernizedCProgram.writer.getSelected_nr() >= ModernizedCProgram.writer.getSelected_alloc()) {
			...
			(ModernizedCProgram.writer.getSelected()) = ModernizedCProgram.xrealloc((ModernizedCProgram.writer.getSelected()), ModernizedCProgram.st_mult(, (ModernizedCProgram.writer.getSelected_alloc())));
		} 
	...
	public int add_to_include_set(Object base) {
		khiter_t hash_pos = new khiter_t();
		object generatedObject = this.getObject();
		object_id generatedOid = generatedObject.getOid();
		uint32_t bitmap_pos = ModernizedCProgram.find_object_pos(generatedOid);
		if (base.bitmap_get(bitmap_pos)) {
			return 0;
		} 
		hash_pos = generatedOid.kh_get_oid_map(ModernizedCProgram.writer.getBitmaps());
		ewah_bitmap generatedBitmap = bc.getBitmap();
		if (hash_pos < ((ModernizedCProgram.writer.getBitmaps()).getN_buckets())) {
			bitmapped_commit bc = ((ModernizedCProgram.writer.getBitmaps()).getVals()[hash_pos]);
			ModernizedCProgram.bitmap_or_ewah(base, generatedBitmap);
			return 0;
		} 
		base.bitmap_set(bitmap_pos);
		return 1;
		khiter_t hash_pos = new khiter_t();
		bitmap generatedSeen = data.getSeen();
		if (generatedSeen && generatedSeen.bitmap_get(bitmap_pos)) {
			return 0;
		} 
		bitmap generatedBase = data.getBase();
		if (generatedBase.bitmap_get(bitmap_pos)) {
			return 0;
		} 
		hash_pos = oid.kh_get_oid_map(ModernizedCProgram.bitmap_git.getBitmaps());
		ewah_bitmap ewah_bitmap = new ewah_bitmap();
		if (hash_pos < ((ModernizedCProgram.bitmap_git.getBitmaps()).getN_buckets())) {
			stored_bitmap st = ((ModernizedCProgram.bitmap_git.getBitmaps()).getVals()[hash_pos]);
			ModernizedCProgram.bitmap_or_ewah(generatedBase, ewah_bitmap.lookup_stored_bitmap(st));
			return 0;
		} 
	...
	public int should_include(Object _data) {
		...
		int generatedFlags = generatedObject.getFlags();
		commit_list generatedNext = parent.getNext();
		if (!commit.add_to_include_set(base)) {
		...
		int generatedFlags = generatedObject.getFlags();
		commit_list generatedNext = parent.getNext();
		if (!data.add_to_include_set(generatedBitmap_git, generatedOid, bitmap_pos)) {
			commit_list parent = generatedParents;
			while (parent) {
				generatedFlags |=  (-1024 << 0);
				parent = generatedNext;
	...
	public void bitmap_writer_select_commits(int indexed_commits_nr, int max_bitmaps) {
		...
		int j;
		int next;
		ModernizedCProgram.sane_qsort((ModernizedCProgram.indexed_commits), (indexed_commits_nr), , date_compare);
	...
	public int merging_a_throwaway_tag() {
		...
		if (!.merge_remote_util(/* Are we merging a tag? */commit) || !.merge_remote_util(commit).getObj() || .merge_remote_util(commit).getObj().getType() != object_type.OBJ_TAG) {
		...
		tag_ref = ModernizedCProgram.xstrfmt("refs/tags/%s", ((tag).merge_remote_util(commit).getObj()).getTag());
		if (!oid.read_ref(tag_ref) && ModernizedCProgram.oideq(oid, .merge_remote_util(commit).getObj().getOid())) {
			is_throwaway_tag = 0;
		} else {
				is_throwaway_tag = 1;
		} 
	...
	public commit interesting(commit_list list) {
		...
		return ((Object)0);
		long generatedFlag = sline.getFlag();
		lline generatedLost = sline.getLost();
	...
	public void name_commit(Object head_name, int nth) {
		...
		if (!name) {
			name = ModernizedCProgram.xmalloc();
	...
	public void show_one_commit(int no_name) {
		...
		if (!no_name) {
			if (name && generatedHead_name) {
				.printf("[%s", generatedHead_name);
				if (generatedGeneration) {
					if (generatedGeneration == 1) {
						.printf("^");
					} else {
							.printf("~%d", generatedGeneration);
					} 
				} 
				.printf("] ");
			} else {
					.printf("[%s] ", ModernizedCProgram.the_repository.repo_find_unique_abbrev(generatedOid, ModernizedCProgram.default_abbrev));
			} 
		} 
		.puts(pretty_str);
		pretty.strbuf_release();
	}
	public int show_independent(int num_rev, Integer rev_mask) {
		int i;
		object generatedObject = commit.getObject();
		int generatedFlags = generatedObject.getFlags();
		object_id generatedOid = generatedObject.getOid();
	...
	public int is_original_commit_empty() {
		...
		return ModernizedCProgram.oideq(ptree_oid, .get_commit_tree_oid(commit/*
	...
	public commit lookup_label(Object label, int len, strbuf buf) {
		...
		byte generatedBuf = buf.getBuf();
		commit = .lookup_commit_reference_by_name(generatedBuf);
		if (!commit) {
			buf.strbuf_splice(0, .strlen("refs/rewritten/"), "", /* fall back to non-rewritten ref or commit */0);
			commit = .lookup_commit_reference_by_name(generatedBuf);
		} 
		if (!commit) {
			();
	...
	public void index_commit_for_bitmap() {
		if (ModernizedCProgram.indexed_commits_nr >= ModernizedCProgram.indexed_commits_alloc) {
			...
			(ModernizedCProgram.indexed_commits) = ModernizedCProgram.xrealloc((ModernizedCProgram.indexed_commits), ModernizedCProgram.st_mult(, (ModernizedCProgram.indexed_commits_alloc)));
		} 
	...
	public commit get_base_commit(Object base_commit, int total) {
		...
		}  else if ((base_commit && !.strcmp(base_commit, "auto")) || ModernizedCProgram.base_auto) {
			branch curr_branch = .branch_get(((Object)0));
			byte upstream = .branch_get_upstream(curr_branch, ((Object)0));
			if (upstream) {
		...
		(rev) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (total)));
		for (i = 0; i < total; i++) {
	...
	public void mark_commit(Object data) {
		object generatedObject = this.getObject();
		generatedObject.mark_object(((Object)0), data);
	}
	...
	public int commit_is_complete() {
		...
		if (!is_incomplete/*
			...
			for (i = 0; i < generatedNr; i++) {
				...
				if (!ModernizedCProgram.tree_is_complete(.get_commit_tree_oid(c))) {
					is_incomplete = 1;
					generatedFlags |=  (-1024 << 10);
				} 
			}
	...
	public void print_new_head_line() {
		...
		object generatedObject = this.getObject();
		object_id generatedOid = generatedObject.getOid();
		.printf(ModernizedCProgram._("HEAD is now at %s"), ModernizedCProgram.the_repository.repo_find_unique_abbrev(generatedOid, ModernizedCProgram.default_abbrev));
		buf.pp_commit_easy(cmit_fmt.CMIT_FMT_ONELINE, commit);
		Object generatedLen = buf.getLen();
		byte generatedBuf = buf.getBuf();
		if (generatedLen > 0) {
			.printf(" %s", generatedBuf);
		} 
		.putchar((byte)'\n');
		buf.strbuf_release();
	}
	...
	public int mark_redundant_parents() {
		...
		int orig_cnt = .commit_list_count(generatedParents);
		...
		while (1) {
			...
			if (generatedReflog_info) {
				...
				commit = commit.next_topo_commit(revs);
					...
					commit = .pop_commit(generatedCommits);
			} 
			if (!commit) {
				return ((Object)0);
			} 
	...
	public commit get_revision(rev_info revs) {
		...
		if (!c) {
			...
			if (generatedPrevious_parents) {
				.free_commit_list(generatedPrevious_parents);
	...
	public int stdin_diff_commit(Object p) {
		...
		while (((ModernizedCProgram.sane_ctype[(byte)(p++)] & (true)) != 0) && !oid.parse_oid_hex(p, /* Graft the fake parents locally to the commit */p)) {
			commit parent = .lookup_commit(ModernizedCProgram.the_repository, oid);
			if (!pptr) {
				.free_commit_list(generatedParents);
				this.setParents(((Object)0));
			...
			if (parent) {
				pptr = .commit_list_insert(parent, pptr).getNext();
			} 
		}
	...
	public int parse_commit_no_graph() {
		return .repo_parse_commit_internal(ModernizedCProgram.the_repository, commit, 0, 0);
	}
