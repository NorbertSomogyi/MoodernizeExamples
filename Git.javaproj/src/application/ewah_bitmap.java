...
public class ewah_bitmap {
	...
	public int ewah_serialize_to(Object write_fun, Object data) {
		...
		bitsize = .htonl((uint32_t)generatedBit_size);
		if (.write_fun(data, bitsize, 4) != 4) {
			return -1;
		} 
		Object generatedBuffer_size = this.getBuffer_size();
		word_count = .htonl((uint32_t)generatedBuffer_size);
	...
	public Object ewah_read_mmap(Object map, Object len) {
		...
		this.setBit_size(.get_be32(ptr));
		ptr += ;
		len -= ;
		if (len < ) {
			return ();
		} 
		this.setBuffer_size(this.setAlloc_size(.get_be32(ptr)));
		ptr += ;
		len -= ;
		Object generatedBuffer = this.getBuffer();
		Object generatedAlloc_size = this.getAlloc_size();
		(generatedBuffer) = ModernizedCProgram.xrealloc((generatedBuffer), ModernizedCProgram.st_mult(, (generatedAlloc_size)));
		Object generatedBuffer_size = this.getBuffer_size();
		data_len = ModernizedCProgram.st_mult(generatedBuffer_size, );
		if (len < data_len) {
			return ();
		} 
		.memcpy(generatedBuffer, ptr, data_len);
		ptr += data_len;
		len -= data_len;
		for (i = 0; i < generatedBuffer_size; ++i) {
			generatedBuffer[i] = .ntohll(generatedBuffer[i]);
		}
		if (len < ) {
			return ();
		} 
		this.setRlw(generatedBuffer + .get_be32(ptr));
		ptr += ;
		len -= ;
	...
	public void buffer_grow(Object new_size) {
		...
		(generatedBuffer) = ModernizedCProgram.xrealloc((generatedBuffer), ModernizedCProgram.st_mult(, (generatedAlloc_size)));
		this.setRlw(generatedBuffer + (rlw_offset / ));
	}
	...
	public Object add_empty_words(int v, Object number) {
		...
		can_add = ModernizedCProgram.min_size(number, (((eword_t)1 << ( * 4)) - 1) - runlen);
		ModernizedCProgram.rlw_set_running_len(generatedRlw, runlen + can_add);
		number -= can_add;
		while (number >= (((eword_t)1 << ( * 4)) - 1)) {
			self.buffer_push_rlw(0);
			added++;
			if (v) {
				ModernizedCProgram.rlw_set_run_bit(generatedRlw, v);
			} 
			ModernizedCProgram.rlw_set_running_len(generatedRlw, (((eword_t)1 << ( * 4)) - 1));
			number -= (((eword_t)1 << ( * 4)) - 1);
		}
		if (number > 0) {
			self.buffer_push_rlw(0);
			added++;
	...
	public Object add_literal(Object new_data) {
		...
		if (current_num >= (((eword_t)1 << ( * 8 - 1 - ( * 4))) - 1)) {
			self.buffer_push_rlw(0);
		...
		((ModernizedCProgram.rlw_get_literal_words(generatedRlw) == current_num + 1) ? (Object)0 : ._assert("rlw_get_literal_words(self->rlw) == current_num + 1", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", /* sanity check */122));
		self.buffer_push(new_data);
		return 1;
	}
	public void ewah_add_dirty_words(Object buffer, Object number, int negate) {
		...
		while (1) {
			...
			can_add = ModernizedCProgram.min_size(number, (((eword_t)1 << ( * 8 - 1 - ( * 4))) - 1) - literals);
			...
			if (negate) {
				...
				for (i = 0; i < can_add; ++i) {
					...
					.memcpy(generatedBuffer + generatedBuffer_size, buffer, can_add * );
					generatedBuffer_size += can_add;
			} 
			generatedBit_size += can_add * ( * 8);
			if (number - can_add == 0) {
				break;
			} 
			self.buffer_push_rlw(0);
	...
	public Object add_empty_word(int v) {
		...
		if (no_literal && run_len == 0) {
			ModernizedCProgram.rlw_set_run_bit(generatedRlw, v);
			((ModernizedCProgram.rlw_get_run_bit(generatedRlw) == v) ? (Object)0 : ._assert("rlw_get_run_bit(self->rlw) == v", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 171));
		} 
		if (no_literal && ModernizedCProgram.rlw_get_run_bit(generatedRlw) == v && run_len < (((eword_t)1 << ( * 4)) - 1)) {
			ModernizedCProgram.rlw_set_running_len(generatedRlw, run_len + 1);
			((ModernizedCProgram.rlw_get_running_len(generatedRlw) == run_len + 1) ? (Object)0 : ._assert("rlw_get_running_len(self->rlw) == run_len + 1", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 177));
			return 0;
		} else {
				self.buffer_push_rlw(0);
				((ModernizedCProgram.rlw_get_running_len(generatedRlw) == 0) ? (Object)0 : ._assert("rlw_get_running_len(self->rlw) == 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 182));
				((ModernizedCProgram.rlw_get_run_bit(generatedRlw) == 0) ? (Object)0 : ._assert("rlw_get_run_bit(self->rlw) == 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 183));
				((ModernizedCProgram.rlw_get_literal_words(generatedRlw) == 0) ? (Object)0 : ._assert("rlw_get_literal_words(self->rlw) == 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 184));
				ModernizedCProgram.rlw_set_run_bit(generatedRlw, v);
				((ModernizedCProgram.rlw_get_run_bit(generatedRlw) == v) ? (Object)0 : ._assert("rlw_get_run_bit(self->rlw) == v", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 187));
				ModernizedCProgram.rlw_set_running_len(generatedRlw, 1);
				((ModernizedCProgram.rlw_get_running_len(generatedRlw) == 1) ? (Object)0 : ._assert("rlw_get_running_len(self->rlw) == 1", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 190));
				((ModernizedCProgram.rlw_get_literal_words(generatedRlw) == 0) ? (Object)0 : ._assert("rlw_get_literal_words(self->rlw) == 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 191));
				return 1;
		} 
	}
	...
	public void ewah_set(Object i) {
		Object generatedBit_size = this.getBit_size();
		size_t dist = (((i + 1) + (( * 8)) - 1) / (( * 8))) - (((generatedBit_size) + (( * 8)) - 1) / (( * 8)));
		((i >= generatedBit_size) ? (Object)0 : ._assert("i >= self->bit_size", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\ewah_bitmap.c", 215));
		this.setBit_size(i + 1);
		if (dist > 0) {
			if (dist > 1) {
				self.add_empty_words(0, dist - 1);
			} 
			self.add_literal((eword_t)1 << (i % ( * 8)));
			return ;
		} 
		Object generatedRlw = this.getRlw();
		...
		Object generatedBuffer = this.getBuffer();
		Object generatedBuffer_size = this.getBuffer_size();
		generatedBuffer[generatedBuffer_size - 1] |=  ((eword_t)1 << (i % ( * 8)));
	...
	public void ewah_each_bit(Object callback, Object payload) {
		...
		while (pointer < generatedBuffer_size) {
			...
			if (ModernizedCProgram.rlw_get_run_bit(word)) {
				size_t len = ModernizedCProgram.rlw_get_running_len(word) * ( * 8);
				for (k = 0; k < ModernizedCProgram.len; ) {
					.callback(pos, payload);
				}
			} else {
					pos += ModernizedCProgram.rlw_get_running_len(word) * ( * 8);
			} 
			++pointer;
	...
	public ewah_bitmap ewah_new() {
		...
		self = ModernizedCProgram.xmalloc();
		self.setAlloc_size(32);
		Object generatedBuffer = self.getBuffer();
		Object generatedAlloc_size = self.getAlloc_size();
		(generatedBuffer) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (generatedAlloc_size)));
		self.ewah_clear();
		return self;
	}
	...
	public Object ewah_checksum() {
		...
		Object generatedBuffer_size = this.getBuffer_size();
		size_t size = generatedBuffer_size * ;
		while (size--) {
