...
public class pathspec {
	...
	public void parse_pathspec(int magic_mask, int flags, Object prefix, Object argv) {
		...
		int prefixlen;
		int nr_exclude = 0;
		.memset(ModernizedCProgram.pathspec, 0, );
		if (flags & (1 << 2)) {
		...
		if (!/* No arguments with prefix -> prefix pathspec */entry) {
			...
			ModernizedCProgram.pathspec.setItems(item = ModernizedCProgram.xcalloc(1, ));
			item.setMatch(ModernizedCProgram.xstrdup(prefix));
			item.setOriginal(ModernizedCProgram.xstrdup(prefix));
			item.setNowildcard_len(item.setLen(.strlen(prefix)));
		...
		(ModernizedCProgram.pathspec.getItems()) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (n + 1)));
		...
		if (ModernizedCProgram.pathspec.getMagic() & (1 << 1)) {
			...
			ModernizedCProgram.sane_qsort((ModernizedCProgram.pathspec.getItems()), (ModernizedCProgram.pathspec.getNr()), , pathspec_item_cmp);
		} 
	}
	public void copy_pathspec(Object src) {
		...
		for (i = 0; i < generatedNr; i++) {
			...
			(generatedAttr_match) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (generatedAttr_match_nr)));
			ModernizedCProgram.copy_array((generatedAttr_match), (generatedAttr_match), (generatedAttr_match_nr),  + ( - 1));
	...
	public void chmod_pathspec(byte flip) {
		...
		for (i = 0; i < (ModernizedCProgram.the_index.getCache_nr()); i++) {
			...
			if (ModernizedCProgram.chmod_index_entry(ModernizedCProgram.the_index, (ce), (flip)) < 0) {
				.fprintf((_iob[2]), "cannot chmod %cx '%s'\n", flip, generatedName);
			} 
		}
	}
