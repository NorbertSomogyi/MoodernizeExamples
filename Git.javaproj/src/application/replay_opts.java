...
public class replay_opts {
	...
	public int run_sequencer(int argc, Object[][] argv) {
		...
		if (/* Check for incompatible command line arguments */cmd) {
			...
			if (cmd == (byte)'q') {
				...
				this_operation = "--skip";
					...
					((cmd == (byte)'a') ? (Object)0 : /*Error: Function owner not recognized*/_assert("cmd == 'a'", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Git\\src\\revert.c", 158));
		...
		if (cmd) {
			this.setRevs(((Object)0));
				...
				this.setRevs(ModernizedCProgram.xmalloc(/*Error: sizeof expression not supported yet*/));
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(s_r_opt, 0, /*Error: sizeof expression not supported yet*/);
	...
	public void sequencer_init_config() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/git_config(git_sequencer_config, opts);
	}
	...
	public int sequencer_remove_state() {
		...
		if (ModernizedCProgram.is_rebase_i(opts) && buf.strbuf_read_file(ModernizedCProgram.rebase_path_refs_to_delete(), 0) > 0) {
			byte p = generatedBuf;
			while (p) {
				byte eol = /*Error: Function owner not recognized*/strchr(p, (byte)'\n');
				if (eol) {
					eol = (byte)'\0';
				} 
	...
	public int read_populate_opts() {
		...
		if (ModernizedCProgram.is_rebase_i(opts)) {
			...
			if (ModernizedCProgram.buf.read_oneliner(ModernizedCProgram.rebase_path_allow_rerere_autoupdate(), 1)) {
				if (!/*Error: Function owner not recognized*/strcmp(ModernizedCProgram.buf.getBuf(), "--rerere-autoupdate")) {
					this.setAllow_rerere_auto(1);
				}  else if (!/*Error: Function owner not recognized*/strcmp(ModernizedCProgram.buf.getBuf(), "--no-rerere-autoupdate")) {
			...
			if (generatedCurrent_fixups.read_oneliner(ModernizedCProgram.rebase_path_current_fixups(), 1)) {
				...
				while ((p = /*Error: Function owner not recognized*/strchr(p, (byte)'\n'))) {
		...
		if (/*Error: Function owner not recognized*/git_config_from_file(populate_opts_cb, ModernizedCProgram.git_path_opts_file(), opts) < 0) {
			return ();
		} 
		return 0;
	}
	...
	public int save_opts() {
		...
		if (generatedNo_commit) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.no-commit", "true");
		} 
		int generatedEdit = this.getEdit();
		if (generatedEdit) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.edit", "true");
		} 
		int generatedAllow_empty = this.getAllow_empty();
		if (generatedAllow_empty) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.allow-empty", "true");
		} 
		int generatedAllow_empty_message = this.getAllow_empty_message();
		if (generatedAllow_empty_message) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.allow-empty-message", "true");
		} 
		int generatedKeep_redundant_commits = this.getKeep_redundant_commits();
		if (generatedKeep_redundant_commits) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.keep-redundant-commits", "true");
		} 
		int generatedSignoff = this.getSignoff();
		if (generatedSignoff) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.signoff", "true");
		} 
		int generatedRecord_origin = this.getRecord_origin();
		if (generatedRecord_origin) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.record-origin", "true");
		} 
		int generatedAllow_ff = this.getAllow_ff();
		if (generatedAllow_ff) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.allow-ff", "true");
		} 
		int generatedMainline = this.getMainline();
		if (generatedMainline) {
			strbuf buf = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
			ModernizedCProgram.buf.strbuf_addf("%d", generatedMainline);
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.mainline", ModernizedCProgram.buf.getBuf());
			ModernizedCProgram.buf.strbuf_release();
		} 
		Byte generatedStrategy = this.getStrategy();
		if (generatedStrategy) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.strategy", generatedStrategy);
		} 
		Byte generatedGpg_sign = this.getGpg_sign();
		if (generatedGpg_sign) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.gpg-sign", generatedGpg_sign);
		} 
		byte[][] generatedXopts = this.getXopts();
		Object generatedXopts_nr = this.getXopts_nr();
		if (generatedXopts) {
			int i;
			for (i = 0; i < generatedXopts_nr; i++) {
				res |=  /*Error: Function owner not recognized*/git_config_set_multivar_in_file_gently(opts_file, "options.strategy-option", generatedXopts[i], "^$", 0);
			}
		} 
		int generatedAllow_rerere_auto = this.getAllow_rerere_auto();
		if (generatedAllow_rerere_auto) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.allow-rerere-auto", generatedAllow_rerere_auto == 1 ? "true" : "false");
		} 
		...
		if (generatedExplicit_cleanup) {
			res |=  /*Error: Function owner not recognized*/git_config_set_in_file_gently(opts_file, "options.default-msg-cleanup", ModernizedCProgram.describe_cleanup_mode(generatedDefault_msg_cleanup));
		} 
		return res;
	}
	public int apply_autostash() {
		strbuf stash_sha1 = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (!child.run_command()) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Applied autostash.\n"));
		} else {
				...
				if (store.run_command()) {
					ret = ();
						...
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), ModernizedCProgram._("Applying autostash resulted in conflicts.\nYour changes are safe in the stash.\nYou can run \"git stash pop\" or \"git stash drop\" at any time.\n"));
				} 
		} 
		stash_sha1.strbuf_release();
		return ret;
		byte path = opts.state_dir_path("autostash");
		strbuf autostash = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		...
		if (!stash_apply.run_command()) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/printf(ModernizedCProgram._("Applied autostash.\n"));
		} else {
