...
public class packed_git {
	...
	public int load_idx(Object path, Object hashsz, Object idx_map, Object idx_size) {
		...
		if (generatedIdx_signature == .htonl(-1024)) {
			version = .ntohl(generatedIdx_version);
	...
	public packed_git alloc_packed_git(int extra) {
		packed_git p = ModernizedCProgram.xmalloc(ModernizedCProgram.st_add(, extra));
		.memset(p, 0, );
		p.setPack_fd(-1);
		return p;
	}
	public packed_git parse_pack_index(Byte sha1, Object idx_path) {
		...
		size_t alloc = ModernizedCProgram.st_add(.strlen(path), 1);
		packed_git packed_git = new packed_git();
		packed_git p = packed_git.alloc_packed_git(alloc);
		Object generatedPack_name = p.getPack_name();
		.memcpy(generatedPack_name, path, /* includes NUL */alloc);
	...
	public int unuse_one_window() {
		...
		if (lru_p) {
			.munmap(generatedBase, generatedLen);
	...
	public void close_pack_index() {
		...
		if (generatedIndex_data) {
			.munmap((Object)generatedIndex_data, generatedIndex_size);
	...
	public Object pack_basename() {
		Object generatedPack_name = this.getPack_name();
		byte ret = .strrchr(generatedPack_name, (byte)'/');
		if (ret) {
	...
	public int open_packed_git_1() {
		...
		int generatedPack_fd = this.getPack_fd();
		if (generatedPack_fd < 0 || .fstat(generatedPack_fd, st)) {
			return -1;
		} 
		ModernizedCProgram.pack_open_fds++;
		...
		read_result = ModernizedCProgram.read_in_full(generatedPack_fd, hdr, );
		if (read_result < 0) {
			return ();
		} 
		if (read_result != ) {
		...
		if (generatedNum_objects != .ntohl(generatedHdr_entries)) {
			return ();
		} 
		if (.lseek(generatedPack_fd, generatedPack_size - hashsz, 0) == -1) {
			return ();
		} 
	...
	public packed_git add_packed_git(Object path, Object path_len, int local) {
		...
		alloc = ModernizedCProgram.st_add(ModernizedCProgram.st_add((path_len), (.strlen(".promisor"))), (true));
		packed_git packed_git = new packed_git();
		p = packed_git.alloc_packed_git(alloc);
		Object generatedPack_name = p.getPack_name();
		.memcpy(generatedPack_name, path, path_len);
		ModernizedCProgram.xsnprintf(generatedPack_name + path_len, alloc - path_len, ".keep");
		if (!.access(generatedPack_name, 0)) {
			p.setPack_keep(1);
		} 
		...
		Object generatedSt_mode = st.getSt_mode();
		if (.stat(generatedPack_name, st) || !(((generatedSt_mode) & -1024) == -1024)) {
			ModernizedCProgram.free(p);
			return ((Object)0);
		} 
		Object generatedSt_size = st.getSt_size();
	...
	public void add_delta_base_cache(Object base_offset, Object base, long base_size, object_type type) {
		delta_base_cache_entry ent = ModernizedCProgram.xmalloc();
	...
	public int add_promisor_object(Object oid, Object pos, Object set_) {
		...
		if (generatedType == object_type.OBJ_TREE) {
			...
			set.oidset_insert(.get_commit_tree_oid(commit));
			for (; parents; parents = generatedNext) {
				set.oidset_insert(generatedOid);
			}
		}  else if (generatedType == object_type.OBJ_TAG) {
			tag tag = (tag)obj;
			set.oidset_insert(.get_tagged_oid(tag));
		} 
		return 0;
	}
	...
	public void create_object_entry(Object oid, object_type type, Object hash, int exclude, int no_try_delta, Object found_offset) {
		...
		entry = .packlist_alloc(ModernizedCProgram.to_pack, oid);
		entry.setHash(hash);
	...
	public void create_pack_revindex() {
		...
		int hashsz = ModernizedCProgram.the_repository.getHash_algo().getRawsz();
		revindex_entry generatedRevindex = this.getRevindex();
		(generatedRevindex) = ModernizedCProgram.xmalloc(ModernizedCProgram.st_mult(, (num_ent + 1)));
		index += 4 * 256;
		int generatedIndex_version = this.getIndex_version();
		if (generatedIndex_version > 1) {
			...
			for (i = 0; i < num_ent; i++) {
				...
				if (!(off & -1024)) {
					generatedRevindex[i].setOffset(off);
						...
						generatedRevindex[i].setOffset(.get_be64(off_64));
						off_64 += 2;
				} 
				...
				for (i = 0; i < num_ent; i++) {
					...
					generatedRevindex[i].setOffset(.ntohl(hl));
					generatedRevindex[i].setNr(i);
				}
		} 
		Object generatedPack_size = this.getPack_size();
		...
		for (p = packed_git.get_all_packs(ModernizedCProgram.the_repository); p; p = generatedNext) {
			...
			if (limit) {
				if (generatedPack_size >= limit) {
					packs.string_list_append(generatedPack_name);
				} 
	...
	public Object estimate_repack_memory() {
		...
		heap =  * /* then pack-objects needs lots more for book keeping */nr_objects/*
		...
		heap +=  * nr_objects / 2/*
			 ...
			 */;
		heap +=  * nr_objects / 2;
		heap +=  * /* and then obj_hash[], underestimated in fact */nr_objects;
		heap +=  * /* revindex is used also */nr_objects/*
	...
	public int verify_pack_index() {
		...
		.UNRECOGNIZEDFUNCTIONNAME(/* Verify SHA1 sum of the index file */ctx);
		.UNRECOGNIZEDFUNCTIONNAME(ctx, index_base, (int)(index_size - ModernizedCProgram.the_repository.getHash_algo().getRawsz()));
		.UNRECOGNIZEDFUNCTIONNAME(hash, ctx);
