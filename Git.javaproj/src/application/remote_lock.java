...
public class remote_lock {
	...
	public curl_slist get_dav_token_headers(dav_header_flag options) {
		...
		if (dav_header_flag.options & dav_header_flag.DAV_HEADER_IF) {
			...
			dav_headers = /*Error: Function owner not recognized*/curl_slist_append(dav_headers, generatedBuf);
			buf.strbuf_setlen(0);
		} 
	...
	public int refresh_lock() {
		...
		if (slot.start_active_slot()) {
			...
			if (generatedCurl_result != CURLE_OK) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "LOCK HTTP error %ld\n", generatedHttp_code);
			} else {
					this.setStart_time(/*Error: Function owner not recognized*/time(((Object)0)));
					rc = 1;
			} 
		} 
		this.setRefreshing(0);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/curl_slist_free_all(dav_headers);
		return rc;
	}
	public remote_lock lock_remote(Object path, long timeout) {
		active_request_slot slot = new active_request_slot();
		slot_results results = new slot_results();
		buffer out_buffer = new buffer(new buffer(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/), 0);
		strbuf in_buffer = new strbuf(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/);
		byte url;
		byte ep;
		...
		ep = /*Error: Function owner not recognized*/strchr(url + /*Error: Function owner not recognized*/strlen(ModernizedCProgram.repo.getUrl()) + 1, /* Make sure leading directories exist for the remote ref */(byte)'/');
		active_request_slot active_request_slot = new active_request_slot();
		Object generatedCurl = slot.getCurl();
		Object generatedCurl_result = results.getCurl_result();
		long generatedHttp_code = results.getHttp_code();
		while (ep) {
			...
			if (slot.start_active_slot()) {
				...
				if (generatedCurl_result != CURLE_OK && generatedHttp_code != 405) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Unable to create branch path %s\n", url);
					ModernizedCProgram.free(url);
					return ((Object)0);
				} 
			} else {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Unable to start MKCOL request\n");
					ModernizedCProgram.free(url);
					return ((Object)0);
			} 
			ep[1] = saved_character;
			ep = /*Error: Function owner not recognized*/strchr(ep + 1, (byte)'/');
		}
		...
		ModernizedCProgram.xsnprintf(timeout_header, /*Error: sizeof expression not supported yet*/, "Timeout: Second-%ld", timeout);
		dav_headers = /*Error: Function owner not recognized*/curl_slist_append(dav_headers, timeout_header);
		dav_headers = /*Error: Function owner not recognized*/curl_slist_append(dav_headers, "Content-Type: text/xml");
		slot = active_request_slot.get_active_slot();
		slot.setResults(results);
		out_buffer.curl_setup_http(generatedCurl, url, "LOCK", ModernizedCProgram.fwrite_buffer);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/curl_easy_setopt(generatedCurl, CURLOPT_HTTPHEADER, dav_headers);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/curl_easy_setopt(generatedCurl, CURLOPT_FILE, in_buffer);
		lock = ModernizedCProgram.xcalloc(1, /*Error: sizeof expression not supported yet*/);
		lock.setTimeout(-1);
		...
		if (slot.start_active_slot()) {
			slot.run_active_slot();
			if (generatedCurl_result == CURLE_OK) {
				 parser = /*Error: Function owner not recognized*/XML_ParserCreate(((Object)0));
				XML_Status result;
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/XML_SetUserData(parser, ctx);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/XML_SetElementHandler(parser, xml_start_tag, xml_end_tag);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/XML_SetCharacterDataHandler(parser, xml_cdata);
				XML_Status.result = /*Error: Function owner not recognized*/XML_Parse(parser, generatedBuf, generatedLen, 1);
				ModernizedCProgram.free(generatedName);
				if (XML_Status.result != 1) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "XML error: %s\n", /*Error: Function owner not recognized*/XML_ErrorString(/*Error: Function owner not recognized*/XML_GetErrorCode(parser)));
					lock.setTimeout(-1);
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/XML_ParserFree(parser);
			} else {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "error: curl result=%d, HTTP code=%ld\n", generatedCurl_result, generatedHttp_code);
			} 
		} else {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Unable to start LOCK request\n");
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/curl_slist_free_all(dav_headers);
		...
		if (generatedToken == ((Object)0) || generatedTimeout <= 0) {
			...
			do {
				...
				lock.setUrl(url);
				lock.setStart_time(/*Error: Function owner not recognized*/time(((Object)0)));
		...
		if (slot.start_active_slot()) {
			...
			if (generatedCurl_result == CURLE_OK) {
				rc = 1;
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "UNLOCK HTTP error %ld\n", generatedHttp_code);
			} 
		} else {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Unable to start UNLOCK request\n");
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/curl_slist_free_all(dav_headers);
	...
	public int update_remote(Object oid) {
		...
		if (slot.start_active_slot()) {
			...
			generatedBuf.strbuf_release();
			if (generatedCurl_result != CURLE_OK) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "PUT error: curl result=%d, HTTP code=%ld\n", generatedCurl_result, generatedHttp_code);
				return /* We should attempt recovery? */0;
			} 
		} else {
				generatedBuf.strbuf_release();
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fprintf((_iob[2]), "Unable to start PUT request\n");
				return 0;
		} 
		return 1;
	}
	public void update_remote_info_refs() {
		buffer buffer = new buffer(new buffer(/*Error: Invalid initializer*/, /*Error: Invalid initializer*/, /*Error: Invalid initializer*/), 0);
		active_request_slot slot = new active_request_slot();
		slot_results results = new slot_results();
		curl_slist dav_headers = new curl_slist();
