...
public class file_buffer {
	...
	public int check_readonly(Integer forceit) {
	...
	public void home_replace(Object[] src, Object dst, int dstlen, int one) {
		...
		if (ModernizedCProgram.homedir != ((Object)0)) {
			dirlen = /*Error: Function owner not recognized*/strlen((byte)(ModernizedCProgram.homedir));
		} 
		homedir_env_orig = homedir_env = (char_u)/*Error: Function owner not recognized*/getenv((byte)((char_u)"HOME"));
		if (homedir_env == ((Object)0)) {
			homedir_env_orig = homedir_env = (char_u)/*Error: Function owner not recognized*/getenv((byte)((char_u)"USERPROFILE"));
		} 
		...
		if (homedir_env != ((Object)0) && homedir_env == (byte)'~') {
			...
			flen = (int)/*Error: Function owner not recognized*/strlen((byte)(homedir_env));
		...
		if (homedir_env != ((Object)0)) {
			envlen = /*Error: Function owner not recognized*/strlen((byte)(homedir_env));
		} 
		if (!one) {
	...
	public Object home_replace_save(Object src) {
		...
		// just in caseif (src != ((Object)0)) {
			len += (int)/*Error: Function owner not recognized*/strlen((byte)(src));
		} 
	...
	public void filemess(Object name, Object s, int attr) {
		...
		if (/*Error: Function owner not recognized*/strlen((byte)(ModernizedCProgram.IObuff)) > (1024 + 1) - /* If it's extremely long, truncate it. */80) {
			ModernizedCProgram.IObuff[(1024 + 1) - 80] = (byte)'\000';
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(ModernizedCProgram.IObuff), (byte)(s/*
	...
	public int buf_check_timestamp(int focus) {
			...
			if (ModernizedCProgram.mch_isdir(generatedB_fname)) {
				...
				reload = 1;
					...
					if (n) {
						...
						if (/*Error: Function owner not recognized*/strcmp((byte)(s), (byte)("reload")) == 0 && reason != (byte)'d') {
							reload = 1;
						}  else if (/*Error: Function owner not recognized*/strcmp((byte)(s), (byte)("ask")) == 0) {
							n = 0;
						} else {
								return 2;
						} 
		...
		if (mesg != ((Object)0)) {
			...
			if (path != ((Object)0)) {
				...
				tbuf = ModernizedCProgram.alloc(/*Error: Function owner not recognized*/strlen((byte)(path)) + /*Error: Function owner not recognized*/strlen((byte)(mesg)) + /*Error: Function owner not recognized*/strlen((byte)(mesg2)) + 2);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/sprintf(tbuf, mesg, path);
				...
				if (can_reload) {
					if (mesg2 != (byte)'\000') {
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(tbuf), (byte)("\n"));
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(tbuf), (byte)(mesg2));
					} 
	 ...
	 */
	public int tclgetlinenum(Object interp, Object obj, Integer valueP) {
		int err;
		int i;
		byte[] keyw = new byte[]{"begin", "bottom", "end", "first", "last", "start", "top", (byte)0};
		err = /*Error: Function owner not recognized*/Tcl_GetIndexFromObj(interp, obj, keyw, "", 0, i);
		memline generatedB_ml = this.getB_ml();
		Object generatedMl_line_count = generatedB_ml.getMl_line_count();
		if (err == TCL_OK) {
			switch (i) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Tcl_ResetResult(interp);
		err = /*Error: Function owner not recognized*/Tcl_GetIntFromObj(interp, obj, i);
		if (err != TCL_OK) {
			return err;
		} 
		i = ((i) + (ModernizedCProgram.tclinfo.getLbase() == 0));
		if (i < 1 || i > generatedMl_line_count) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Tcl_SetResult(interp, ((byte)("line number out of range")), TCL_STATIC);
			return TCL_ERROR;
		} 
		valueP = i;
		return TCL_OK/*
	...
	public int ml_open() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedB0_version), (byte)("VIM "), (size_t)(true));
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strncpy((byte)(generatedB0_version + 4), (byte)(ModernizedCProgram.Version), (size_t)(true));
		...
		generatedDb_free -= 1 + (/*Error: Unsupported expression*/);
	...
	public void ml_set_crypt_key(Object old_key, Object old_cm) {
		...
		if (old_method > 0 && old_key != (byte)'\000') {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedMf_old_seed), (byte)(generatedMf_seed), (size_t)(true));
		} 
	...
	public void ml_setname() {
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			...
			if (generatedMf_fd >= /* need to close the swap file before renaming */0) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/close(generatedMf_fd);
				mfp.setMf_fd(-1);
			} 
	...
	public Object ml_get_buf(Object lnum, int will_change) {
		...
		if (generatedMl_line_lnum != lnum || mf_dont_release) {
			...
			start = ((generatedDb_index[idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
			if (idx == 0) {
				end = generatedDb_txt_end;
			} else {
					end = ((generatedDb_index[idx - 1]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
			} 
			len = end - start;
	...
	public void add_text_props_for_append(Object lnum, Object line, Integer len, Object tofree) {
		int round;
		...
		// 1. calculate the extra space needed// 2. allocate the space and fill itfor (round = 1; round <= 2; ++round) {
			if (round == 2) {
				...
				new_len = len + new_prop_count * /*Error: Unsupported expression*/;
				new_line = ModernizedCProgram.alloc(new_len);
				if (new_line == ((Object)0)) {
					return /*Error: Unsupported expression*/;
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(new_line), (byte)(line), (size_t)(len));
				new_prop_count = 0;
			} 
			count = buf.get_text_props(lnum, props, 0);
			for (n = 0; n < count; ++n) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(prop), (byte)(props + n * /*Error: Unsupported expression*/), (size_t)(/*Error: Unsupported expression*/));
				if (generatedTp_flags & 1) {
					if (round == 2) {
						generatedTp_flags |=  2;
						prop.setTp_col(1);
						prop.setTp_len(len);
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(new_line + len + new_prop_count * /*Error: Unsupported expression*/), (byte)(prop), (size_t)(/*Error: Unsupported expression*/));
					} 
					++new_prop_count;
				} 
			}
	...
	public int ml_append_int(Object lnum, Object line_arg, Object len_arg, int newfile, int mark) {
		...
		if (len == 0) {
			len = (colnr_T)/*Error: Function owner not recognized*/strlen((byte)(line)) + 1;
		} 
		...
		// space needed for text + index// space needed for text + indexspace_needed = len + (/*Error: Unsupported expression*/);
		mfp = generatedMl_mfp;
		int generatedMf_page_size = mfp.getMf_page_size();
		...
		if ((int)generatedDb_free >= /* enough room in data block */space_needed/*
			...
			if (line_count > db_idx + /* if there are following lines */1/*
				...
				if (db_idx < 0) {
					offset = generatedDb_txt_end;
						...
						offset = ((generatedDb_index[db_idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp + generatedDb_txt_start), (byte)((byte)dp + generatedDb_txt_start + len), (size_t)((size_t)(offset - (generatedDb_txt_start + len))));
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp + generatedDb_index[db_idx + 1]), (byte)(line), (size_t)((size_t)len));
			if (mark) {
				generatedDb_index[db_idx + 1] |=  ((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1/*
			...
			if (!newfile) {
					...
					in_left = 1/* space_needed does not change */;
						...
						if (lines_moved == 0) {
							in_left = /* put new line in right block */0/* space_needed does not change */;
								...
								data_moved = ((generatedDb_index[db_idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))) - generatedDb_txt_start;
								total_moved = data_moved + lines_moved * (/*Error: Unsupported expression*/);
				...
				page_count = ((space_needed + (/*Error: Unsupported expression*/ - (/*Error: Unsupported expression*/))) + page_size - 1) / page_size;
				...
				if (!in_left) {
					generatedDb_txt_start -= len;
					generatedDb_free -= len + (/*Error: Unsupported expression*/);
					generatedDb_index[0] = generatedDb_txt_start;
					if (mark) {
						generatedDb_index[0] |=  ((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1));
					} 
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp_right + generatedDb_txt_start), (byte)(line), (size_t)((size_t)len));
					++line_count_right/*
				...
				if (lines_moved) {
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp_right + generatedDb_txt_start), (byte)((byte)dp_left + generatedDb_txt_start), (size_t)((size_t)data_moved));
				...
				if (/*
					...
					if (mark) {
						generatedDb_index[line_count_left] |=  ((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1));
					} 
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp_left + generatedDb_txt_start), (byte)(line), (size_t)((size_t)len));
					++line_count_left;
				} 
				if (db_idx < /* left block is new */0) {
					lnum_left = lnum + 1;
				...
				for (stack_idx = generatedMl_stack_top - 1; stack_idx >= 0; --stack_idx) {
					...
					if (generatedPb_count < generatedPb_count_max) {
						if (pb_idx + 1 < (int)generatedPb_count) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedPb_pointer[pb_idx + 2]), (byte)(generatedPb_pointer[pb_idx + 1]), (size_t)((size_t)(generatedPb_count - pb_idx - 1) * /*Error: Unsupported expression*/));
						} 
						++generatedPb_count;
						...
						break;
							...
							for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
								...
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(pp_new), (byte)(pp), (size_t)((size_t)page_size));
								pp.setPb_count(1);
							...
							if (total_moved) {
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedPb_pointer[0]), (byte)(generatedPb_pointer[pb_idx + 1]), (size_t)((size_t)(total_moved) * /*Error: Unsupported expression*/));
	...
	public void adjust_text_props_for_delete(Object lnum, Object del_props, int del_props_len, int above) {
		...
		int generatedMl_flags = generatedB_ml.getMl_flags();
		for (done_del = 0; done_del < del_props_len; done_del += /*Error: Unsupported expression*/) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(prop_del), (byte)(del_props + done_del), (size_t)(/*Error: Unsupported expression*/));
			if ((above && (generatedTp_flags & 2) && !(generatedTp_flags & 1)) || (!above && (generatedTp_flags & 1) && !(generatedTp_flags & 2))) {
				if (!did_get_line) {
					...
					dp = (DATA_BL)(generatedBh_data);
					idx = lnum - generatedMl_locked_low;
					line_start = ((generatedDb_index[idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
					if (idx == 0) {
						line_size = generatedDb_txt_end - line_start;
					} else {
							line_size = ((generatedDb_index[idx - 1]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))) - line_start;
					} 
					text = (char_u)dp + line_start;
					textlen = /*Error: Function owner not recognized*/strlen((byte)(text)) + 1;
				...
				found = 0;
				for (done_this = 0; done_this < this_props_len; done_this += /*Error: Unsupported expression*/) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(prop_this), (byte)(text + textlen + done_del), (size_t)(/*Error: Unsupported expression*/));
					if (generatedTp_id == generatedTp_id && generatedTp_type == generatedTp_type) {
						int flag = above ? 1 : 2;
						found = 1;
						if (generatedTp_flags & flag) {
							generatedTp_flags &=  ~flag;
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(text + textlen + done_del), (byte)(prop_this), (size_t)(/*Error: Unsupported expression*/));
						}  else if (above) {
	...
	public int ml_delete_int(Object lnum, int message) {
		...
		// If there are text properties, make a copy, so that we can update// properties in preceding and following lines.if (generatedB_has_textprop) {
			size_t textlen = /*Error: Function owner not recognized*/strlen((byte)((char_u)dp + line_start)) + 1;
			if ((long)textlen < line_size) {
		...
		if (count == 1) {
				...
				if (count == /* the pointer block becomes empty! */0) {
					ModernizedCProgram.mf_free(mfp, hp);
						...
						if (count != /* move entries after the deleted one */idx) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedPb_pointer[idx]), (byte)(generatedPb_pointer[idx + 1]), (size_t)((size_t)(count - idx) * /*Error: Unsupported expression*/));
						} 
						ModernizedCProgram.mf_put(mfp, hp, 1, 0);
			...
			do {
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp + text_start + line_size), (byte)((byte)dp + text_start), (size_t)((size_t)(line_start - text_start/*
				...
				generatedDb_free += line_size + (/*Error: Unsupported expression*/);
				...
				ModernizedCProgram.siemsg(((byte)("E320: Cannot find line %ld")), lnum);
					...
					if (idx == /* line is last in block */0) {
						old_len = generatedDb_txt_end - start;
							...
							old_len = (generatedDb_index[idx - 1] & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))) - /* text of previous line follows */start;
					} 
					...
					if ((int)generatedDb_free >= extra) {
						...
						if (extra != 0 && idx < count - 1) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((byte)dp + generatedDb_txt_start - extra), (byte)((byte)dp + generatedDb_txt_start), (size_t)((size_t)(start - generatedDb_txt_start)));
						...
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(old_line - extra), (byte)(new_line), (size_t)((size_t)/* copy new line into the data block */new_len));
						generatedMl_flags |=  (4 | 8);
						buf.ml_updatechunk(lnum, (long)extra, /* The else case is already covered by the insert and delete */3/*
							...
							(Object)buf.ml_append_int(lnum, new_line, new_len, /* How about handling errors??? */0, (generatedDb_index[idx] & ((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
	...
	public int ml_add_stack() {
		...
		if (top == generatedMl_stack_size) {
			...
			newstack = (infoptr_T)ModernizedCProgram.alloc(/*Error: Unsupported expression*/ * (generatedMl_stack_size + 5));
			if (newstack == ((Object)0)) {
				return -1;
			} 
			if (top > 0) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(newstack), (byte)(generatedMl_stack), (size_t)((size_t)top * /*Error: Unsupported expression*/));
			} 
			ModernizedCProgram.vim_free(generatedMl_stack);
	...
	public Object makeswapname(Object fname, Object ffname, Object[] dir_name) {
		...
		// Need _very_ long file namesint len = (int)/*Error: Function owner not recognized*/strlen((byte)(dir_name));
		s = dir_name + len;
	...
	public Object findswapname(Object dirp, Object old_fname) {
		...
		if (buf_fname != ((Object)0) && !ModernizedCProgram.mch_isFullName(buf_fname) && ModernizedCProgram.vim_strchr(ModernizedCProgram.gettail(buf_fname), (byte)':')) {
			...
			if (buf_fname == ((Object)0)) {
				buf_fname = generatedB_fname;
					...
					for (t = ModernizedCProgram.gettail(buf_fname); t != (byte)'\000'; t += /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(t)) {
						if (t == (byte)':') {
							t = (byte)'%'/*
		...
		dir_name = ModernizedCProgram.alloc(/*Error: Function owner not recognized*/strlen((byte)(dirp)) + 1);
		...
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			...
			if ((n = (int)/*Error: Function owner not recognized*/strlen((byte)(fname))) == /* safety check */0) {
				do {
					if ((fname) != ((Object)0)) {
						ModernizedCProgram.vim_free(fname);
						(fname) = ((Object)0);
			...
			if (fname[n - 2] == (byte)'w' && fname[n - 1] == /* first try */(byte)'p'/*
				...
				if (!recoverymode && buf_fname != ((Object)0) && !generatedB_help && !(generatedB_flags & (-1024 | -1024))) {
					...
					if (fd >= 0) {
						if (ModernizedCProgram.read_eintr(fd, b0, /*Error: sizeof expression not supported yet*/) == /*Error: sizeof expression not supported yet*/) {
					...
					if (differ == 0 && !(generatedB_flags & /* give the ATTENTION message when there is an old swap file
						...
						if (dummyfd != ((Object)0)) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fclose(dummyfd);
							dummyfd = ((Object)0);
						...
						if (swap_exists_action != 0 && choice == 0) {
							...
							name = ModernizedCProgram.alloc(/*Error: Function owner not recognized*/strlen((byte)(fname)) + /*Error: Function owner not recognized*/strlen((byte)(((byte)("Swap file \"")))) + /*Error: Function owner not recognized*/strlen((byte)(((byte)("\" already exists!")))) + 5);
							if (name != ((Object)0)) {
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcpy((byte)(name), (byte)(((byte)("Swap file \""))));
								((Object)0).home_replace(fname, name + /*Error: Function owner not recognized*/strlen((byte)(name)), 1000, 1);
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(name), (byte)(((byte)("\" already exists!"))));
							} 
	...
	public void ml_updatechunk(long line, long len, int updtype) {
		...
		Object generatedMl_chunksize = generatedB_ml.getMl_chunksize();
		if (generatedMl_chunksize == ((Object)0)) {
			generatedB_ml.setMl_chunksize((chunksize_T)ModernizedCProgram.alloc(/*Error: Unsupported expression*/ * (true)));
			if (generatedMl_chunksize == ((Object)0)) {
		...
		if (updtype == 1) {
			...
			if (generatedMl_usedchunks + 1 >= generatedMl_numchunks) {
				...
				generatedB_ml.setMl_chunksize((chunksize_T)/*Error: Function owner not recognized*/realloc((generatedMl_chunksize), (/*Error: Unsupported expression*/ * generatedMl_numchunks)));
				if (generatedMl_chunksize == ((Object)0)) {
			...
			if (generatedMlcs_numlines >= 800) {
				...
				int text_end;
				int linecnt;
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedMl_chunksize + curix + 1), (byte)(generatedMl_chunksize + curix), (size_t)((generatedMl_usedchunks - curix) * /*Error: Unsupported expression*/));
				...
				while (curline < generatedMl_line_count && linecnt < 400) {
					...
					if (generatedB_has_textprop) {
						...
						for (i = end_idx; i < idx; ++i) {
							size += (int)/*Error: Function owner not recognized*/strlen((byte)((char_u)dp + (generatedDb_index[i] & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))))) + 1;
						}
					} else {
							if (idx == /* first line in block, text at the end */0) {
								text_end = generatedDb_txt_end;
							} else {
									text_end = ((generatedDb_index[idx - 1]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
							} 
							size += text_end - ((generatedDb_index[end_idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
					} 
				}
					...
					curchnk.setMlcs_totalsize(0);
						...
						if (generatedDb_line_count == 1) {
							rest = generatedDb_txt_end - generatedDb_txt_start;
								...
								rest = ((generatedDb_index[generatedDb_line_count - 2]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))) - generatedDb_txt_start;
						} 
			...
			if (curix < (generatedMl_usedchunks - 1) && (generatedMlcs_numlines + generatedMlcs_numlines) <= 400) {
				...
				generatedMl_usedchunks--;
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedMl_chunksize), (byte)(generatedMl_chunksize + 1), (size_t)(generatedMl_usedchunks * /*Error: Unsupported expression*/));
				return /*Error: Unsupported expression*/;
			...
			if (curix < generatedMl_usedchunks) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(generatedMl_chunksize + curix), (byte)(generatedMl_chunksize + curix + 1), (size_t)((generatedMl_usedchunks - curix) * /*Error: Unsupported expression*/));
			} 
			return /*Error: Unsupported expression*/;
		} 
		ml_upd_lastbuf = buf;
		...
		while ((lnum != 0 && curline < lnum) || (offset != 0 && size < offset)) {
			...
			if (lnum != /* Compute index of last line to use in this MEMLINE */0) {
				if (curline + (count - idx) >= lnum) {
					...
					extra = 0;
					while (offset >= size + text_end - (int)((generatedDb_index[idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))) + ffdos) {
						if (ffdos) {
							size++;
						} 
						if (idx == count - 1) {
			...
			if (generatedB_has_textprop) {
				...
				for (i = start_idx; i <= idx; ++i) {
					len += (int)/*Error: Function owner not recognized*/strlen((byte)((char_u)dp + ((generatedDb_index[i]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))))) + 1;
				}
			} else {
					len = text_end - ((generatedDb_index[idx]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1))));
			} 
			size += len;
			if (offset != 0 && size >= offset) {
				if (size + ffdos == offset) {
					...
					offp = offset - size + len;
						...
						offp = offset - size + len - (text_end - ((generatedDb_index[idx - 1]) & (~((int)1 << ((/*Error: Unsupported expression*/ * 8) - 1)))));
				} 
	...
	public int sign_place(Integer sign_id, Object sign_group, Object sign_name, Object lnum, int prio) {
		...
		for (sp = ModernizedCProgram.first_sign; sp != ((Object)0); sp = generatedSn_next) {
			if (/*Error: Function owner not recognized*/strcmp((byte)(generatedSn_name), (byte)(sign_name)) == 0) {
				break;
			} 
		}
		if (sp == ((Object)0)) {
	...
	public Object sign_jump(int sign_id, Object sign_group) {
		...
		// goto a sign ...if (window_S.buf_jump_open_win(buf) != ((Object)0)) {
			...
			ModernizedCProgram.beginline(1);
				...
				cmd = ModernizedCProgram.alloc(/*Error: Function owner not recognized*/strlen((byte)(generatedB_fname)) + 25);
				if (cmd == ((Object)0)) {
					return -1;
				} 
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/sprintf((byte)cmd, "e +%ld %s", (long)lnum, generatedB_fname);
	...
	public int parse_sign_cmd_args(int cmd, Object arg, Object sign_name, Integer signid, Object group, Integer prio, Object lnum) {
		...
		file_buffer file_buffer = new file_buffer();
		while (arg != (byte)'\000') {
			if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("line="), (size_t)(true)) == 0) {
				arg += 5;
				lnum = /*Error: Function owner not recognized*/atoi((byte)arg);
				arg = ModernizedCProgram.skiptowhite(arg);
				lnum_arg = 1;
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("*"), (size_t)(true)) == 0 && cmd == 4) {
				if (signid != -1) {
					ModernizedCProgram.emsg(((byte)(e_invarg)));
					return 0;
				} 
			...
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("name="), (size_t)(true)) == 0) {
				arg += 5;
				name = arg;
				arg = ModernizedCProgram.skiptowhite(arg);
				if (arg != (byte)'\000') {
				...
				sign_name = name;
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("group="), (size_t)(true)) == 0) {
				arg += 6;
				group = arg;
				arg = ModernizedCProgram.skiptowhite(arg);
				if (arg != (byte)'\000') {
			...
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("priority="), (size_t)(true)) == 0) {
				arg += 9;
				prio = /*Error: Function owner not recognized*/atoi((byte)arg);
				arg = ModernizedCProgram.skiptowhite(arg);
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("file="), (size_t)(true)) == 0) {
				arg += 5;
				filename = arg;
				buf = file_buffer.buflist_findname_exp(arg);
				break;
			}  else if (/*Error: Function owner not recognized*/strncmp((byte)(arg), (byte)("buffer="), (size_t)(true)) == 0) {
				arg += 7;
				filename = arg;
	...
	public void buf_copy_options(int flags) {
		...
		if (ModernizedCProgram.p_cpo != ((Object)0/*
			...
			if (should_copy || (flags & 2)) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset((generatedB_p_script_ctx), (false), (/*Error: sizeof expression not supported yet*/));
	...
	public void save_file_ff() {
		...
		Object generatedB_p_fenc = this.getB_p_fenc();
		if (generatedB_start_fenc == ((Object)/* Only use free/alloc when necessary, they take time. */0) || /*Error: Function owner not recognized*/strcmp((byte)(generatedB_start_fenc), (byte)(generatedB_p_fenc)) != 0) {
	...
	public int file_ff_differs(int ignore_empty) {
		...
		return (/*Error: Function owner not recognized*/strcmp((byte)(generatedB_start_fenc), (byte)(generatedB_p_fenc)) != 0/*
	...
	public void close_windows(int keep_curwin) {
		...
		window_S generatedW_next = wp.getW_next();
		for (wp = ModernizedCProgram.firstwin; wp != ((Object)0) && !(ModernizedCProgram.firstwin == ModernizedCProgram.lastwin); /*Error: Unsupported expression*/) {
	...
	public int get_text_props(Object lnum, Object props, int will_change) {
		...
		if (proplen % /*Error: Unsupported expression*/ != 0) {
		...
		return (int)(proplen / /*Error: Unsupported expression*//*
	...
	public int buf_init_chartab(int global) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset((generatedB_chartab), (false), ((size_t)/*
	...
	public int vim_iswordp_buf(Object p) {
		...
		if (has_mbyte && ModernizedCProgram.mb_bytelen_tab[c] > 1) {
			c = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(p);
		} 
		return buf/*
	...
	public void parse_cino() {
		...
		Object generatedB_p_cino = this.getB_p_cino();
		for (p = generatedB_p_cino; p; /*Error: Unsupported expression*/) {
			l = p++;
			if (p == (byte)'-') {
				++p;
			} 
			...
			divider = 0;
			if (p == (byte)'.') {
				fraction = /*Error: Function owner not recognized*/atol((byte)++p);
	...
	public file_buffer open_spellbuf() {
		buf_T buf = new buf_T();
		buf = (buf_T)ModernizedCProgram.alloc_clear(/*Error: Unsupported expression*/);
		if (buf != ((Object)0)) {
			buf.setB_spell(1);
	...
	public int utf_class_buf(int c) {
		...
		int bot = 0;
		int top = /*Error: sizeof expression not supported yet*/ / /*Error: Unsupported expression*/ - 1;
		int mid;
		...
		if (ModernizedCProgram.emoji_all.intable(/*Error: sizeof expression not supported yet*/, /* emoji */c)) {
			return 3;
		} 
	...
	public void mf_close_file(int getlines) {
		...
		if (/*Error: Function owner not recognized*/close(generatedMf_fd) < /* close the file */0) {
	...
	public void u_write_undo(Object name, int forceit, Object hash) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset((bi), (false), (/*Error: sizeof expression not supported yet*/));
		Object[] generatedB_ffname = this.getB_ffname();
		if (name == ((Object)0)) {
			...
			if (name == ((Object)0) || !forceit) {
				...
				if (fd < 0) {
					...
					;
						...
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/close(fd);
						if (len < 9 || /*Error: Function owner not recognized*/memcmp(mbuf, "Vim\237UnDo\345", 9) != 0) {
							if (name != ((Object)0) || ModernizedCProgram.p_verbose > 0) {
								if (name == ((Object)0)) {
									ModernizedCProgram.verbose_enter();
								} 
		...
		/*
		     ...
		     */
		/* sequent-ptx lacks fchown() *//* sequent-ptx lacks fchown() */fp = /*Error: Function owner not recognized*/fdopen(fd, "w");
		if (fp == ((Object)0)) {
			ModernizedCProgram.semsg(((byte)(ModernizedCProgram.e_not_open)), file_name);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/close(fd);
			ModernizedCProgram.mch_remove(file_name);
	...
	public void ruby_buffer_free() {
		...
		if (generatedB_ruby_ref) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/rb_hash_aset(ModernizedCProgram.objtbl, /*Error: Function owner not recognized*/rb_obj_id(()generatedB_ruby_ref), Qnil);
			/*Error: Function owner not recognized*/RDATA(generatedB_ruby_ref).setData(((Object)0));
		} 
	}
	public Object buffer_new() {
		...
		if (generatedB_ruby_ref) {
			return ()generatedB_ruby_ref;
				 ...
				 obj = /*Error: Function owner not recognized*/Data_Wrap_Struct(ModernizedCProgram.cBuffer, 0, 0, buf);
	...
	public file_buffer get_buf(Object obj) {
		buf_T buf = new buf_T();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Data_Get_Struct(obj, , buf);
		if (buf == ((Object)0)) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/rb_raise(ModernizedCProgram.eDeletedBufferError, "attempt to refer to deleted buffer");
		} 
		return buf;
	}
	public Object get_buffer_line(Object n) {
		...
		if (n <= 0 || n > generatedMl_line_count) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/rb_raise(rb_eIndexError, "line number %ld out of range", (long)n);
		} 
		return ModernizedCProgram.vim_str2rb_enc_str((byte)buf.ml_get_buf(n, 0));
	}
	public Object set_buffer_line(Object n, Object str) {
		byte line = /*Error: Function owner not recognized*/STR2CSTR(str);
		aco_save_T aco = new aco_save_T();
		memline generatedB_ml = this.getB_ml();
		Object generatedMl_line_count = generatedB_ml.getMl_line_count();
	...
	public int apply_autocmds_retval(auto_event event, Object fname, Object fname_io, int force, Integer retval) {
	...
	public void buf_hashtab_add() {
		...
		int generatedB_fnum = this.getB_fnum();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/sprintf((byte)generatedB_key, "%x", generatedB_fnum);
	...
	public void init_changedtick() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcpy((byte)(generatedDi_key), (byte)("changedtick"));
		Object generatedB_vars = this.getB_vars();
	...
	public file_buffer buflist_new(Object ffname_arg, Object sfname_arg, Object lnum, int flags) {
		...
		buf.clear_wininfo();
		buf.setB_wininfo((wininfo_T)ModernizedCProgram.alloc_clear(/*Error: Unsupported expression*/));
	...
	public file_buffer buflist_findnr(int nr) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/sprintf((byte)key, "%x", nr);
		hashitem_S hashitem_S = new hashitem_S();
	...
	public int buf_contents_changed() {
		...
		if (curbuf.ml_open() == 1 && ea.readfile(generatedB_ffname, generatedB_fname, (linenr_T)0, (linenr_T)0, (linenr_T)LONG_MAX, -1024 | -1024) == 1) {
			if (generatedMl_line_count == generatedMl_line_count) {
				differ = 0;
				for (lnum = 1; lnum <= generatedMl_line_count; ++lnum) {
					if (/*Error: Function owner not recognized*/strcmp((byte)(buf.ml_get_buf(lnum, 0)), (byte)(ModernizedCProgram.ml_get(lnum))) != 0) {
						differ = 1;
						break;
					} 
				}
	...
	public file_buffer buflist_find_by_name(Object name, int curtab_only) {
		...
		file_buffer file_buffer = new file_buffer();
		buf = file_buffer.buflist_findnr(ModernizedCProgram.buflist_findpat(name, name + /*Error: Function owner not recognized*/strlen((byte)(name)), 1, 0, curtab_only));
	...
	public file_buffer find_buffer( avar) {
		...
		if (generatedV_type == .VAR_NUMBER) {
			...
			if (buf == ((Object)0)) {
				for (buf = firstbuf; buf != ((Object)0); buf = generatedB_next) {
					if (generatedB_fname != ((Object)0) && (ModernizedCProgram.path_with_url(generatedB_fname) || buf.bt_nofilename()) && /*Error: Function owner not recognized*/strcmp((byte)(generatedB_fname), (byte)(generatedVval.getV_string())) == 0) {
						break;
					} 
				}
			} 
	...
	public void dialog_changed(int checkall) {
		...
		/*Error: Function owner not recognized*/// function./*Error: Function owner not recognized*/// function.memset((ea), (false), (/*Error: sizeof expression not supported yet*/));
		Object[] generatedB_ffname = this.getB_ffname();
	...
	public  getmark_buf_fnum(int c, int changefile, Integer fnum) {
	...
	public void crypt_append_msg() {
		...
		if (buf.crypt_get_method_nr() == 0) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[crypted]"))));
		} else {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(ModernizedCProgram.IObuff), (byte)("["));
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(ModernizedCProgram.IObuff), (byte)(generatedB_p_cm == (byte)'\000' ? p_cm : generatedB_p_cm));
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(ModernizedCProgram.IObuff), (byte)("]"/* FEAT_CRYPT */));
		} 
	}
	/*
