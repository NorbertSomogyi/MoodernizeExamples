...
public class VTerm {
	...
	public void do_control(byte control) {
		if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getControl()) {
			if (.UNRECOGNIZEDFUNCTIONNAME(control, ModernizedCProgram.vt.getParser().getCbdata())) {
				return ;
			} 
		} 
		;
	...
	public void do_csi(byte command) {
		if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getCsi()) {
			if (.UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.vt.getParser().getCsi_leaderlen() ? ModernizedCProgram.vt.getParser().getCsi_leader() : ((Object)0), ModernizedCProgram.vt.getParser().getCsi_args(), ModernizedCProgram.vt.getParser().getCsi_argi(), ModernizedCProgram.vt.getParser().getIntermedlen() ? ModernizedCProgram.vt.getParser().getIntermed() : ((Object)0), command, ModernizedCProgram.vt.getParser().getCbdata())) {
				return ;
			} 
		} 
		;
	...
	public void do_escape(byte command) {
		...
		.strncpy(seq, ModernizedCProgram.vt.getParser().getIntermed(), len);
		seq[len++] = command;
		seq[len] = 0;
		if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getEscape()) {
			if (.UNRECOGNIZEDFUNCTIONNAME(seq, len, ModernizedCProgram.vt.getParser().getCbdata())) {
				return ;
			} 
		} 
		;
	...
	public void append_strbuffer(Object str, Object len) {
		...
		if (len > 0) {
			.strncpy(ModernizedCProgram.vt.getParser().getStrbuffer() + ModernizedCProgram.vt.getParser().getStrbuffer_cur(), str, len);
	...
	public void done_string(Object str, Object len) {
		...
		case .VTERM_PARSER_DCS:
				if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getDcs()) {
					if (.UNRECOGNIZEDFUNCTIONNAME(str, len, ModernizedCProgram.vt.getParser().getCbdata())) {
						return ;
					} 
				} 
				;
					...
					if (c >= -1024 && c < -1024 && !ModernizedCProgram.vt.getMode().getUtf8()) {
						...
						default:
							...
							if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getText()) {
								eaten = .UNRECOGNIZEDFUNCTIONNAME(bytes + pos, len - pos, ModernizedCProgram.vt.getParser().getCbdata());
							} 
							if (!eaten) {
								;
								eaten = 1;
	...
	public VTerm vterm_new_with_allocator(int rows, int cols,  funcs, Object allocdata) {
		// Need to bootstrap using the allocator function directlyVTerm vt = .UNRECOGNIZEDFUNCTIONNAME(, allocdata);
		if (vt == ((Object)0)) {
			return ((Object)0);
		} 
	...
	public Object vterm_allocator_malloc(Object size) {
		return .UNRECOGNIZEDFUNCTIONNAME(size, ModernizedCProgram.vt.getAllocdata());
	}
	public void vterm_allocator_free(Object ptr) {
		if (ptr) {
			.UNRECOGNIZEDFUNCTIONNAME(ptr, ModernizedCProgram.vt.getAllocdata());
		} 
	}
	public void vterm_set_size(int rows, int cols) {
		...
		if (ModernizedCProgram.vt.getParser().getCallbacks() && ModernizedCProgram.vt.getParser().getCallbacks().getResize()) {
			.UNRECOGNIZEDFUNCTIONNAME(rows, cols, ModernizedCProgram.vt.getParser().getCbdata());
		} 
	}
	...
	public void vterm_push_output_bytes(Object bytes, Object len) {
		...
		.memcpy(ModernizedCProgram.vt.getOutbuffer() + ModernizedCProgram.vt.getOutbuffer_cur(), bytes, len);
	...
	public void vterm_push_output_vsprintf(Object format, Object args) {
		...
		// output was truncated// output was truncatedwritten = .vsprintf(buffer, format, args);
		...
		if (written > 0) {
			.strncpy(ModernizedCProgram.vt.getOutbuffer() + ModernizedCProgram.vt.getOutbuffer_cur(), buffer, written + 1);
	...
	public Object vterm_output_read(Byte buffer, Object len) {
		...
		.memcpy(buffer, ModernizedCProgram.vt.getOutbuffer(), len);
		if (len < ModernizedCProgram.vt.getOutbuffer_cur()) {
			.memmove(ModernizedCProgram.vt.getOutbuffer(), ModernizedCProgram.vt.getOutbuffer() + len, ModernizedCProgram.vt.getOutbuffer_cur() - len);
		} 
		ModernizedCProgram.vt.getOutbuffer_cur() -= len;
		return len;
	}
