...
public class slang_S {
		...
		while (depth >= 0) {
			if (fword[fwordidx[depth]] == (byte)'\000') {
				...
				--depth;
					...
					if (has_mbyte) {
						flen = (enc_utf8 ? ModernizedCProgram.utf_ptr2len(fword + fwordidx[depth]) : /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(fword + fwordidx[depth]));
						ulen = (enc_utf8 ? ModernizedCProgram.utf_ptr2len(uword + uwordidx[depth]) : /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(uword + uwordidx[depth]));
					} else {
							ulen = flen = 1;
					} 
					if (round[depth] == 1) {
					...
					if (l == 0) {
						if (round[depth] == 1) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strncpy((byte)(kword + kwordlen[depth]), (byte)(fword + fwordidx[depth]), (size_t)(flen));
							kwordlen[depth + 1] = kwordlen[depth] + flen;
						} else {
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strncpy((byte)(kword + kwordlen[depth]), (byte)(uword + uwordidx[depth]), (size_t)(ulen));
	...
	public int similar_chars(int c1, int c2) {
		...
		if (c1 >= 256) {
			buf[/*Error: Function owner not recognized*/mb_char2bytes(c1, buf)] = 0;
			hi = hashitem_S.hash_find(generatedSl_map_hash, buf);
			if ((generatedHi_key == ((Object)0) || generatedHi_key == ModernizedCProgram.hash_removed)) {
				m1 = 0;
			} else {
					m1 = /*Error: Function owner not recognized*/mb_ptr2char(generatedHi_key + /*Error: Function owner not recognized*/strlen((byte)(generatedHi_key)) + 1);
			} 
		} else {
		...
		if (c2 >= 256) {
			buf[/*Error: Function owner not recognized*/mb_char2bytes(c2, buf)] = 0;
			hi = hashitem_S.hash_find(generatedSl_map_hash, buf);
			if ((generatedHi_key == ((Object)0) || generatedHi_key == ModernizedCProgram.hash_removed)) {
				m2 = 0;
			} else {
					m2 = /*Error: Function owner not recognized*/mb_ptr2char(generatedHi_key + /*Error: Function owner not recognized*/strlen((byte)(generatedHi_key)) + 1);
			} 
		} else {
	...
	public int spell_edit_score(Object[] badword, Object[] goodword) {
		...
		if (has_mbyte) {
			for (; p != (byte)'\000'; /*Error: Unsupported expression*/) {
			...
			wgoodword[goodlen++] = 0;
				...
				badlen = (int)/*Error: Function owner not recognized*/strlen((byte)(badword)) + 1;
				goodlen = (int)/*Error: Function owner not recognized*/strlen((byte)(goodword)) + 1;
		} 
		// Get the characters from the multi-byte strings and put them in an
		// We use "cnt" as an array: CNT(badword_idx, goodword_idx).// We use "cnt" as an array: CNT(badword_idx, goodword_idx).cnt = (int)ModernizedCProgram.alloc(/*Error: Unsupported expression*/ * ((badlen + 1) * (goodlen + 1)));
		if (cnt == ((Object)0)) {
			return 0;
		} 
	...
	public int spell_edit_score_limit_w(Object badword, Object goodword, int limit) {
		...
		for (p = badword; p != (byte)'\000'; /*Error: Unsupported expression*/) {
			wbadword[bi++] = ModernizedCProgram.mb_cptr2char_adv(p);
		}
		wbadword[bi++] = 0;
		gi = 0;
		for (p = goodword; p != (byte)'\000'; /*Error: Unsupported expression*/) {
	...
	public int can_compound(Object word, Object[] flags) {
		...
		if (generatedSl_compsylmax < 254 && slang.count_syllables(word) > generatedSl_compsylmax) {
			return (int)/*Error: Function owner not recognized*/strlen((byte)(flags)) < generatedSl_compmax;
		} 
		return 1/*
	...
	public int match_compoundrule(Object[] compflags) {
		...
		for (p = generatedSl_comprules; p != (byte)'\000'; ++/* loop over all the COMPOUNDRULE entries */p) {
			for (i = 0; /*Error: Unsupported expression*/; ++/* loop over the flags in the compound word we have made, match
	...
	public slang_S slang_alloc(Object lang) {
		...
		lp = (slang_T)ModernizedCProgram.alloc_clear(/*Error: Unsupported expression*/);
		...
		if (lp != ((Object)0)) {
			...
			generatedSl_rep.ga_init2(/*Error: Unsupported expression*/, 10);
			generatedSl_repsal.ga_init2(/*Error: Unsupported expression*/, 10);
	...
	public void count_common_word(Object word, int len, int count) {
		...
		Object generatedWc_count = wc.getWc_count();
		if ((generatedHi_key == ((Object)0) || generatedHi_key == ModernizedCProgram.hash_removed)) {
			wc = ModernizedCProgram.alloc(/*Error: Unsupported expression*/ + /*Error: Function owner not recognized*/strlen((byte)(p)));
			if (wc == ((Object)0)) {
				return /*Error: Unsupported expression*/;
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcpy((byte)(generatedWc_word), (byte)(p));
	...
	public int init_syl_tab() {
		...
		Object generatedSl_syl_items = this.getSl_syl_items();
		generatedSl_syl_items.ga_init2(/*Error: Unsupported expression*/, 4);
		Object generatedSl_syllable = this.getSl_syllable();
		...
		while (p != ((Object)0)) {
			...
			if (p == ((Object)0)) {
				l = (int)/*Error: Function owner not recognized*/strlen((byte)(s));
			} else {
					l = (int)(p - s);
			} 
			if (l >= 30) {
	...
	public int count_syllables(Object word) {
		...
		for (p = word; p != (byte)'\000'; p += len) {
			...
			for (i = 0; i < generatedSl_syl_items.getGa_len(); ++i) {
				...
				if (generatedSy_len > len && /*Error: Function owner not recognized*/strncmp((byte)(p), (byte)(generatedSy_chars), (size_t)(generatedSy_len)) == 0) {
					len = generatedSy_len;
				} 
			}
			if (len != /* found a match, count syllable  */0) {
				...
				skip = 0;
					...
					c = /*Error: Function owner not recognized*/mb_ptr2char(/* No recognized syllable item, at least a syllable char then? */p);
			...
			slang.spell_soundfold_sofo(inword, /* SOFOFROM and SOFOTO used */res);
				...
				if (/* SAL items used.  Requires the word to be case-folded. */folded) {
					word = inword;
						...
						(Object)ModernizedCProgram.spell_casefold(inword, (int)/*Error: Function owner not recognized*/strlen((byte)(inword)), fword, 254);
						word = fword;
				} 
				if (has_mbyte) {
	...
	public void spell_soundfold_sofo(Object inword, Object[] res) {
		...
		if (has_mbyte) {
			...
			for (s = inword; s != /* The sl_sal_first[] table contains the translation for chars up to
				 * 255, sl_sal the rest. */(byte)'\000'; /*Error: Unsupported expression*/) {
				...
				if (c != (byte)'\000' && c != prevc) {
					ri += /*Error: Function owner not recognized*/mb_char2bytes(c, res + ri);
					if (ri + 21 > 254) {
						break;
					} 
					prevc = c;
				...
				for (; (s = smp[n].getSm_lead())[0] == c; ++/* check all rules for the same letter */n) {
					...
					if (s == (byte)'\000' || (s == (byte)'^' && (i == 0 || !(word[i - 1] == (byte)' ' || ModernizedCProgram.curwin.spell_iswordp(word + i - 1))) && ((s + 1) != (byte)'$' || (!ModernizedCProgram.curwin.spell_iswordp(word + i + k0)))) || (s == (byte)'$' && i > 0 && ModernizedCProgram.curwin.spell_iswordp(word + i - 1) && (!ModernizedCProgram.curwin.spell_iswordp(word + i + k0)))) {
						...
						if (p0 == 1 && z == 0) {
							...
							if (k > k0) {
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((word + i + k0)), (byte)((word + i + k)), (size_t)(/*Error: Function owner not recognized*/strlen((byte)(word + i + k)) + 1));
							} 
							c = word[/* new "actual letter" */i];
								...
								if (/*Error: Function owner not recognized*/strstr((byte)pf, "^^") != ((Object)0)) {
									if (c != (byte)'\000') {
										res[reslen++] = c;
									} 
									/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)((word)), (byte)((word + i + 1)), (size_t)(/*Error: Function owner not recognized*/strlen((byte)(word + i + 1)) + 1));
									i = 0;
									z0 = 1;
								} 
						} 
	...
	public void spell_soundfold_wsal(Object inword, Object[] res) {
		...
		wordlen = 0;
		int generatedSl_rem_accents = this.getSl_rem_accents();
		for (s = inword; s != (byte)'\000'; /*Error: Unsupported expression*/) {
			t = s;
			c = ModernizedCProgram.mb_cptr2char_adv(s);
			if (generatedSl_rem_accents) {
				if (enc_utf8 ? ModernizedCProgram.utf_class(c) == 0 : ((c) == (byte)' ' || (c) == (byte)'\t')) {
				...
				for (; ((ws = smp[n].getSm_lead_w())[0] & -1024) == (c & -1024) && ws[0] != (byte)'\000'; ++n) {
					...
					if (s == (byte)'\000' || (s == (byte)'^' && (i == 0 || !(word[i - 1] == (byte)' ' || ModernizedCProgram.curwin.spell_iswordp_w(word + i - 1))) && ((s + 1) != (byte)'$' || (!ModernizedCProgram.curwin.spell_iswordp_w(word + i + k0)))) || (s == (byte)'$' && i > 0 && ModernizedCProgram.curwin.spell_iswordp_w(word + i - 1) && (!ModernizedCProgram.curwin.spell_iswordp_w(word + i + k0)))) {
						...
						if (p0 == 1 && z == 0) {
							...
							if (k > k0) {
								/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(word + i + k0), (byte)(word + i + k), (size_t)(/*Error: Unsupported expression*/ * (wordlen - (i + k) + 1)));
							} 
							c = word[/* new "actual letter" */i];
						} else {
								i += k - /* no '<' rule used */1;
								...
								if (/*Error: Function owner not recognized*/strstr((byte)s, "^^") != ((Object)0)) {
									if (c != (byte)'\000') {
										wres[reslen++] = c;
									} 
									/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(word), (byte)(word + i + 1), (size_t)(/*Error: Unsupported expression*/ * (wordlen - (i + 1) + 1)));
									i = 0;
									z0 = 1;
								} 
						} 
		...
		for (n = 0; n < reslen; ++n) {
			l += /*Error: Function owner not recognized*/mb_char2bytes(wres[n], res + l);
			if (l + 21 > 254) {
				break;
			} 
		}
	 ...
	 */
	public void dump_word(Object word, Object pat, Integer dir, int dumpflags, int wordflags, Object lnum) {
		int keepcap = 0;
		char_u p = new char_u();
		...
		if (pat == ((Object)0)) {
			if ((flags & (-1024 | -1024 | -1024)) || /* Add flags and regions after a slash. */keepcap) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcpy((byte)(badword), (byte)(p));
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(badword), (byte)("/"));
				if (keepcap) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/strcat((byte)(badword), (byte)("="));
				} 
				if (flags & -1024) {
				...
				if (flags & -1024) {
					for (i = 0; i < 7; ++i) {
						if (flags & (-1024 << i)) {
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/sprintf((byte)badword + /*Error: Function owner not recognized*/strlen((byte)(badword)), "%d", i + 1);
						} 
					}
				} 
				p = badword;
		...
		}  else if (((dumpflags & 4) ? ModernizedCProgram.mb_strnicmp((char_u)(p), (char_u)(pat), (int)(/*Error: Function owner not recognized*/strlen((byte)(pat)))) == 0 : /*Error: Function owner not recognized*/strncmp((byte)(p), (byte)(pat), (size_t)(/*Error: Function owner not recognized*/strlen((byte)(pat)))) == 0) && ModernizedCProgram.ins_compl_add_infercase(p, (int)/*Error: Function owner not recognized*/strlen((byte)(p)), ModernizedCProgram.p_ic, ((Object)0), dir, 0) == 1) {
	...
	public Object dump_prefixes(Object word, Object pat, Integer dir, int dumpflags, int flags, Object startlnum) {
		...
		int len;
		int i;
		c = (has_mbyte ? /*Error: Function owner not recognized*/mb_ptr2char(word) : (int)(/* If the word starts with a lower-case letter make the word with an
	...
	public slang_S spell_load_file(Object fname, Object lang, int silent) {
		...
		if (old_lp == ((Object)0)) {
			...
			lp.setSl_add(/*Error: Function owner not recognized*/strstr((byte)ModernizedCProgram.gettail(fname), ".add.") != ((Object)/* Check for .add.spl (_add.spl for VMS). */0));
		} else {
				lp = old_lp;
		} 
		...
		sourcing_lnum = 0/*
		     ...
		     */;
		for (i = 0; i < 8; ++i) {
			buf[i] = /*Error: Function owner not recognized*/getc(/* <fileID> */fd);
		}
		if (/*Error: Function owner not recognized*/strncmp((byte)(buf), (byte)("VIMspell"), (size_t)(true)) != 0) {
			ModernizedCProgram.emsg(((byte)("E757: This does not look like a spell file")));
			;
		} 
		c = /*Error: Function owner not recognized*/getc(/* <versionnr> */fd);
		if (c < 50) {
	...
	public int set_sofo(Object[] from, Object[] to) {
		...
		if (has_mbyte/* Use "sl_sal" as an array with 256 pointers to a list of wide
			...
			gap = generatedSl_sal;
			gap.ga_init2(/*Error: Unsupported expression*/, 1);
			if (gap.ga_grow(256) == 0) {
				return -3;
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset((generatedGa_data), (false), (/*Error: Unsupported expression*/ * 256));
			gap.setGa_len(256);
			for (; p != (byte)'\000' && s != /* First count the number of items for each list.  Temporarily use
				 * sl_sal_first[] for this. */(byte)'\000'; /*Error: Unsupported expression*/) {
				c = ModernizedCProgram.mb_cptr2char_adv(p);
				s += enc_utf8 ? ModernizedCProgram.utf_ptr2len(s) : /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(s);
				if (c >= 256) {
			...
			for (i = 0; i < 256; ++/* Allocate the lists. */i) {
				if (generatedSl_sal_first[i] > 0) {
					p = ModernizedCProgram.alloc(/*Error: Unsupported expression*/ * (generatedSl_sal_first[i] * 2 + 1));
					if (p == ((Object)0)) {
						return -3;
					} 
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset((generatedSl_sal_first), (false), (/*Error: Unsupported expression*/ * /* Put the characters up to 255 in sl_sal_first[] the rest in a sl_sal
				 * list. */256));
			for (; p != (byte)'\000' && s != (byte)'\000'; /*Error: Unsupported expression*/) {
				c = ModernizedCProgram.mb_cptr2char_adv(p);
				...
				if (/*Error: Function owner not recognized*/strlen((byte)(from)) != /*Error: Function owner not recognized*/strlen((byte)(/* mapping bytes to bytes is done in sl_sal_first[] */to))) {
					return -2;
				} 
				...
				if (has_mbyte) {
					...
					for (n = 1; i + n < generatedGa_len; ++n) {
						if ((smp[i + n].getSm_lead_w() & -1024) == c) {
							...
							--n;
							tsal = smp[i + n];
							/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memmove((byte)(smp + i + 1), (byte)(smp + i), (size_t)(/*Error: Unsupported expression*/ * n));
							smp[i] = tsal;
						} 
					}
				} 
	...
	public void set_map_str(Object map) {
		...
		hashitem_S hashitem_S = new hashitem_S();
		Object generatedHi_key = (hi).getHi_key();
		for (p = map; p != (byte)'\000'; /*Error: Unsupported expression*/) {
			c = ModernizedCProgram.mb_cptr2char_adv(p);
			if (c == (byte)'/') {
				headc = 0;
			} else {
					...
					if (c >= 256) {
						int cl = /*Error: Function owner not recognized*/mb_char2len(c);
						int headcl = /*Error: Function owner not recognized*/mb_char2len(headc);
						char_u b = new char_u();
						...
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/mb_char2bytes(c, b);
						b[cl] = (byte)'\000';
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/mb_char2bytes(headc, b + cl + 1);
