...
public class exarg {
	...
	public void ex_jumps() {
		...
		for (i = 0; i < ModernizedCProgram.curwin.getW_jumplistlen() && !got_int; ++i) {
			if (ModernizedCProgram.curwin.getW_jumplist()[i].getFmark().getMark().getLnum() != 0) {
				...
				.sprintf((byte)ModernizedCProgram.IObuff, "%c %2d %5ld %4d ", i == ModernizedCProgram.curwin.getW_jumplistidx() ? (byte)'>' : (byte)' ', i > ModernizedCProgram.curwin.getW_jumplistidx() ? i - ModernizedCProgram.curwin.getW_jumplistidx() : ModernizedCProgram.curwin.getW_jumplistidx() - i, ModernizedCProgram.curwin.getW_jumplist()[i].getFmark().getMark().getLnum(), ModernizedCProgram.curwin.getW_jumplist()[i].getFmark().getMark().getCol());
	...
	public void ex_changes() {
		...
		for (i = 0; i < generatedB_changelistlen && !got_int; ++i) {
			if (generatedB_changelist[i].getLnum() != 0) {
				...
				.sprintf((byte)ModernizedCProgram.IObuff, "%c %3d %5ld %4d ", i == ModernizedCProgram.curwin.getW_changelistidx() ? (byte)'>' : (byte)' ', i > ModernizedCProgram.curwin.getW_changelistidx() ? i - ModernizedCProgram.curwin.getW_changelistidx() : ModernizedCProgram.curwin.getW_changelistidx() - i, (long)generatedB_changelist[i].getLnum(), generatedB_changelist[i].getCol());
				ModernizedCProgram.msg_outtrans(ModernizedCProgram.IObuff);
	...
	public void ex_mkspell() {
		...
		char_u arg = generatedArg;
		int ascii = 0;
		if (.strncmp((byte)(arg), (byte)("-ascii"), (size_t)(true)) == 0) {
			ascii = 1;
	...
	public void ex_spell() {
		...
		ModernizedCProgram.spell_add_word(generatedArg, (int).strlen((byte)(generatedArg)), generatedCmdidx == CMD_index.CMD_spellwrong ? 1 : generatedCmdidx == CMD_index.CMD_spellrare ? 2 : 0, generatedForceit ? 0 : (int)generatedLine2, generatedCmdidx == CMD_index.CMD_spellundo/*
	...
	public void ex_profile() {
		...
		if (len == 5 && .strncmp((byte)(generatedArg), (byte)("start"), (size_t)(true)) == 0 && e != (byte)'\000') {
		...
		}  else if (.strcmp((byte)(generatedArg), (byte)("pause")) == 0) {
			if (do_profiling == 1) {
				ModernizedCProgram.profile_start(ModernizedCProgram.pause_time);
			} 
			do_profiling = 2;
		}  else if (.strcmp((byte)(generatedArg), (byte)("continue")) == 0) {
			if (do_profiling == 2) {
					...
					if (ptr[col] == (byte)' ') {
						...
						if (got_tab || (generatedForceit && num_spaces > 1/* Retabulate this string of white-space */)) {
							...
							if (generatedB_p_et || got_tab || (num_spaces + num_tabs < len)) {
								...
								old_len = (long).strlen((byte)(ptr));
								new_line = ModernizedCProgram.alloc(old_len - col + start_col + len + 1);
								if (new_line == ((Object)0)) {
									break;
								} 
								if (start_col > 0) {
									.memmove((byte)(new_line), (byte)(ptr), (size_t)((size_t)start_col));
								} 
								.memmove((byte)(new_line + start_col + len), (byte)(ptr + col), (size_t)((size_t)(old_len - col + 1)));
	...
	public void ex_ruby() {
		...
		if (!generatedSkip && ModernizedCProgram.ensure_ruby_initialized()) {
			if (script == ((Object)0)) {
				.rb_eval_string_protect((byte)generatedArg, state);
			} else {
	...
	public void ex_rubydo() {
		...
		if (ModernizedCProgram.ensure_ruby_initialized()) {
			...
			for (i = generatedLine1; i <= generatedLine2; i++) {
				...
				.rb_lastline_set(ModernizedCProgram.line);
				...
				ModernizedCProgram.line = .rb_lastline_get();
				if (!.NIL_P(ModernizedCProgram.line)) {
					if (.TYPE(ModernizedCProgram.line) != T_STRING) {
						ModernizedCProgram.emsg(((byte)("E265: $_ must be an instance of String")));
						return ;
					} 
					ModernizedCProgram.ml_replace(i, (char_u).STR2CSTR(ModernizedCProgram.line), 1);
	...
	public void ex_rubyfile() {
		...
		Object generatedArg = this.getArg();
		if (ModernizedCProgram.ensure_ruby_initialized()) {
			 file_to_load = .rb_str_new2((byte)generatedArg);
	...
	public int parse_command_modifiers(byte errormsg, int skip_only) {
		char_u p = new char_u();
		.memset((ModernizedCProgram.cmdmod), (false), ());
		...
		// Repeat until no more command modifiers are found.for (; ; ) {
			...
			case (byte)'v':
					...
					if (!skip_only) {
						...
						if (ModernizedCProgram.vim_isdigit(generatedCmd)) {
							ModernizedCProgram.p_verbose = .atoi((byte)generatedCmd);
						} else {
	...
	public int parse_cmd_address(byte errormsg, int silent) {
		...
		// Repeat for all ',' or ';' separated addresses.for (; ; ) {
	...
	public Object find_command(int full) {
		int len;
		char_u p = new char_u();
		int i;
		/*
		...
		CMD_index generatedCmdidx = this.getCmdidx();
		if (p == (byte)'k') {
			this.setCmdidx(CMD_index.CMD_k);
			++p;
		}  else if (p[0] == (byte)'s' && ((p[1] == (byte)'c' && (p[2] == (byte)'\000' || (p[2] != (byte)'s' && p[2] != (byte)'r' && (p[3] == (byte)'\000' || (p[3] != (byte)'i' && p[4] != (byte)'p'))))) || p[1] == (byte)'g' || (p[1] == (byte)'i' && p[2] != (byte)'m' && p[2] != (byte)'l' && p[2] != (byte)'g') || p[1] == (byte)'I' || (p[1] == (byte)'r' && p[2] != (byte)'e'))) {
			this.setCmdidx(CMD_index.CMD_substitute);
			++p;
		} else {
				while ((((int)(p) - (byte)'A' < 26) || ((int)(p) - (byte)'a' < 26))) {
					++p;
				}
				if (generatedCmd[0] == (byte)'p' && generatedCmd[1] == /* for python 3.x support ":py3", ":python3", ":py3file", etc. */(byte)'y') {
					while (((((int)(p) - (byte)'A' < 26) || ((int)(p) - (byte)'a' < 26)) || ((int)(p) - (byte)'0' < 10))) {
						++p;
					}
				} 
				if (p == generatedCmd && ModernizedCProgram.vim_strchr((char_u)"@*!=><&~#", p) != ((Object)/* check for non-alpha command */0)) {
					++p;
				} 
				len = (int)(p - generatedCmd);
				if (generatedCmd == (byte)'d' && (p[-1] == (byte)'l' || p[-1] == (byte)'p')) {
					for (i = 0; i < len; ++/* Check for ":dl", ":dell", etc. to ":deletel": that's
						     * :delete with the 'l' flag.  Same for 'p'. */i) {
						if (generatedCmd[i] != ((char_u)"delete")[i]) {
							break;
						} 
					}
					if (i == len - 1) {
				...
				if (((int)(generatedCmd[0]) - (byte)'a' < 26)) {
					int c1 = generatedCmd[0];
					int c2 = len == 1 ? (byte)'\000' : generatedCmd[1];
					if (ModernizedCProgram.command_count != (int)CMD_index.CMD_SIZE) {
						ModernizedCProgram.iemsg(((byte)("E943: Command table needs to be updated, run 'make cmdidxs'")));
						ModernizedCProgram.getout(1);
					} 
					this.setCmdidx(ModernizedCProgram.cmdidxs1[((c1) - /* Use a precomputed index for fast look-up in cmdnames[]
						     * taking into account the first 2 letters of eap->cmd. */(byte)'a')]);
					if (((int)(c2) - (byte)'a' < 26)) {
						generatedCmdidx += ModernizedCProgram.cmdidxs2[((c1) - (byte)'a')][((c2) - (byte)'a')];
					} 
				} else {
						this.setCmdidx(CMD_index.CMD_bang);
				} 
				for (; (int)generatedCmdidx < (int)CMD_index.CMD_SIZE; this.setCmdidx((cmdidx_T)((int)generatedCmdidx + 1))) {
					if (.strncmp((byte)(cmdnames[(int)generatedCmdidx].getCmd_name()), (byte)((byte)generatedCmd), (size_t)((size_t)len)) == 0) {
						if (full != ((Object)0) && cmdnames[(int)generatedCmdidx].getCmd_name()[len] == (byte)'\000') {
							full = 1;
						} 
						break;
					} 
				}
				if ((generatedCmdidx == CMD_index.CMD_SIZE || generatedCmdidx == CMD_index.CMD_Print) && generatedCmd >= (byte)'A' && generatedCmd <= (byte)'Z') {
					while (((((int)(p) - (byte)'A' < 26) || ((int)(p) - (byte)'a' < 26)) || ((int)(p) - (byte)'0' < 10))) {
						++p;
					}
					p = ModernizedCProgram.find_ucmd(eap, p, full, ((Object)0), ((Object)0));
				} 
				if (p == generatedCmd) {
					this.setCmdidx(CMD_index.CMD_SIZE);
				} 
		} 
		return p;
		int i;
		...
		int bot;
		int c;
		if (cmdchar >= /* A multi-byte character is never a command. */-1024) {
			return -1;
		} 
		if (cmdchar < /* We use the absolute value of the character.  Special keys have a
		     * negative value, but are sorted on their absolute value. */0) {
			cmdchar = -cmdchar;
		} 
		if (cmdchar <= /* If the character is in the first part: The character is the index into
		     * nv_cmd_idx[]. */ModernizedCProgram.nv_max_linear) {
			return ModernizedCProgram.nv_cmd_idx[cmdchar];
		} 
		bot = ModernizedCProgram.nv_max_linear + /* Perform a binary search. */1;
		top = ( / ) - 1;
		idx = -1;
		while (bot <= top) {
			i = (top + bot) / 2;
			c = nv_cmds[ModernizedCProgram.nv_cmd_idx[i]].getCmd_char();
			if (c < 0) {
				c = -c;
			} 
			if (cmdchar == c) {
				idx = ModernizedCProgram.nv_cmd_idx[i];
				break;
			} 
			if (cmdchar > c) {
				bot = i + 1;
	...
	public Object replace_makeprg(Object p, Object cmdlinep) {
		...
		if ((generatedCmdidx == CMD_index.CMD_make || generatedCmdidx == CMD_index.CMD_lmake || generatedCmdidx == CMD_index.CMD_grep || generatedCmdidx == CMD_index.CMD_lgrep || generatedCmdidx == CMD_index.CMD_grepadd || generatedCmdidx == CMD_index.CMD_lgrepadd) && !ModernizedCProgram.grep_internal(generatedCmdidx)) {
			...
			if ((pos = (char_u).strstr((byte)program, "$*")) != ((Object)0)) {
				i = /* replace $* by given arguments */1;
				while ((pos = (char_u).strstr((byte)pos + 2, "$*")) != ((Object)0)) {
					++i;
				}
				len = (int).strlen((byte)(p));
				new_cmdline = ModernizedCProgram.alloc(.strlen((byte)(program)) + i * (len - 2) + 1);
				if (new_cmdline == ((Object)0)) {
					return ((Object)/* out of memory */0);
				} 
				ptr = new_cmdline;
				while ((pos = (char_u).strstr((byte)program, "$*")) != ((Object)0)) {
					i = (int)(pos - program);
					.strncpy((byte)(ptr), (byte)(program), (size_t)(i));
					.strcpy((byte)(ptr += i), (byte)(p));
					ptr += len;
					program = pos + 2;
				}
				.strcpy((byte)(ptr), (byte)(program));
			} else {
					new_cmdline = ModernizedCProgram.alloc(.strlen((byte)(program)) + .strlen((byte)(p)) + 2);
					if (new_cmdline == ((Object)0)) {
						return ((Object)/* out of memory */0);
					} 
					.strcpy((byte)(new_cmdline), (byte)(program));
					.strcat((byte)(new_cmdline), (byte)(" "));
					.strcat((byte)(new_cmdline), (byte)(p));
			} 
		...
		if ((generatedArgt & -1024) && !generatedUsefilter) {
			for (n = 1; n <= 2; ++n) {
				...
				if (has_wildcards) {
					...
					if (p != ((Object)0)) {
						(Object)eap.repl_cmdline(generatedArg, (int).strlen((byte)(generatedArg)), p, cmdlinep);
	...
	public Object repl_cmdline(Object src, int srclen, Object repl, Object cmdlinep) {
		...
		/*
		     ...
		     */
		len = (int).strlen((byte)(repl));
		i = (int)(src - cmdlinep) + (int).strlen((byte)(src + srclen)) + len + 3;
		Object generatedNextcmd = this.getNextcmd();
		if (generatedNextcmd != ((Object)0)) {
			i += (int).strlen((byte)(generatedNextcmd));
		} 
		...
		.memmove((byte)(new_cmdline), (byte)(cmdlinep), (size_t)((size_t)i));
		.memmove((byte)(new_cmdline + i), (byte)(repl), (size_t)((size_t)len));
		i += /* remember the end of the string */len;
		.strcpy((byte)(new_cmdline + i), (byte)(src + srclen));
		src = new_cmdline + /* remember where to continue */i;
		if (generatedNextcmd != ((Object)/* append next command */0)) {
			i = (int).strlen((byte)(new_cmdline)) + 1;
			.strcpy((byte)(new_cmdline + i), (byte)(generatedNextcmd));
	...
	public void separate_nextcmd() {
		...
		CMD_index generatedCmdidx = this.getCmdidx();
		Object generatedArg = this.getArg();
		for (; p; p += .UNRECOGNIZEDFUNCTIONNAME(p)) {
			if (p == 22) {
				if (generatedArgt & (-1024 | -1024)) {
					++/* skip CTRL-V and next char */p;
				} else {
						.memmove((byte)((p)), (byte)((p + 1)), (size_t)(.strlen((byte)(p + 1)) + /* remove CTRL-V and skip next char */1));
				} 
				...
				if ((ModernizedCProgram.vim_strchr(ModernizedCProgram.p_cpo, (byte)'b') == ((Object)0) || !(generatedArgt & -1024)) && (p - 1) == (byte)'\\') {
					.memmove((byte)((p - 1)), (byte)((p)), (size_t)(.strlen((byte)(p)) + /* remove the '\' */1));
					--p;
				} else {
	...
	public int getargopt() {
		...
		int bad_char_idx;
		char_u p = new char_u();
		if (.strncmp((byte)(arg), (byte)("bin"), (size_t)(true)) == 0 || .strncmp((byte)(arg), (byte)("nobin"), (size_t)(true)) == /* ":edit ++[no]bin[ary] file" */0) {
			if (arg == (byte)'n') {
				arg += 2;
				this.setForce_bin(2);
			} else {
		...
		if (.strncmp((byte)(arg), (byte)("edit"), (size_t)(true)) == /* ":read ++edit file" */0) {
			this.setRead_edit(1);
		...
		if (.strncmp((byte)(arg), (byte)("ff"), (size_t)(true)) == 0) {
			arg += 2;
			pp = generatedForce_ff;
		}  else if (.strncmp((byte)(arg), (byte)("fileformat"), (size_t)(true)) == 0) {
			arg += 10;
			pp = generatedForce_ff;
		}  else if (.strncmp((byte)(arg), (byte)("enc"), (size_t)(true)) == 0) {
			if (.strncmp((byte)(arg), (byte)("encoding"), (size_t)(true)) == 0) {
				arg += 8;
			} else {
					arg += 3;
			} 
			pp = generatedForce_enc;
		}  else if (.strncmp((byte)(arg), (byte)("bad"), (size_t)(true)) == 0) {
			arg += 3;
			pp = bad_char_idx;
		} 
	...
	public void ex_only() {
		...
		if (generatedAddr_count > 0) {
			wnr = generatedLine2;
			for (wp = ModernizedCProgram.firstwin; --wnr > 0; ) {
	...
	public void ex_stop() {
		...
		if (!ModernizedCProgram.check_restricted()) {
			...
			.windgoto((int)ModernizedCProgram.Rows - 1, 0);
	...
	public void ex_splitview() {
		...
		if (generatedCmdidx == CMD_index.CMD_sfind || generatedCmdidx == CMD_index.CMD_tabfind) {
			fname = ModernizedCProgram.find_file_in_path(generatedArg, (int).strlen((byte)(generatedArg)), 1, 1, generatedB_ffname);
			if (fname == ((Object)0)) {
				;
			} 
			this.setArg(fname);
	...
	public void ex_sleep() {
		...
		if (ModernizedCProgram.cursor_valid()) {
			...
			if (n >= 0) {
				.windgoto((int)n, ModernizedCProgram.curwin.getW_wincol() + ModernizedCProgram.curwin.getW_wcol());
			} 
		} 
		Object generatedLine2 = this.getLine2();
		len = generatedLine2;
	...
	public void ex_winpos() {
		...
		if (arg == (byte)'\000') {
			if (ModernizedCProgram.mch_get_winpos(x, y) != 0) {
				.sprintf((byte)ModernizedCProgram.IObuff, ((byte)("Window position: X %d, Y %d")), x, y);
					...
					if ((((int)(arg) - (byte)'A' < 26) || ((int)(arg) - (byte)'a' < 26)) || arg == (byte)'*' || arg == (byte)'+' || arg == (byte)'"') {
						...
						if (arg == (byte)'>' && arg[1] == /* append */(byte)'>') {
							arg += 2;
								...
								if (arg == (byte)'\000' && !.isupper(redir_reg)) {
	...
	public void ex_normal() {
		...
		if (has_mbyte) {
			...
			for (p = generatedArg; p != (byte)'\000'; ++/* Count the number of characters to be escaped. */p) {
				for (l = .UNRECOGNIZEDFUNCTIONNAME(p) - 1; l > 0; --/* leadbyte CSI */l) {
			...
			if (len > 0) {
				arg = ModernizedCProgram.alloc(.strlen((byte)(generatedArg)) + len + 1);
				if (arg != ((Object)0)) {
					len = 0;
					for (p = generatedArg; p != (byte)'\000'; ++p) {
						arg[len++] = p;
						for (l = .UNRECOGNIZEDFUNCTIONNAME(p) - 1; l > 0; --l) {
							arg[len++] = ++p;
							if (p == (true)) {
								arg[len++] = 254;
								arg[len++] = ((byte)'X');
	...
	public void ex_findpat() {
		...
		Object generatedLine2 = this.getLine2();
		if (!generatedSkip) {
			ModernizedCProgram.find_pattern_in_path(generatedArg, 0, (int).strlen((byte)(generatedArg)), whole, !generatedForceit, generatedCmd == (byte)'d' ? 2 : 1, n, action, generatedLine1, generatedLine2);
		} 
	}
	/*
	...
	public void ex_behave() {
		Object generatedArg = this.getArg();
		if (.strcmp((byte)(generatedArg), (byte)("mswin")) == 0) {
		...
		}  else if (.strcmp((byte)(generatedArg), (byte)("xterm")) == 0) {
	...
	public void ex_filetype() {
		...
		/* Accept "plugin" and "indent" in any order. */for (; ; ) {
			if (.strncmp((byte)(arg), (byte)("plugin"), (size_t)(true)) == 0) {
				plugin = 1;
				arg = ModernizedCProgram.skipwhite(arg + 6);
				continue;
			} 
			if (.strncmp((byte)(arg), (byte)("indent"), (size_t)(true)) == 0) {
				indent = 1;
				arg = ModernizedCProgram.skipwhite(arg + 6);
				continue;
			} 
		...
		if (.strcmp((byte)(arg), (byte)("on")) == 0 || .strcmp((byte)(arg), (byte)("detect")) == 0) {
		...
		}  else if (.strcmp((byte)(arg), (byte)("off")) == 0) {
			if (plugin || indent) {
				if (plugin) {
	...
	public void ex_setfiletype() {
		...
		if (!did_filetype) {
			char_u arg = generatedArg;
			if (.strncmp((byte)(arg), (byte)("FALLBACK "), (size_t)(true)) == 0) {
				arg += 9;
			} 
			...
			this.setErrmsg("E601: :try nesting too deep");
				...
				if (!skip/* Set ACTIVE and TRUE.  TRUE means that the corresponding ":catch"
					...
					if (emsg_silent) {
						...
						elem = (eslist_elem)ModernizedCProgram.alloc();
						if (elem == ((Object)0)) {
	...
	public void ex_spellrepall() {
		...
		addlen = (int)(.strlen((byte)(repl_to)) - .strlen((byte)(repl_from)));
		frompat = ModernizedCProgram.alloc(.strlen((byte)(repl_from)) + 7);
		if (frompat == ((Object)0)) {
			return ;
		} 
		.sprintf((byte)frompat, "\\V\\<%s\\>", repl_from);
		...
		while (!got_int) {
			...
			if (addlen <= 0 || .strncmp((byte)(line + ModernizedCProgram.curwin.getW_cursor().getCol()), (byte)(repl_to), (size_t)(.strlen((byte)(repl_to)))) != 0) {
				p = ModernizedCProgram.alloc(.strlen((byte)(line)) + addlen + 1);
				if (p == ((Object)0)) {
					break;
				} 
				.memmove((byte)(p), (byte)(line), (size_t)(ModernizedCProgram.curwin.getW_cursor().getCol()));
				.strcpy((byte)(p + ModernizedCProgram.curwin.getW_cursor().getCol()), (byte)(repl_to));
				.strcat((byte)(p), (byte)(line + ModernizedCProgram.curwin.getW_cursor().getCol() + .strlen((byte)(repl_from))));
			...
			ModernizedCProgram.curwin.getW_cursor().getCol() += (colnr_T).strlen((byte)(repl_to));
		}
	...
	public void ex_simalt() {
		...
		.PostMessageA(ModernizedCProgram.s_hwnd, 274, (WPARAM)-1024, (LPARAM)0);
		while (keys) {
			if (keys == (byte)'~') {
				keys = /* for showing system menu */(byte)' ';
			} 
			.PostMessageA(ModernizedCProgram.s_hwnd, 258, (WPARAM)keys, (LPARAM)0);
			keys++;
			fill_typebuf = 1;
		}
		if (fill_typebuf) {
		...
		if (ModernizedCProgram.vim_isdigit(generatedArg)) {
			...
			del_all = 1;
				...
				for (i = 0; i < generatedGa_len; ++i) {
					...
					if (generatedDbg_type == generatedDbg_type && .strcmp((byte)(generatedDbg_name), (byte)(generatedDbg_name)) == 0 && (generatedDbg_lnum == generatedDbg_lnum || (generatedDbg_lnum == 0 && (best_lnum == 0 || generatedDbg_lnum < best_lnum)))) {
						todel = i;
			...
			ModernizedCProgram.semsg(((byte)("E161: Breakpoint not found: %s")), generatedArg);
				...
				while (generatedGa_len > 0) {
					...
					if (todel < generatedGa_len) {
						.memmove((byte)((((debuggy)generatedGa_data)[todel])), (byte)((((debuggy)generatedGa_data)[todel + 1])), (size_t)((generatedGa_len - todel) * ));
					} 
	...
	public void ex_history() {
		...
		if (!(((int)(arg) - (byte)'0' < 10) || arg == (byte)'-' || arg == (byte)',')) {
			...
			if (histype1 == -1) {
				if (ModernizedCProgram.vim_strnicmp((byte)(arg), (byte)("all"), (size_t)(.strlen((byte)(arg)))) == 0) {
					histype1 = 0;
					histype2 = 5 - 1;
				} else {
						end = i;
		...
		for (; !got_int && histype1 <= histype2; ++histype1) {
			.strcpy((byte)(ModernizedCProgram.IObuff), (byte)("\n      #  "));
			.strcat((byte)(.strcat((byte)(ModernizedCProgram.IObuff), (byte)(ModernizedCProgram.history_names[histype1]))), (byte)(" history"));
			...
			if (idx >= 0 && j <= k) {
				for (i = idx + 1; !got_int; ++i) {
					...
					if (hist[i].getHisstr() != ((Object)0) && hist[i].getHisnum() >= j && hist[i].getHisnum() <= k) {
						ModernizedCProgram.msg_putchar((byte)'\n');
						.sprintf((byte)ModernizedCProgram.IObuff, "%c%6d  ", i == idx ? (byte)'>' : (byte)' ', hist[i].getHisnum());
						if (ModernizedCProgram.vim_strsize(hist[i].getHisstr()) > (int)Columns - 10) {
							ModernizedCProgram.trunc_string(hist[i].getHisstr(), ModernizedCProgram.IObuff + .strlen((byte)(ModernizedCProgram.IObuff)), (int)Columns - 10, (1024 + 1) - (int).strlen((byte)(ModernizedCProgram.IObuff)));
						} else {
								.strcat((byte)(ModernizedCProgram.IObuff), (byte)(hist[i].getHisstr()));
						} 
						ModernizedCProgram.msg_outtrans(ModernizedCProgram.IObuff);
						ModernizedCProgram.out_flush();
					} 
	...
	public void ex_args() {
		...
		if (generatedArg != (byte)'\000') {
			...
			if (((ModernizedCProgram.curwin).getW_alist().getAl_ga().getGa_len()) > 0) {
				char_u items = (char_u)ModernizedCProgram.alloc( * (((ModernizedCProgram.curwin).getW_alist().getAl_ga().getGa_len())));
				if (items != ((Object)0)) {
		...
		if (generatedAddr_count > 0) {
			...
			if (generatedArg != (byte)'\000') {
				...
				if (generatedLine1 != 1 || generatedLine2 != 0) {
					...
					.memmove((byte)(((aentry_T)(ModernizedCProgram.curwin).getW_alist().getAl_ga().getGa_data()) + generatedLine1 - 1), (byte)(((aentry_T)(ModernizedCProgram.curwin).getW_alist().getAl_ga().getGa_data()) + generatedLine2), (size_t)((size_t)((((ModernizedCProgram.curwin).getW_alist().getAl_ga().getGa_len()) - generatedLine2) * )));
					...
					for (; p != (byte)'\000' && !got_int; ++p) {
						if (p == (byte)'\n' || p == (byte)'\r' || p == (byte)'\011') {
							...
							ModernizedCProgram.msg_putchar_attr(p, ModernizedCProgram.echo_attr);
								...
								if (has_mbyte) {
									int i = .UNRECOGNIZEDFUNCTIONNAME(p);
	...
	public void ex_execute() {
		...
		while (arg != (byte)'\000' && arg != (byte)'|' && arg != (byte)'\n') {
			...
			if (!generatedSkip) {
				...
				.strcpy((byte)((char_u)(generatedGa_data) + generatedGa_len), (byte)(p));
				generatedGa_len += len;
			} 
			rettv.clear_tv();
			arg = ModernizedCProgram.skipwhite(arg);
	...
	public void do_ascii() {
		...
		if (!has_mbyte || (enc_dbcs != 0 && c < -1024) || c < -1024) {
			...
			if (ModernizedCProgram.vim_isprintc_strict(c) && (c < (byte)' ' || c > (byte)'~')) {
				...
				ModernizedCProgram.vim_snprintf(buf1, , "  <%s>", (byte)buf3);
			} else {
					buf1[0] = (byte)'\000';
			} 
			if (c >= -1024) {
				ModernizedCProgram.vim_snprintf(buf2, , "  <M-%s>", (byte)ModernizedCProgram.transchar(c & -1024));
			} else {
					buf2[0] = (byte)'\000';
			} 
		...
		while (has_mbyte && (c >= -1024 || (enc_utf8 && c >= /* Repeat for combining characters. */-1024))) {
			len = (int).strlen((byte)(ModernizedCProgram.IObuff));
			...
			len += .UNRECOGNIZEDFUNCTIONNAME(c, ModernizedCProgram.IObuff + len);
	...
	public void ex_sort() {
		...
		nrs = (sorti_T)ModernizedCProgram.alloc( * (count));
		if (nrs == ((Object)0)) {
			;
		} 
					...
					if (s == (byte)'\000') {
						...
						if (s == (byte)'\000') {
							nrs[lnum - generatedLine1].getSt_u().setValue_flt(-((double)/* empty line should sort before any number */-1024.0));
								...
								nrs[lnum - generatedLine1].getSt_u().setValue_flt(.strtod((byte)s, ((Object)0)));
						} 
				} 
				s2 = c;
			} else {
		...
		.qsort((Object)nrs, count, , /* Sort the array of line numbers.  Note: can't be interrupted! */ModernizedCProgram.sort_compare);
		...
		for (i = 0; i < count; ++i) {
			...
			if (!unique || i == 0 || (ModernizedCProgram.sort_ic ? ModernizedCProgram.vim_stricmp((byte)(s), (byte)(ModernizedCProgram.sortbuf1)) : .strcmp((byte)(s), (byte)(ModernizedCProgram.sortbuf1))) != 0) {
				.strcpy((byte)(ModernizedCProgram.sortbuf1), (byte)(s));
	...
	public void do_bang(int addr_count, int forceit, int do_in, int do_out) {
		...
		trailarg = arg;
		do {
			len = (int).strlen((byte)(trailarg)) + 1;
			if (newcmd != ((Object)0)) {
				len += (int).strlen((byte)(newcmd));
			} 
			if (ins_prevcmd) {
				...
				len += (int).strlen((byte)(ModernizedCProgram.prevcmd));
			} 
			...
			t = (byte)'\000';
			if (newcmd != ((Object)0)) {
				.strcat((byte)(t), (byte)(newcmd));
			} 
			if (ins_prevcmd) {
				.strcat((byte)(t), (byte)(ModernizedCProgram.prevcmd));
			} 
			p = t + .strlen((byte)(t));
			.strcat((byte)(t), (byte)(trailarg));
			ModernizedCProgram.vim_free(newcmd);
			newcmd = t/*
			...
			while (p) {
				if (p == (byte)'!') {
					if (p > newcmd && p[-1] == (byte)'\\') {
						.memmove((byte)((p - 1)), (byte)((p)), (size_t)(.strlen((byte)(p)) + 1));
					} else {
							trailarg = p;
							trailarg++ = (byte)'\000';
							ins_prevcmd = 1;
		...
		if (ModernizedCProgram.p_shq != (byte)'\000') {
			newcmd = ModernizedCProgram.alloc(.strlen((byte)(ModernizedCProgram.prevcmd)) + 2 * .strlen((byte)(ModernizedCProgram.p_shq)) + 1);
			if (newcmd == ((Object)0)) {
				return ;
			} 
			.strcpy((byte)(newcmd), (byte)(ModernizedCProgram.p_shq));
			.strcat((byte)(newcmd), (byte)(ModernizedCProgram.prevcmd));
			.strcat((byte)(newcmd), (byte)(ModernizedCProgram.p_shq));
			free_newcmd = 1;
		} 
		if (addr_count == /* :! */0) {
			ModernizedCProgram.msg_start();
			...
			ModernizedCProgram.msg_clr_eos();
			.windgoto(ModernizedCProgram.msg_row, ModernizedCProgram.msg_col);
	...
	public void do_filter(Object line1, Object line2, Object cmd, int do_in, int do_out) {
		...
		if (!do_out || .strcmp((byte)(ModernizedCProgram.p_srr), (byte)(">")) == 0 || !do_in) {
			ModernizedCProgram.redraw_later_clear();
		} 
		if (do_out) {
	...
	public void ex_append() {
		...
		for (; ; ) {
			...
			if (generatedGetline == ((Object)0)) {
				...
				if (p == ((Object)0)) {
					p = generatedNextcmd + .strlen((byte)(generatedNextcmd));
				} 
				...
				this.setNextcmd(p);
					...
					theline = .UNRECOGNIZEDFUNCTIONNAME(generatedCstack.getCs_looplevel() > 0 ? -1 : (byte)'\000', generatedCookie, indent, 1);
					State = save_State;
			} 
	...
	public void do_sub() {
		...
		if (generatedCmd[0] == (byte)'s' && cmd != (byte)'\000' && !((cmd) == (byte)' ' || (cmd) == /* new pattern and substitution */(byte)'\t') && ModernizedCProgram.vim_strchr((char_u)"0123456789cegriIp|\"", cmd) == ((Object)0)) {
			if (.isalpha(/* don't accept alphanumeric for separator */cmd)) {
			...
			while (cmd[0]) {
				...
				cmd += .UNRECOGNIZEDFUNCTIONNAME(cmd);
			}
			if (!generatedSkip) {
				if (.strcmp((byte)(sub), (byte)("%")) == /* In POSIX vi ":s/pat/%/" uses the previous subst. string. */0 && ModernizedCProgram.vim_strchr(ModernizedCProgram.p_cpo, (byte)'/') != ((Object)0)) {
	...
	public void ex_help() {
		...
		p = arg + .strlen((byte)(arg)) - /* remove trailing blanks */1;
		...
		if (!ModernizedCProgram.curwin.getW_buffer().bt_help() || ModernizedCProgram.cmdmod.getTab() != /*
			...
			if (wp != ((Object)0) && generatedB_nwindows > 0) {
				wp.win_enter(1/*
					...
					.fclose(helpfd/* Split off help window; put it at far top if no position
	...
	public void ex_helptags() {
		...
		int add_help_tags = 0;
		Object generatedArg = this.getArg();
		if (.strncmp((byte)(generatedArg), (byte)("++t"), (size_t)(true)) == 0 && ((generatedArg[3]) == (byte)' ' || (generatedArg[3]) == /* Check for ":helptags ++t {dir}". */(byte)'\t')) {
			add_help_tags = 1;
			this.setArg(ModernizedCProgram.skipwhite(generatedArg + 3));
		} 
		if (.strcmp((byte)(generatedArg), (byte)("ALL")) == 0) {
	...
	public void ex_runtime() {
		...
		if (.strncmp((byte)(arg), (byte)("START"), (size_t)(len)) == 0) {
			flags += -1024 + -1024;
			arg = ModernizedCProgram.skipwhite(arg + len);
		}  else if (.strncmp((byte)(arg), (byte)("OPT"), (size_t)(len)) == 0) {
			flags += -1024 + -1024;
			arg = ModernizedCProgram.skipwhite(arg + len);
		}  else if (.strncmp((byte)(arg), (byte)("PACK"), (size_t)(len)) == 0) {
			flags += -1024 + -1024 + -1024;
			arg = ModernizedCProgram.skipwhite(arg + len);
		}  else if (.strncmp((byte)(arg), (byte)("ALL"), (size_t)(len)) == 0) {
			flags += -1024 + -1024;
			arg = ModernizedCProgram.skipwhite(arg + len);
		} 
	...
	public void ex_packadd() {
		...
		// Round 1: use "start", round 2: use "opt".for (round = 1; round <= 2; ++round) {
			...
			len = (int).strlen((byte)(plugpat)) + (int).strlen((byte)(generatedArg)) + 5;
	...
	public void ex_diffupdate() {
		...
		// Only use the internal method if it did not fail for one of the buffers.// Only use the internal method if it did not fail for one of the buffers..memset((diffio), (false), ());
	...
	public void ex_diffpatch() {
		...
		buflen = .strlen((byte)(tmp_orig)) + .strlen((byte)(esc_name)) + .strlen((byte)(tmp_new)) + 16;
		...
		.strcpy((byte)(buf), (byte)(/* Delete any .orig or .rej file created. */tmp_new));
		.strcat((byte)(buf), (byte)(".orig"));
		ModernizedCProgram.mch_remove(buf);
		.strcpy((byte)(buf), (byte)(tmp_new));
		.strcat((byte)(buf), (byte)(".rej"));
		ModernizedCProgram.mch_remove(buf);
		...
		if ((st).vim_stat(((byte)tmp_new)) < 0 || generated_stat64 == /* Only continue if the output file was created. */0) {
			ModernizedCProgram.emsg(((byte)("E816: Cannot read patch output")));
				...
				if (generatedB_fname != ((Object)0)) {
					newname = ModernizedCProgram.vim_strnsave(generatedB_fname, (int)(.strlen((byte)(generatedB_fname)) + 4));
					if (newname != ((Object)0)) {
						.strcat((byte)(newname), (byte)(".new"));
					} 
				} 
	...
	public void ex_diffgetput() {
		...
		for (dp = ModernizedCProgram.curtab.getTp_first_diff(); dp != ((Object)0); ) {
	...
	public void ex_vimgrep() {
		...
		for (fi = 0; fi < fcount && !got_int && tomatch > 0; ++fi) {
			...
			if (.time(((Object)0)) > seconds) {
				seconds = .time(((Object)0));
			...
			if (buf == ((Object)0)) {
				if (!got_int) {
					...
					if (using_dummy) {
						...
						if (buf != ((Object)0)) {
							generatedB_flags &=  ~-1024;
							if (buf == first_match_buf && target_dir == ((Object)0) && .strcmp((byte)(dirname_start), (byte)(dirname_now)) != 0) {
	...
	public int tclinit() {
		...
		if (!ModernizedCProgram.tclinfo.getInterp()) {
			 ...
			 ch1 = new ();
			 ch2 = new ();
			ch1 = .Tcl_CreateChannel(ModernizedCProgram.tcl_channel_type, "vimout", (()1), TCL_WRITABLE);
			ch2 = .Tcl_CreateChannel(ModernizedCProgram.tcl_channel_type, "vimerr", (()2), TCL_WRITABLE);
			...
			.Tcl_DeleteCommand(interp, /* replace standard Tcl exit command */"exit");
			.Tcl_CreateObjCommand(interp, "exit", exitcmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::buffer", /* new commands, in ::vim namespace */buffercmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::window", windowcmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::command", commandcmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::beep", beepcmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::option", optioncmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			.Tcl_CreateObjCommand(interp, "::vim::expr", exprcmd, .UNRECOGNIZEDFUNCTIONNAME((Object)0), ()((Object)0));
			ModernizedCProgram.tclinfo.setLbase(/* "lbase" variable */1);
			.strcpy(varname, "::vim::lbase");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getLbase(), TCL_LINK_INT);
			ModernizedCProgram.tclinfo.setRange_start(generatedLine1);
			.strcpy(varname, "::vim::range(start)");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getRange_start(), TCL_LINK_INT | TCL_LINK_READ_ONLY);
			.strcpy(varname, "::vim::range(begin)");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getRange_start(), TCL_LINK_INT | TCL_LINK_READ_ONLY);
			ModernizedCProgram.tclinfo.setRange_end(generatedLine2);
			.strcpy(varname, "::vim::range(end)");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getRange_end(), TCL_LINK_INT | TCL_LINK_READ_ONLY);
			ModernizedCProgram.tclinfo.setCurbuf(.Tcl_Alloc(/* "current" variable */64));
			ModernizedCProgram.tclinfo.setCurwin(.Tcl_Alloc(64));
			name = curbuf.tclgetbuffer(interp);
			.strcpy(ModernizedCProgram.tclinfo.getCurbuf(), name);
			.strcpy(varname, "::vim::current(buffer)");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getCurbuf(), TCL_LINK_STRING | TCL_LINK_READ_ONLY);
			name = ModernizedCProgram.curwin.tclgetwindow(interp);
			.strcpy(ModernizedCProgram.tclinfo.getCurwin(), name);
			.strcpy(varname, "::vim::current(window)");
			.Tcl_LinkVar(interp, varname, (byte)ModernizedCProgram.tclinfo.getCurwin(), TCL_LINK_STRING | TCL_LINK_READ_ONLY);
	...
	public void ex_tcl() {
		...
		if (!generatedSkip) {
			err = eap.tclinit();
			if (err == 1) {
				.Tcl_AllowExceptions(ModernizedCProgram.tclinfo.getInterp());
				if (script == ((Object)0)) {
					err = .Tcl_Eval(ModernizedCProgram.tclinfo.getInterp(), (byte)generatedArg);
				} else {
						err = .Tcl_Eval(ModernizedCProgram.tclinfo.getInterp(), (byte)script);
				} 
	...
	public void ex_tclfile() {
		...
		if (err == 1) {
			.Tcl_AllowExceptions(ModernizedCProgram.tclinfo.getInterp());
			err = .Tcl_EvalFile(ModernizedCProgram.tclinfo.getInterp(), file);
			err = ModernizedCProgram.tclexit(err/*
			 * ":tcldo"
			 */);
		} 
	...
	public void ex_tcldo() {
		...
		.strcpy(var_lnum, "lnum");
		.strcpy(var_line, "line");
		err = eap.tclinit();
		if (err != 1) {
			return ;
		} 
		lnum = ((rs) - (ModernizedCProgram.tclinfo.getLbase() == 0));
		.Tcl_LinkVar(ModernizedCProgram.tclinfo.getInterp(), var_lnum, (byte)lnum, TCL_LINK_INT | TCL_LINK_READ_ONLY);
		err = TCL_OK;
		if (ModernizedCProgram.u_save((linenr_T)(rs - 1), (linenr_T)(re + 1)) != 1) {
			.Tcl_SetResult(ModernizedCProgram.tclinfo.getInterp(), ((byte)("cannot save undo information")), TCL_STATIC);
			err = TCL_ERROR;
		} 
		memline generatedB_ml = curbuf.getB_ml();
		Object generatedMl_line_count = generatedB_ml.getMl_line_count();
		while (err == TCL_OK && rs <= re) {
			...
			if (!line) {
				.Tcl_SetResult(ModernizedCProgram.tclinfo.getInterp(), ((byte)("cannot get line")), TCL_STATIC);
				err = TCL_ERROR;
				break;
			} 
			.Tcl_SetVar(ModernizedCProgram.tclinfo.getInterp(), var_line, line, 0);
			.Tcl_AllowExceptions(ModernizedCProgram.tclinfo.getInterp());
			err = .Tcl_Eval(ModernizedCProgram.tclinfo.getInterp(), script);
			if (err != TCL_OK || .Tcl_InterpDeleted(ModernizedCProgram.tclinfo.getInterp()) || curbuf != was_curbuf) {
				break;
			} 
			line = (byte).Tcl_GetVar(ModernizedCProgram.tclinfo.getInterp(), var_line, 0);
			if (line) {
				if (ModernizedCProgram.ml_replace((linenr_T)rs, (char_u)line, 1) != 1) {
					.Tcl_SetResult(ModernizedCProgram.tclinfo.getInterp(), ((byte)("cannot replace line")), TCL_STATIC);
					err = TCL_ERROR;
					break;
				} 
				if (first_line == 0) {
			...
			++rs;
			++lnum;
			.Tcl_UpdateLinkedVar(ModernizedCProgram.tclinfo.getInterp(), var_lnum);
		}
		if (first_line) {
			ModernizedCProgram.changed_lines(first_line, 0, last_line + 1, (long)0);
		} 
		.Tcl_UnsetVar(ModernizedCProgram.tclinfo.getInterp(), var_line, 0);
		.Tcl_UnlinkVar(ModernizedCProgram.tclinfo.getInterp(), var_lnum);
		if (err == TCL_OK) {
			.Tcl_ResetResult(ModernizedCProgram.tclinfo.getInterp());
		} 
	...
	public void ex_compiler() {
		...
		if (generatedArg == (byte)'\000') {
			ModernizedCProgram.do_cmdline_cmd((char_u)/* List all compiler scripts. */"echo globpath(&rtp, 'compiler/*.vim')"/* ) keep the indenter happy... */);
				...
				buf = ModernizedCProgram.alloc(.strlen((byte)(generatedArg)) + 14);
				if (buf != ((Object)0)) {
					if (generatedForceit) {
					...
					.sprintf((byte)buf, "compiler/%s.vim", generatedArg);
		...
		if (name == (byte)'\000') {
			if (what == 6789) {
				p = ModernizedCProgram.get_mess_env();
					...
					p = (char_u).setlocale(what, ((Object)0));
			} 
			if (p == ((Object)0) || p == (byte)'\000') {
				p = (char_u)"Unknown";
			} 
			ModernizedCProgram.smsg(((byte)("Current %slanguage: \"%s\"")), whatstr, p);
				...
				if (what == 6789) {
					loc = "";
						...
						loc = .setlocale(what, (byte)name);
						.setlocale(4, /* Make sure strtod() uses a decimal point, not a comma. */"C");
				} 
				if (loc == ((Object)0)) {
	...
	public int readfile(Object fname, Object sfname, Object from, Object lines_to_skip, Object lines_to_read, int flags) {
		...
		if (fname != ((Object)0) && fname != (byte)'\000') {
			p = fname + .strlen((byte)(fname));
			if (ModernizedCProgram.after_pathsep(fname, p) || .strlen((byte)(fname)) >= 1024) {
		...
		file_readonly = 0;
		if (read_stdin) {
			.setmode(0, /* Force binary I/O on stdin to avoid CR-LF -> LF conversion. */-1024);
		}  else if (!read_buffer) {
			...
			if (isdir_f) {
				...
				if (perm < 0 && (._errno()) == 2) {
					...
					return /* a new file is not an error */1;
						...
						curbuf.filemess(sfname, (char_u)(((._errno()) == 27) ? ((byte)("[File too big]")) : ((byte)("[Permission Denied]"))), 0);
						curbuf.setB_p_ro(/* must use "w!" now */1);
				} 
			} 
			return 0/*
		...
		if (converted) {
			if (.strcmp((byte)(fenc), (byte)("ucs-bom")) == /* "ucs-bom" means we need to check the first bytes of the file
				 * for a BOM. */0) {
				fio_flags = -1024/*
					 ...
					 * the buffer.
					 */;
			}  else if (enc_utf8 || .strcmp((byte)(ModernizedCProgram.p_enc), (byte)("latin1")) == 0) {
		...
		if (!read_buffer && !read_stdin) {
			.close(/* errors are ignored */fd);
		} 
		ModernizedCProgram.vim_free(buffer);
		if (read_stdin) {
			.close(/* Use stderr for stdin, makes shell commands work. */0);
			ModernizedCProgram.vim_ignored = .dup(2);
		} 
		if (tmpname != ((Object)0)) {
			ModernizedCProgram.mch_remove(/* delete converted file */tmpname);
			ModernizedCProgram.vim_free(tmpname);
		...
		if (!recoverymode) {
			...
			if (!filtering && !(flags & -1024)) {
				...
				c = 0/* fifo */;
				if (generatedB_p_ro) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(ModernizedCProgram.shortmess((byte)'r') ? ((byte)("[RO]")) : ((byte)("[readonly]"))));
					c = 1;
				} 
				if (read_no_eol_lnum) {
				...
				if (ff_error == 1) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[CR missing]"))));
					c = 1;
				} 
				if (split) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[long lines split]"))));
					c = 1;
				} 
				if (notconverted) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[NOT converted]"))));
					c = 1;
				}  else if (converted) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[converted]"))));
					c = 1;
				} 
				...
				if (conv_error != 0) {
					.sprintf((byte)ModernizedCProgram.IObuff + .strlen((byte)(ModernizedCProgram.IObuff)), ((byte)("[CONVERSION ERROR in line %ld]")), (long)conv_error);
					c = 1;
				}  else if (illegal_byte > 0) {
					.sprintf((byte)ModernizedCProgram.IObuff + .strlen((byte)(ModernizedCProgram.IObuff)), ((byte)("[ILLEGAL BYTE in line %ld]")), (long)illegal_byte);
					c = 1;
				}  else if (error) {
					.strcat((byte)(ModernizedCProgram.IObuff), (byte)(((byte)("[READ ERRORS]"))));
					c = 1;
				} 
	...
	public void ex_loadkeymap() {
		...
		generatedB_kmap_ga.ga_init2((int), 20);
		...
		for (; ; ) {
			line = .UNRECOGNIZEDFUNCTIONNAME(0, generatedCookie, 0, 1);
			if (line == ((Object)0)) {
				break;
			} 
			...
			if (p != (byte)'"' && p != (byte)'\000' && generatedB_kmap_ga.ga_grow(1) == 1) {
				...
				if (generatedFrom == ((Object)0) || generatedTo == ((Object)0) || .strlen((byte)(generatedFrom)) + .strlen((byte)(generatedTo)) >= 200 || generatedFrom == (byte)'\000' || generatedTo == (byte)'\000') {
		...
		for (i = 0; i < generatedB_kmap_ga.getGa_len(); ++/*
		     ...
		     */i) {
			ModernizedCProgram.vim_snprintf((byte)buf, , "<buffer> %s %s", generatedFrom, generatedTo);
	...
	public void ex_buffer_all() {
		...
		for (wp = ModernizedCProgram.lastwin; open_wins > count; ) {
			...
			if (generatedY_array != ((Object)0)) {
				...
				for (j = 0; j < generatedY_size && n > 1; ++j) {
					...
					for (p = generatedY_array[j]; p && (n -= ModernizedCProgram.ptr2cells(p)) >= 0; ++p) {
						clen = .UNRECOGNIZEDFUNCTIONNAME(p);
	...
	public void ex_function() {
				...
				if (generatedRegprog != ((Object)0)) {
					...
					for (hi = ModernizedCProgram.func_hashtab.getHt_array(); todo > 0 && !got_int; ++hi) {
						if (!(generatedHi_key == ((Object)0) || generatedHi_key == ModernizedCProgram.hash_removed)) {
							...
							if (!.isdigit(generatedUf_name) && regmatch.vim_regexec(generatedUf_name, 0)) {
								fp.list_func_head(0);
							} 
						} 
					}
		...
		/* find extra arguments "range", "dict", "abort" and "closure" */for (; ; ) {
			p = ModernizedCProgram.skipwhite(p);
			if (.strncmp((byte)(p), (byte)("range"), (size_t)(true)) == 0) {
				flags |=  -1024;
				p += 5;
			}  else if (.strncmp((byte)(p), (byte)("dict"), (size_t)(true)) == 0) {
				flags |=  -1024;
				p += 4;
			}  else if (.strncmp((byte)(p), (byte)("abort"), (size_t)(true)) == 0) {
				flags |=  -1024;
				p += 5;
			}  else if (.strncmp((byte)(p), (byte)("closure"), (size_t)(true)) == 0) {
				flags |=  -1024;
				p += 7;
		...
		for (; ; ) {
				...
				if (p == ((Object)0)) {
					...
					if (generatedGetline == ((Object)0)) {
						theline = ModernizedCProgram.getcmdline((byte)':', -1024, indent, do_concat);
							...
							theline = .UNRECOGNIZEDFUNCTIONNAME((byte)':', generatedCookie, indent, do_concat);
					} 
			...
			if (skip_until != ((Object)0)) {
				if (heredoc_trimmed == ((Object)0) || (is_heredoc && ModernizedCProgram.skipwhite(theline) == theline) || .strncmp((byte)(theline), (byte)(heredoc_trimmed), (size_t)(.strlen((byte)(heredoc_trimmed)))) == 0) {
					if (heredoc_trimmed == ((Object)0)) {
						p = theline;
					}  else if (is_heredoc) {
						p = ModernizedCProgram.skipwhite(theline) == theline ? theline : theline + .strlen((byte)(heredoc_trimmed));
					} else {
							p = theline + .strlen((byte)(heredoc_trimmed));
					} 
					if (.strcmp((byte)(p), (byte)(skip_until)) == 0) {
						do {
							if ((skip_until) != ((Object)0)) {
					...
					if (indent > 2 && .strncmp((byte)(p), (byte)("end"), (size_t)(true)) == /* Increase indent inside "if", "while", "for" and "try", decrease
						     * at "end". */0) {
						indent -= 2;
					}  else if (.strncmp((byte)(p), (byte)("if"), (size_t)(true)) == 0 || .strncmp((byte)(p), (byte)("wh"), (size_t)(true)) == 0 || .strncmp((byte)(p), (byte)("for"), (size_t)(true)) == 0 || .strncmp((byte)(p), (byte)("try"), (size_t)(true)) == 0) {
						indent += 2;
					} 
					...
					if ((p[0] == (byte)'a' && (!(((int)(p[1]) - (byte)'A' < 26) || ((int)(p[1]) - (byte)'a' < 26)) || p[1] == (byte)'p')) || (p[0] == (byte)'c' && (!(((int)(p[1]) - (byte)'A' < 26) || ((int)(p[1]) - (byte)'a' < 26)) || (p[1] == (byte)'h' && (!(((int)(p[2]) - (byte)'A' < 26) || ((int)(p[2]) - (byte)'a' < 26)) || (p[2] == (byte)'a' && (.strncmp((byte)(p[3]), (byte)("nge"), (size_t)(true)) != 0 || !(((int)(p[6]) - (byte)'A' < 26) || ((int)(p[6]) - (byte)'a' < 26)))))))) || (p[0] == (byte)'i' && (!(((int)(p[1]) - (byte)'A' < 26) || ((int)(p[1]) - (byte)'a' < 26)) || (p[1] == (byte)'n' && (!(((int)(p[2]) - (byte)'A' < 26) || ((int)(p[2]) - (byte)'a' < 26)) || (p[2] == (byte)'s')))))) {
					...
					if (arg[0] == (byte)'=' && arg[1] == (byte)'<' && arg[2] == (byte)'<' && ((p[0] == (byte)'l' && p[1] == (byte)'e' && (!((((int)(p[2]) - (byte)'A' < 26) || ((int)(p[2]) - (byte)'a' < 26)) || ((int)(p[2]) - (byte)'0' < 10)) || (p[2] == (byte)'t' && !((((int)(p[3]) - (byte)'A' < 26) || ((int)(p[3]) - (byte)'a' < 26)) || ((int)(p[3]) - (byte)'0' < 10))))))) {
						...
						if (.strncmp((byte)(p), (byte)("trim"), (size_t)(true)) == 0) {
		...
		if (fp == ((Object)0)) {
			if (generatedFd_dict == ((Object)0) && ModernizedCProgram.vim_strchr(name, (byte)'#') != ((Object)0)) {
				...
				if (sourcing_name != ((Object)0)) {
					...
					if (scriptname != ((Object)0)) {
						...
						plen = (int).strlen((byte)(p));
						slen = (int).strlen((byte)(sourcing_name));
			...
			fp = ModernizedCProgram.alloc_clear(((size_t)generatedUf_name) + .strlen((byte)(name)) + 1);
			if (fp == ((Object)0)) {
				;
			} 
			if (generatedFd_dict != ((Object)0)) {
			...
			.strcpy((byte)(generatedUf_name), (byte)(/* insert the new function in the function list */name));
			if (overwrite) {
	...
	public Object set_cmdarg(Object oldarg) {
		...
		Object generatedCmd = this.getCmd();
		if (generatedForce_enc != 0) {
			len += (int).strlen((byte)(generatedCmd + generatedForce_enc)) + 7;
		} 
		int generatedBad_char = this.getBad_char();
		...
		if (generatedForce_bin == 1) {
			.sprintf((byte)newval, " ++bin");
		}  else if (generatedForce_bin == 2) {
			.sprintf((byte)newval, " ++nobin");
		} else {
				newval = (byte)'\000';
		} 
		if (generatedRead_edit) {
			.strcat((byte)(newval), (byte)(" ++edit"));
		} 
		if (generatedForce_ff != 0) {
			.sprintf((byte)newval + .strlen((byte)(newval)), " ++ff=%s", generatedForce_ff == (byte)'u' ? "unix" : generatedForce_ff == (byte)'d' ? "dos" : "mac");
		} 
		if (generatedForce_enc != 0) {
			.sprintf((byte)newval + .strlen((byte)(newval)), " ++enc=%s", generatedCmd + generatedForce_enc);
		} 
		if (generatedBad_char == -1) {
			.strcpy((byte)(newval + .strlen((byte)(newval))), (byte)(" ++bad=keep"));
		}  else if (generatedBad_char == -2) {
			.strcpy((byte)(newval + .strlen((byte)(newval))), (byte)(" ++bad=drop"));
		}  else if (generatedBad_char != 0) {
			.sprintf((byte)newval + .strlen((byte)(newval)), " ++bad=%c", generatedBad_char);
		} 
	...
	public void ex_command() {
		...
		if (!has_attr && ModernizedCProgram.ends_excmd(p)) {
			...
			return ;
		}  else if ((name_len == 1 && name == (byte)'X') || (name_len <= 4 && .strncmp((byte)(name), (byte)("Next"), (size_t)(name_len > 4 ? 4 : name_len)) == 0)) {
	...
	public void ex_delcommand() {
		...
		for (; ; ) {
			for (i = 0; i < generatedGa_len; ++i) {
				...
				cmp = .strcmp((byte)(generatedArg), (byte)(generatedUc_name));
				if (cmp <= 0) {
					break;
				} 
			}
		...
		if (i < generatedGa_len) {
			.memmove((byte)(cmd), (byte)(cmd + 1), (size_t)((generatedGa_len - i) * /*
	...
	public void do_ucmd() {
		...
		for (; ; ) {
			...
			for (; ; ) {
				...
				if (buf != ((Object)0)) {
					...
					if (ksp == (true) && (start == ((Object)0) || ksp < start || end == ((Object)0)) && ((ksp[1] == 254 && ksp[2] == ((byte)'X')))) {
						len = ksp - p;
						if (len > 0) {
							.memmove((byte)(q), (byte)(p), (size_t)(len));
							q += len;
						} 
			...
			if (buf != ((Object)0)) {
				.strcpy((byte)(q), (byte)(p));
				break;
			} 
			totlen += .strlen((byte)(p));
		...
		if (arg == (byte)'\000') {
			...
			if (ModernizedCProgram.curwin.getW_s().getB_syn_isk() != empty_option) {
				...
				if (ModernizedCProgram.vim_strnicmp((byte)(arg), (byte)("clear"), (size_t)(true)) == 0) {
					.memmove((byte)(ModernizedCProgram.curwin.getW_s().getB_syn_chartab()), (byte)(generatedB_chartab), (size_t)((size_t)32));
					ModernizedCProgram.clear_string_option(ModernizedCProgram.curwin.getW_s().getB_syn_isk());
				} else {
						.memmove((byte)(save_chartab), (byte)(generatedB_chartab), (size_t)((size_t)32));
						save_isk = generatedB_p_isk;
						curbuf.setB_p_isk(ModernizedCProgram.vim_strsave(arg));
						curbuf.buf_init_chartab(0);
						.memmove((byte)(ModernizedCProgram.curwin.getW_s().getB_syn_chartab()), (byte)(generatedB_chartab), (size_t)((size_t)32));
						.memmove((byte)(generatedB_chartab), (byte)(save_chartab), (size_t)((size_t)32));
						ModernizedCProgram.clear_string_option(ModernizedCProgram.curwin.getW_s().getB_syn_isk());
	...
	public void syn_cmd_onoff(Byte name) {
		...
		int generatedSkip = this.getSkip();
		if (!generatedSkip) {
			.strcpy((byte)(buf), (byte)("so "));
			ModernizedCProgram.vim_snprintf((byte)buf + 3,  - 3, "$VIMRUNTIME\\syntax\\%s.vim", name);
	...
	public void syn_cmd_match(int syncing) {
		...
		Object generatedFlags = syn_opt_arg.getFlags();
		if (.vim_regcomp_had_eol() && !(generatedFlags & -1024)) {
			generatedFlags |=  -1024;
		} 
		rest = syn_opt_arg.get_syn_options(rest, conceal_char, generatedSkip);
	...
	public void syn_cmd_region(int syncing) {
		...
		while (rest != ((Object)0) && !ModernizedCProgram.ends_excmd(rest)) {
			...
			if (.strcmp((byte)(key), (byte)("MATCHGROUP")) == 0) {
				item = 3;
			}  else if (.strcmp((byte)(key), (byte)("START")) == 0) {
				item = 0;
			}  else if (.strcmp((byte)(key), (byte)("END")) == 0) {
				item = 2;
			}  else if (.strcmp((byte)(key), (byte)("SKIP")) == 0) {
			...
			if (item == 3) {
				...
				if ((p - rest == 4 && .strncmp((byte)(rest), (byte)("NONE"), (size_t)(true)) == 0) || generatedSkip) {
					matchgroup_id = 0;
				} else {
				...
				rest = ModernizedCProgram.skipwhite(p/*
					...
					ppp = (pat_ptr)ModernizedCProgram.alloc();
					if (ppp == ((Object)0)) {
						rest = ((Object)0);
						break;
					} 
					...
					pat_ptrs[item] = ppp;
					ppp.setPp_synp((synpat_T)ModernizedCProgram.alloc_clear());
					...
					reg_do_extmatch = 0;
					if (item == 2 && .vim_regcomp_had_eol() && !(generatedFlags & -1024)) {
	...
	public void syn_cmd_sync(int syncing) {
		...
		while (!ModernizedCProgram.ends_excmd(arg_start)) {
			...
			if (.strcmp((byte)(key), (byte)("CCOMMENT")) == 0) {
				if (!generatedSkip) {
					ModernizedCProgram.curwin.getW_s().getB_syn_sync_flags() |=  -1024;
				} 
			...
			}  else if (.strncmp((byte)(key), (byte)("LINES"), (size_t)(true)) == 0 || .strncmp((byte)(key), (byte)("MINLINES"), (size_t)(true)) == 0 || .strncmp((byte)(key), (byte)("MAXLINES"), (size_t)(true)) == 0 || .strncmp((byte)(key), (byte)("LINEBREAKS"), (size_t)(true)) == 0) {
				if (key[4] == (byte)'S') {
					arg_end = key + 6;
			...
			}  else if (.strcmp((byte)(key), (byte)("FROMSTART")) == 0) {
				if (!generatedSkip) {
			...
			}  else if (.strcmp((byte)(key), (byte)("LINECONT")) == 0) {
				...
				next_arg = ModernizedCProgram.skipwhite(arg_end + 1);
					...
					if (.strcmp((byte)(key), (byte)("MATCH")) == 0) {
						eap.syn_cmd_match(1);
					}  else if (.strcmp((byte)(key), (byte)("REGION")) == 0) {
						eap.syn_cmd_region(1);
					}  else if (.strcmp((byte)(key), (byte)("CLEAR")) == 0) {
						eap.syn_cmd_clear(1);
					} else {
							illegal = 1;
					} 
	...
	public void ex_syntax() {
		...
		if (subcmd_name != ((Object)0)) {
			...
			for (i = 0; ; ++i) {
				...
				if (.strcmp((byte)(subcmd_name), (byte)((char_u)ModernizedCProgram.subcommands[i].getName())) == 0) {
					this.setArg(ModernizedCProgram.skipwhite(subcmd_end));
					.UNRECOGNIZEDFUNCTIONNAME(eap, 0);
					break;
				} 
			}
	...
	public void ex_ownsyntax() {
		...
		if (ModernizedCProgram.curwin.getW_s() == ModernizedCProgram.curwin.getW_buffer().getB_s()) {
			ModernizedCProgram.curwin.setW_s((synblock_T)ModernizedCProgram.alloc());
			.memset(ModernizedCProgram.curwin.getW_s(), 0, );
	...
	public void ex_syntime() {
		Object generatedArg = this.getArg();
		if (.strcmp((byte)(generatedArg), (byte)("on")) == 0) {
			ModernizedCProgram.syn_time_on = 1;
		}  else if (.strcmp((byte)(generatedArg), (byte)("off")) == 0) {
			ModernizedCProgram.syn_time_on = 0;
		}  else if (.strcmp((byte)(generatedArg), (byte)("clear")) == 0) {
			ModernizedCProgram.syntime_clear();
		}  else if (.strcmp((byte)(generatedArg), (byte)("report")) == 0) {
	...
	public void ex_hardcopy() {
		...
		for (lnum = generatedLine1; lnum <= generatedLine2; /*
		     ...
		     */lnum++) {
			bytes_to_print += (long_u).strlen((byte)(ModernizedCProgram.skipwhite(ModernizedCProgram.ml_get(lnum))));
		}
		if (bytes_to_print == 0) {
			...
			for (ModernizedCProgram.page_count = 0; generatedFile_line <= generatedLine2; ++/*
				...
				for (uncollated_copies = 0; uncollated_copies < generatedN_uncollated_copies; uncollated_copies++) {
					...
					for (side = 0; side <= generatedDuplex; ++side/*
						...
						.sprintf((byte)ModernizedCProgram.IObuff, ((byte)("Printing page %d (%d%%)")), ModernizedCProgram.page_count + 1 + side, generatedBytes_printed > 1000000 ? (int)(generatedBytes_printed / (bytes_to_print / 100)) : (int)((generatedBytes_printed * 100) / bytes_to_print));
						if (!ModernizedCProgram.mch_print_begin_page(ModernizedCProgram.IObuff)) {
							;
						} 
						if (generatedN_collated_copies > 1) {
							.sprintf((byte)ModernizedCProgram.IObuff + .strlen((byte)(ModernizedCProgram.IObuff)), ((byte)(" Copy %d of %d")), collated_copies + 1, generatedN_collated_copies);
						} 
						...
						for (page_line = 0; page_line < generatedLines_per_page; ++page_line) {
							...
							if (generatedColumn == 0) {
								generatedBytes_printed += .strlen((byte)(ModernizedCProgram.skipwhite(ModernizedCProgram.ml_get(generatedFile_line))));
	...
	public void ex_menu() {
		...
		if (.strncmp((byte)(arg), (byte)("icon="), (size_t)(true)) == /* Locate an optional "icon=filename" argument. */0) {
			arg += 5;
			while (arg != (byte)'\000' && arg != (byte)' ') {
				if (arg == (byte)'\\') {
					.memmove((byte)((arg)), (byte)((arg + 1)), (size_t)(.strlen((byte)(arg + 1)) + 1));
				} 
				arg += .UNRECOGNIZEDFUNCTIONNAME(arg);
			}
			if (arg != (byte)'\000') {
				arg++ = (byte)'\000';
				arg = ModernizedCProgram.skipwhite(arg);
		...
		pri_tab[10] = -/* mark end of the table */1/*
		     ...
		     */;
		if (.strncmp((byte)(arg), (byte)("enable"), (size_t)(true)) == 0 && ((arg[6]) == (byte)' ' || (arg[6]) == (byte)'\t')) {
			enable = 1;
			arg = ModernizedCProgram.skipwhite(arg + 6);
		}  else if (.strncmp((byte)(arg), (byte)("disable"), (size_t)(true)) == 0 && ((arg[7]) == (byte)' ' || (arg[7]) == (byte)'\t')) {
			enable = 0;
		...
		if (enable != 2/*
			 ...
			 */) {
			if (.strcmp((byte)(menu_path), (byte)("*")) == /* meaning: do all menus */0) {
				menu_path = (char_u)"";
			} 
			 ...
			 * Delete menu(s).
			 */) {
			if (.strcmp((byte)(menu_path), (byte)("*")) == /* meaning: remove all menus */0) {
				menu_path = (char_u)""/*
					 * For the PopUp menu, remove a menu for each mode separately.
					 */;
			} 
	...
	public void ex_menutranslate() {
		...
		if (ModernizedCProgram.menutrans_ga.getGa_itemsize() == 0) {
			ModernizedCProgram.menutrans_ga.ga_init2((int), 5/*
			     * ":menutrans clear": clear all translations.
			     */);
		} 
		if (.strncmp((byte)(arg), (byte)("clear"), (size_t)(true)) == 0 && ModernizedCProgram.ends_excmd(ModernizedCProgram.skipwhite(arg + 5))) {
	...
	public Object script_get(Object cmd) {
		...
		Object generatedCookie = this.getCookie();
		for (; ; ) {
			theline = .UNRECOGNIZEDFUNCTIONNAME(generatedCstack.getCs_looplevel() > 0 ? -1 : (byte)'\000', generatedCookie, 0, 1);
			if (theline == ((Object)0) || .strcmp((byte)(end_pattern), (byte)(theline)) == 0) {
				ModernizedCProgram.vim_free(theline);
				break;
			} 
	...
	public void ex_undolist() {
		...
		while (uhp != ((Object)0)) {
			if (generatedPtr == ((Object)0) && generatedUh_walk != nomark && generatedUh_walk != mark) {
				...
				ModernizedCProgram.u_add_time(ModernizedCProgram.IObuff + .strlen((byte)(ModernizedCProgram.IObuff)), (1024 + 1) - .strlen((byte)(ModernizedCProgram.IObuff)), generatedUh_time);
				if (generatedUh_save_nr > 0) {
					while (.strlen((byte)(ModernizedCProgram.IObuff)) < 33) {
	...
	public void ex_messages() {
		...
		int generatedAddr_count = this.getAddr_count();
		Object generatedLine2 = this.getLine2();
		if (.strcmp((byte)(generatedArg), (byte)("clear")) == 0) {
		...
		if (p == ModernizedCProgram.first_msg_hist) {
			s = (char_u).getenv((byte)((char_u)"LANG"));
