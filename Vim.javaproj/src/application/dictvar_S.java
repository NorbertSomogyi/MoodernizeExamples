...
public class dictvar_S {
		...
		if (generatedMl_flags & 1) {
			if (dict == ((Object)0)) {
				...
				for (lnum = 1; lnum <= generatedMl_line_count; ++lnum) {
					...
					if (/* Do extra processing for VIsual mode. */VIsual_active && lnum >= generatedLnum && lnum <= generatedLnum) {
						...
						if (s != ((Object)0)) {
							...
							if (lnum == generatedMl_line_count && !generatedB_p_eol && (generatedB_p_bin || !generatedB_p_fixeol) && (long)/*Error: Function owner not recognized*/strlen((byte)(s)) < len) {
				...
				if (dict == ((Object)0)) {
					if (VIsual_active) {
						if (VIsual_mode == 22 && ModernizedCProgram.curwin.getW_curswant() < INT_MAX) {
							...
							ModernizedCProgram.vim_snprintf((byte)buf1, /*Error: sizeof expression not supported yet*/, ((byte)("%ld Cols; ")), (long)(generatedEnd_vcol - generatedStart_vcol + 1));
						} else {
								buf1[0] = (byte)'\000';
						} 
						if (char_count_cursor == byte_count_cursor && char_count == byte_count) {
							...
							ModernizedCProgram.col_print(buf1, /*Error: sizeof expression not supported yet*/, (int)generatedCol + 1, (int)ModernizedCProgram.curwin.getW_virtcol() + 1);
							ModernizedCProgram.col_print(buf2, /*Error: sizeof expression not supported yet*/, (int)/*Error: Function owner not recognized*/strlen((byte)(p)), ModernizedCProgram.linetabsize(p));
							if (char_count_cursor == byte_count_cursor && char_count == byte_count) {
				...
				if (dict == ((Object)0) && bom_count > 0) {
					ModernizedCProgram.vim_snprintf((byte)ModernizedCProgram.IObuff + /*Error: Function owner not recognized*/strlen((byte)(ModernizedCProgram.IObuff)), (1024 + 1), ((byte)("(+%lld for BOM)")), (long_long_T)bom_count);
				} 
				if (dict == ((Object)0)) {
	...
	public int popup_options_one(Object key) {
		...
		if (/*Error: Function owner not recognized*/strncmp((byte)(val), (byte)("cursor"), (size_t)(true)) != 0) {
		...
		if (s != (byte)'\000') {
			...
			if (ModernizedCProgram.skipwhite(s) == (byte)'+' || ModernizedCProgram.skipwhite(s) == (byte)'-') {
				n = /*Error: Function owner not recognized*/strtol((byte)s, (byte)endp, 10);
			} 
		...
		if (/*Error: Function owner not recognized*/strcmp((byte)(key), (byte)("line")) == 0) {
			n = /*Error: Function owner not recognized*/screen_screenrow() + 1 + n;
		} else {
				n = /*Error: Function owner not recognized*/screen_screencol() + 1 + n;
		} 
		if (n < 1) {
			n = 1;
		} 
	...
	public void set_padding_border(int[] array, Byte name, int max_val) {
		dictitem_T di = new dictitem_T();
		dictitem_S dictitem_S = new dictitem_S();
	...
	public void get_padding_border(int[] array, Byte name) {
	...
	public void ins_compl_add_dict() {
		...
		if (di_refresh != ((Object)0) && generatedV_type == .VAR_STRING) {
			...
			if (v != ((Object)0) && /*Error: Function owner not recognized*/strcmp((byte)(v), (byte)((char_u)"always")) == 0) {
	...
	public int add_tag_field(Byte field_name, Object start, Object[] end) {
		...
		if (start != ((Object)0)) {
			if (end == ((Object)0)) {
				end = start + /*Error: Function owner not recognized*/strlen((byte)(start));
	...
	public dictvar_S make_partial( rettv) {
		...
		if (fp != ((Object)0) && (generatedUf_flags & -1024)) {
			partial_T pt = (partial_T)ModernizedCProgram.alloc_clear(/*Error: Unsupported expression*/);
			if (pt != ((Object)0)) {
				pt.setPt_refcount(1);
				...
				if (generatedV_type == .VAR_FUNC) {
					pt.setPt_name(generatedVval.getV_string());
						...
						if (generatedPt_argc > 0) {
							pt.setPt_argv((typval_T)ModernizedCProgram.alloc(/*Error: Unsupported expression*/ * (generatedPt_argc)));
	...
	public dictvar_S dict_alloc() {
		dict_T d = new dict_T();
		d = (dict_T)ModernizedCProgram.alloc_clear(/*Error: Unsupported expression*/);
