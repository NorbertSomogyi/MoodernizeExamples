...
public class vimoption {
	...
	public int optval_default(Object varp, int compatible) {
		...
		return (.strcmp((byte)((char_u)varp), (byte)(generatedDef_val[dvi])) == /* P_STRING */0/*
	...
	public void option_value2string(int opt_flags) {
		...
		if (generatedFlags & -1024) {
			...
			if (ModernizedCProgram.wc_use_keyname(varp, wc)) {
				.strcpy((byte)(ModernizedCProgram.NameBuff), (byte)(ModernizedCProgram.get_special_key_name((int)wc, 0)));
			}  else if (wc != 0) {
				.strcpy((byte)(ModernizedCProgram.NameBuff), (byte)(ModernizedCProgram.transchar((int)wc)));
			} else {
					.sprintf((byte)ModernizedCProgram.NameBuff, "%ld", (long)varp);
			} 
		} else {
				...
				if (varp == ((Object)/* just in case */0)) {
					...
					.strcpy((byte)(ModernizedCProgram.NameBuff), (byte)("*****"));
