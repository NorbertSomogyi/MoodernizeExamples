...
public class hashes {
	...
	public int module_hash_binary_parse(Object hashconfig, Object user_options, Object user_options_extra) {
		...
		byte in = (byte)ModernizedCProgram.hcmalloc();
		...
		hashinfo generatedHash_info = hash.getHash_info();
		while (!fp.hc_feof()) {
			size_t nread = fp.hc_fread(in, , 1);
			if (nread == 0) {
				break;
			} 
			.memset(hashes_buf[hashes_cnt].getSalt(), 0, );
			.memset(hashes_buf[hashes_cnt].getEsalt(), 0, );
			wpa_eapol_t wpa_eapol = (wpa_eapol_t)hashes_buf[hashes_cnt].getEsalt();
			...
			if (generatedMessage_pair & (1 << 4)) {
				wpa_eapol.setNonce_error_corrections(0);
			} else {
					if (generatedMessage_pair & (1 << 7)) {
			...
			if (generatedMessage_pair & (1 << 5)) {
				wpa_eapol.setDetected_le(1);
				wpa_eapol.setDetected_be(0);
			}  else if (generatedMessage_pair & (1 << 6)) {
				wpa_eapol.setDetected_le(0);
				wpa_eapol.setDetected_be(1);
			} 
			hash_t hash = hashes_buf[hashes_cnt];
			int parser_status = ModernizedCProgram.module_hash_decode(hashconfig, generatedDigest, generatedSalt, generatedEsalt, generatedHook_salt, generatedHash_info, in, );
			if (parser_status != parser_rc.PARSER_OK) {
				continue;
			} 
			hashes_cnt++;
		...
		for (int keyslot_idx = 0;
		 keyslot_idx < 8; keyslot_idx++) {
			hash_t hash = hashes_buf[hashes_cnt];
			.memset(generatedSalt, 0, );
			.memset(generatedEsalt, 0, );
			int parser_status = ModernizedCProgram.module_hash_decode(hashconfig, generatedDigest, generatedSalt, generatedEsalt, generatedHook_salt, generatedHash_info, generatedHashfile, .strlen(generatedHashfile));
			if (parser_status != parser_rc.PARSER_OK) {
				continue;
			} 
			hashes_cnt++;
