	...
	public os_dirent os_readdir(os_dir dir) {
		...
		if (generatedFirst) {
			dir.setFirst(0);
				...
				if (!.FindNextFileW(generatedHandle, generatedWfd)) {
					return ((Object)0);
				} 
		} 
		os_dirent generatedOut = dir.getOut();
		Object generatedD_name = generatedOut.getD_name();
		ModernizedCProgram.os_wcs_to_utf8(generatedWfd.getCFileName(), 0, generatedD_name, );
		...
		dir.setCur_dirent(.readdir(generatedDir));
		...
		os_dirent generatedOut = dir.getOut();
		Object generatedD_name = generatedOut.getD_name();
		.strncpy(generatedD_name, generatedD_name, 255);
