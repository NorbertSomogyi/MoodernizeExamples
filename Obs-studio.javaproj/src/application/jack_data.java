...
public class jack_data {
	...
	public Object jack_init() {
		...
		ModernizedCProgram.data.setJack_client(.jack_client_open(ModernizedCProgram.data.getDevice(), jack_option, 0));
		if (ModernizedCProgram.data.getJack_client() == ((Object)0)) {
		...
		ModernizedCProgram.data.setJack_ports(()ModernizedCProgram.bzalloc( * ModernizedCProgram.data.getChannels()));
		for (int i = 0;
		 i < ModernizedCProgram.data.getChannels(); ++i) {
			byte[] port_name = new byte[]{(byte)'\0'};
			.snprintf(port_name, , "in_%u", i + 1);
			ModernizedCProgram.data.getJack_ports()[i] = .jack_port_register(ModernizedCProgram.data.getJack_client(), port_name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
		...
		if (.jack_set_process_callback(ModernizedCProgram.data.getJack_client(), jack_process_callback, ModernizedCProgram.data) != 0) {
			ModernizedCProgram.blog(LOG_ERROR, "jack-input: jack_set_process_callback Error");
			;
		} 
		if (.jack_activate(ModernizedCProgram.data.getJack_client()) != 0) {
		...
		if (ModernizedCProgram.data.getJack_client()) {
			if (ModernizedCProgram.data.getJack_ports() != ((Object)0)) {
				for (int i = 0;
				 i < ModernizedCProgram.data.getChannels(); ++i) {
					if (ModernizedCProgram.data.getJack_ports()[i] != ((Object)0)) {
						.jack_port_unregister(ModernizedCProgram.data.getJack_client(), ModernizedCProgram.data.getJack_ports()[i]);
					} 
				}
				ModernizedCProgram.bfree(ModernizedCProgram.data.getJack_ports());
				ModernizedCProgram.data.setJack_ports(((Object)0));
			} 
			.jack_client_close(ModernizedCProgram.data.getJack_client());
