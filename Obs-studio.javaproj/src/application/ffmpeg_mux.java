...
public class ffmpeg_mux {
	...
	public void ffmpeg_mux_free() {
		...
		if (generatedInitialized) {
			.av_write_trailer(generatedOutput);
		} 
		ffm.free_avformat();
	...
	public boolean ffmpeg_mux_get_header() {
		...
		boolean success = ModernizedCProgram.safe_read(info, ) == ;
		Object generatedSize = info.getSize();
		if (success) {
			uint8_t data = .malloc(generatedSize);
	...
	public int ffmpeg_mux_init_context() {
		...
		output_format = .av_guess_format(((Object)0), generatedFile, ((Object)0));
		if (output_format == ((Object)0)) {
			.fprintf((_iob[2]), "Couldn't find an appropriate muxer for '%s'\n", generatedFile);
			return -1;
		} 
		Object generatedOutput = this.getOutput();
		ret = .avformat_alloc_output_context2(generatedOutput, output_format, ((Object)0), ((Object)0));
		if (ret < 0) {
			.fprintf((_iob[2]), "Couldn't initialize output context: %s\n", .av_err2str(ret));
			return -1;
		} 
	...
	public int ffmpeg_mux_init_internal(int argc, byte argv) {
		...
		int generatedTracks = generatedParams.getTracks();
		if (generatedTracks) {
			this.setAudio_header(.calloc(1,  * generatedTracks));
		} 
