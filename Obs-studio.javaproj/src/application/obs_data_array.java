...
public class obs_data_array {
	...
	public obs_data_array obs_data_array_create() {
		obs_data_array array = ModernizedCProgram.bzalloc(/*Error: Unsupported expression*/);
		array.setRef(1);
		return array;
	}
	...
	public void obs_data_array_destroy() {
		...
		if (array) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(generatedObjects);
	...
	public void obs_data_array_push_back_array(obs_data_array array2) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back_da(generatedObjects, generatedObjects);
	}
	public void obs_data_array_erase(Object idx) {
		Object generatedObjects = this.getObjects();
		if (array) {
			generatedObjects.getArray()[idx].obs_data_release();
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase(generatedObjects, idx);
		} 
	}
	public obs_data_array data_item_get_array(obs_data_item item, Object get_array) {
		obs_data_array_t array = item.item_valid(obs_data_type.OBS_DATA_ARRAY) ? /*Error: Function owner not recognized*/get_array(item) : ((Object)0);
	...
	public void obs_load_sources(Object cb, Object private_data) {
		...
		obs_core_data data = ModernizedCProgram.obs.getData();
		 count = new ();
		 i = new ();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_init(sources);
		count = array.obs_data_array_count();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_reserve(sources, count);
		...
		for (i = 0; i < count; i++) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back(sources, source);
		...
		for (i = 0; i < sources.getNum(); /* tell sources that we want to load */i++) {
			...
			if (source) {
				...
				if (cb) {
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/cb(private_data, source);
				} 
			} 
			source_data.obs_data_release();
		}
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(sources);
	}
	public obs_data_array obs_save_sources_filtered(Object cb, Object data_) {
		...
		while (source) {
			if ((generatedType != obs_source_type.OBS_SOURCE_TYPE_FILTER) != 0 && !generatedPrivate && /*Error: Function owner not recognized*/cb(data_, source)) {
