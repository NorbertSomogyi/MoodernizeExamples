...
public class os_glob_info {
	...
	public int os_glob(Object pattern, int flags) {
		...
		if (ModernizedCProgram.os_utf8_to_wcs_ptr(pattern, 0, w_path) > 0) {
			...
			if (handle != (HANDLE)(true)) {
				do {
					...
					if (generatedPath) {
						generatedDa.darray_push_back(, ent);
					} 
				} while (.FindNextFileA(handle, wfd));
				.FindClose(handle);
				pglob = ModernizedCProgram.bmalloc();
		...
		int ret = .glob(pattern, 0, ((Object)0), generatedGl);
		...
		if (ret == 0) {
			...
			pglob = ModernizedCProgram.bmemdup(pgi, );
		} else {
				pglob = ((Object)0);
		} 
		(Object)flags;
