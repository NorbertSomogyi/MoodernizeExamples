...
public class lzma_next_coder_s {
	...
	public Object lzma_block_decoder_init(Object allocator, Object block) {
		...
		// except for Uncompressed Size and filters. Filters are validated// by the raw decoder.if (ModernizedCProgram.lzma_block_unpadded_size(block) == 0 || !.lzma_vli_is_valid(block.getUncompressed_size())) {
			return LZMA_PROG_ERROR;
		} 
		lzma_coder_s generatedCoder = this.getCoder();
		// Allocate and initialize *next->coder if needed.if (generatedCoder == ((Object)0)) {
			this.setCoder(ModernizedCProgram.lzma_alloc(, allocator));
			if (generatedCoder == ((Object)0)) {
				return LZMA_MEM_ERROR;
			} 
			this.setCode(block_decode);
			this.setEnd(block_decoder_end);
			generatedCoder.setNext();
		} 
	...
	public Object lzma_stream_decoder_init(Object allocator, Object memlimit, Object flags) {
		...
		if (generatedCoder == ((Object)0)) {
			...
			generatedCoder.setBlock_decoder();
	...
	public Object lzma_lzma2_decoder_init(Object allocator, Object filters) {
		// LZMA2 can only be the last filter in the chain. This is enforced// by the raw_decoder initialization.// LZMA2 can only be the last filter in the chain. This is enforced// by the raw_decoder initialization.((filters[1].getInit() == ((Object)0)) ? (Object)0 : ._assert("filters[1].init == NULL", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lzma2_decoder.c", 256));
	...
	public Object alone_encoder_init(Object allocator, Object options) {
		...
		// - Uncompressed size (always unknown and using EOPM)// - Uncompressed size (always unknown and using EOPM).memset(generatedHeader + 1 + 4, -1024, 8);
		// Initialize the LZMA encoder.lzma_filter_info[] filters = new lzma_filter_info[]{new lzma_filter_info(, ), new lzma_filter_info()};
		lzma_next_coder_s generatedNext = generatedCoder.getNext();
	...
	public Object lzma_delta_coder_init(Object allocator, Object filters) {
		...
		.memset(generatedHistory, 0, LZMA_DELTA_DIST_MAX);
		lzma_next_coder_s generatedNext = generatedCoder.getNext();
	...
	public Object x86_coder_init(Object allocator, Object filters, boolean is_encoder) {
		 ret = next.lzma_simple_coder_init(allocator, filters, x86_code, , 5, 1, is_encoder);
		lzma_coder_s generatedCoder = this.getCoder();
	...
	public Object lzma_lzma_decoder_init(Object allocator, Object filters) {
		// LZMA can only be the last filter in the chain. This is enforced// by the raw_decoder initialization.// LZMA can only be the last filter in the chain. This is enforced// by the raw_decoder initialization.((filters[1].getInit() == ((Object)0)) ? (Object)0 : ._assert("filters[1].init == NULL", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lzma_decoder.c", 986));
	...
	public Object lzma_simple_coder_init(Object allocator, Object filters, Object filter, Object simple_size, Object unfiltered_max, Object alignment, boolean is_encoder) {
		...
		// Allocate memory for the lzma_coder structure if needed.if (generatedCoder == ((Object)0)) {
			this.setCoder(ModernizedCProgram.lzma_alloc( + 2 * unfiltered_max, allocator));
			if (generatedCoder == ((Object)0)) {
				return LZMA_MEM_ERROR;
			} 
	...
	public Object lzma_raw_coder_init(Object allocator, Object options, Object coder_find, boolean is_encoder) {
		...
		if (is_encoder) {
			for (size_t i = 0;
			 i < count; ++i) {
				size_t j = count - i - 1;
				lzma_filter_coder fc = .coder_find(options[i].getId());
	...
	public Object lzma_next_filter_init(Object allocator, Object filters) {
		...
		return generatedInit == ((Object)0) ? LZMA_OK : .UNRECOGNIZEDFUNCTIONNAME(next, allocator, filters);
	}
	public Object lzma_next_filter_update(Object allocator, Object reversed_filters) {
		...
		((generatedUpdate != ((Object)0)) ? (Object)0 : ._assert("next->update != NULL", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\common.c", 114));
		lzma_coder_s generatedCoder = this.getCoder();
		return .UNRECOGNIZEDFUNCTIONNAME(generatedCoder, allocator, ((Object)0), reversed_filters);
	}
	public void lzma_next_end(Object allocator) {
		...
		if (generatedInit != (uintptr_t)(((Object)0))) {
			if (generatedEnd != ((Object)0)) {
				.UNRECOGNIZEDFUNCTIONNAME(generatedCoder, allocator);
			} else {
	...
	public Object lzma_lz_encoder_init(Object allocator, Object filters, Object lz_init) {
		lzma_coder_s generatedCoder = this.getCoder();
		Object generatedLz = generatedCoder.getLz();
		...
		do {
			 ret_ = (.lz_init(generatedLz, allocator, filters[0].getOptions(), lz_options));
			if (ret_ != LZMA_OK) {
				return ret_;
			} 
		} while (0);
	...
	public Object lzma_lz_decoder_init(Object allocator, Object filters, Object lz_init) {
		lzma_coder_s generatedCoder = this.getCoder();
		Object generatedDict = generatedCoder.getDict();
		...
		// Allocate and initialize the dictionary.if (generatedDict.getSize() != generatedDict_size) {
			...
			if (generatedDict.getBuf() == ((Object)0)) {
				return LZMA_MEM_ERROR;
			} 
		...
		// Use the preset dictionary if it was given to us.if (generatedPreset_dict != ((Object)0) && generatedPreset_dict_size > 0) {
			...
			.memcpy(generatedDict.getBuf(), generatedPreset_dict + offset, copy_size);
		...
		Object generatedTemp = generatedCoder.getTemp();
	...
	public Object lzma_stream_encoder_init(Object allocator, Object filters, Object check) {
		...
		if (generatedCoder == ((Object)0)) {
			...
			generatedCoder.setBlock_encoder();
			generatedCoder.setIndex_encoder();
		...
		// Encode the Stream Header stream_flags = new (, );
