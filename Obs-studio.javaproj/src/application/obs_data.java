...
public class obs_data {
	...
	public void scale_filter_update(Object data) {
		...
		if (.strcmp(res_str, ModernizedCProgram.obs_module_text("Base.Canvas")) == 0) {
			...
			filter.setCy_in(generatedBase_height);
				...
				ret = .sscanf(res_str, "%dx%d", generatedCx_in, generatedCy_in);
				if (ret == 2) {
					filter.setAspect_ratio_only(false);
				} else {
						ret = .sscanf(res_str, "%d:%d", generatedCx_in, generatedCy_in);
						if (ret != 2) {
	...
	public Object nvenc_update(Object data) {
		...
		.av_opt_set_int(generatedContext.getPriv_data(), "cbr", false, 0);
		.av_opt_set(generatedContext.getPriv_data(), "profile", profile, 0);
		.av_opt_set(generatedContext.getPriv_data(), "preset", preset, 0);
		if (ModernizedCProgram.astrcmpi(rc, "cqp") == 0) {
			...
			.av_opt_set(generatedContext.getPriv_data(), "preset", hp ? "losslesshp" : "lossless", 0);
		}  else if (ModernizedCProgram.astrcmpi(rc, "vbr") != /* CBR by default */0) {
			.av_opt_set_int(generatedContext.getPriv_data(), "cbr", true, 0);
			generatedContext.setRc_max_rate(bitrate * 1000);
			generatedContext.setRc_min_rate(bitrate * 1000);
			cqp = 0;
		} 
		.av_opt_set(generatedContext.getPriv_data(), "level", "auto", 0);
		.av_opt_set_int(generatedContext.getPriv_data(), "2pass", twopass, 0);
		.av_opt_set_int(generatedContext.getPriv_data(), "gpu", gpu, 0);
		...
		if (generatedCan_change_bitrate) {
			...
			if (enc.nv_failed(.UNRECOGNIZEDFUNCTIONNAME(generatedSession, params), __FUNCTION__, "nv.nvEncReconfigureEncoder(enc->session, &params)")) {
				return false;
			} 
		} 
		return true;
	...
	public void color_correction_filter_update(Object data) {
		...
		double rot_quad1 = ModernizedCProgram.root3 * (double).sin(/* Pseudo-Quaternion To Matrix. */half_angle);
		vec3 generatedRot_quaternion = filter.getRot_quaternion();
		generatedRot_quaternion.vec3_set(rot_quad1, rot_quad1, rot_quad1);
		filter.setRot_quaternion_w((double).cos(half_angle));
	...
	public void vlcs_update(Object data) {
		...
		.libvlc_media_list_player_stop_(generatedMedia_list_player);
		...
		media_list = .libvlc_media_list_new_(ModernizedCProgram.libvlc);
		.libvlc_media_list_lock_(media_list);
		for ( i = 0;
		 i < generatedFiles.getNum(); i++) {
			.libvlc_media_list_add_media_(media_list, generatedFiles.getArray()[i].getMedia());
		}
		.libvlc_media_list_unlock_(media_list);
		.libvlc_media_list_player_set_media_list_(generatedMedia_list_player, media_list);
		.libvlc_media_list_release_(media_list);
		Object generatedLoop = c.getLoop();
		.libvlc_media_list_player_set_playback_mode_(generatedMedia_list_player, generatedLoop ? libvlc_playback_mode_loop : libvlc_playback_mode_default);
		...
		if (generatedFiles.getNum() && (generatedBehavior == behavior.BEHAVIOR_ALWAYS_PLAY || ModernizedCProgram.obs_source_active(generatedSource))) {
			.libvlc_media_list_player_play_(generatedMedia_list_player);
		} else {
	...
	public void frontend_save_callback(Object saving, Object priv) {
		...
		if (ModernizedCProgram.libobs_to_py_("obs_data_t *", save_data, false, py_save_data, NULL, __FUNCTION__, 272)) {
			 args = .Import_Py_BuildValue("(Op)", py_save_data, saving);
			...
			ModernizedCProgram.cur_python_script = (obs_python_script)generatedScript;
			 py_ret = .Import_PyObject_CallObject(generatedFunc, args);
			.Py_XDECREF(py_ret);
	...
	public void stinger_update(Object data) {
		...
		if (generatedTransition_point_is_frame) {
			s.setTransition_point_frame(()point);
				...
				s.setTransition_point_ns(.UNRECOGNIZEDFUNCTIONNAME(point * -1024));
		} 
	...
	public void compressor_update(Object data) {
		...
		cd.setSlope(1.0 - (1.0 / generatedRatio));
		 valid_sidechain = sidechain_name && .strcmp(sidechain_name, "none") != 0;
		...
		if (!valid_sidechain) {
			...
			cd.setSidechain_name(NULL);
				...
				if (!generatedSidechain_name || .strcmp(generatedSidechain_name, sidechain_name) != 0) {
					if (generatedWeak_sidechain) {
						old_weak_sidechain = generatedWeak_sidechain;
						cd.setWeak_sidechain(NULL);
					} 
	...
	public obs_data obs_data_create() {
		obs_data data = ModernizedCProgram.bzalloc();
		data.setRef(1);
		return data;
	}
	...
	public void obs_data_destroy() {
		...
		.free(ModernizedCProgram.data.getJson());
	...
	public Object obs_data_save_json(Object file) {
		...
		if (json && json) {
			return ModernizedCProgram.os_quick_write_utf8_file(file, json, .strlen(json), false);
		} 
		return false;
	}
	public Object obs_data_save_json_safe(Object file, Object temp_ext, Object backup_ext) {
		byte json = ModernizedCProgram.data.obs_data_get_json();
		if (json && json) {
			return ModernizedCProgram.os_quick_write_utf8_file_safe(file, json, .strlen(json), false, temp_ext, backup_ext);
		} 
		return false;
	}
	public void copy_obj(Object name, obs_data obj, Object callback) {
		...
		if (obj) {
			...
			.callback(ModernizedCProgram.data, name, new_obj);
	...
	public obs_data data_item_get_obj(obs_data_item item, Object get_obj) {
		obs_data_t obj = item.item_valid(obs_data_type.OBS_DATA_OBJECT) ? .get_obj(item) : ((Object)0);
	...
	public void set_vec2(Object name, Object val, Object set_obj) {
		...
		.set_obj(ModernizedCProgram.data, name, obj);
	...
	public Object get_frames_per_second(Object fps, Object option) {
		...
		fps.setNumerator(.UNRECOGNIZEDFUNCTIONNAME((num_ll) < 0 ? 0 : ((num_ll) > (long)UINT32_MAX ? (long)UINT32_MAX : (num_ll))));
		fps.setDenominator(.UNRECOGNIZEDFUNCTIONNAME((den_ll) < 0 ? 0 : ((den_ll) > (long)UINT32_MAX ? (long)UINT32_MAX : (den_ll))));
		num.obs_data_item_release();
		den.obs_data_item_release();
		ModernizedCProgram.data.obs_data_release();
	...
	public Object vaapi_update(Object data) {
		...
		Object generatedContext = enc.getContext();
		.av_opt_set_int(generatedContext.getPriv_data(), "qp", qp, 0);
		.av_opt_set_int(generatedContext.getPriv_data(), "quality", quality, 0);
		obs_encoder generatedEncoder = enc.getEncoder();
		 video = ModernizedCProgram.obs_encoder_video(generatedEncoder);
		video_output_info voi = ModernizedCProgram.video_output_get_info(video);
		video_scale_info info = new video_scale_info();
	...
	public Object vt_h264_update(Object data) {
		...
		if (code != noErr) {
		 ...
		 n = new ();
		.VTSessionCopyProperty(generatedSession, kVTCompressionPropertyKey_AverageBitRate, NULL, n);
		 session_bitrate = new ();
	...
	public void alsa_update(Object vptr) {
		...
		if (.strcmp(generatedDevice, device) != 0) {
	...
	public void expander_update(Object data) {
		...
		if (.strcmp(presets, "expander") == 0 && generatedIs_gate) {
		...
		if (.strcmp(presets, "gate") == 0 && !generatedIs_gate) {
			ModernizedCProgram.s.obs_data_clear();
	...
	public Object obs_x264_update(Object data) {
		...
		if (success) {
			ret = .x264_encoder_reconfig(generatedContext, generatedParams);
			if (ret != 0) {
	...
	public void rtmp_common_update(Object data) {
		...
		if (root) {
			byte new_name;
			json_t serv = ModernizedCProgram.find_service(root, generatedService, new_name);
			if (new_name) {
				ModernizedCProgram.bfree(generatedService);
	...
	public void pulse_update(Object vptr) {
		...
		Byte generatedDevice = data.getDevice();
		if (!generatedDevice || .strcmp(generatedDevice, new_device) != 0) {
			if (generatedDevice) {
				ModernizedCProgram.bfree(generatedDevice);
			} 
	...
	public void mask_filter_update(Object data) {
		...
		byte effect_path;
		color &=  -1024;
		color |=  .UNRECOGNIZEDFUNCTIONNAME(((double)opacity) * 2.55) << 24;
		vec4 generatedColor = filter.getColor();
		generatedColor.vec4_from_rgba(color);
		ModernizedCProgram.obs_enter_graphics();
		Object generatedImage = filter.getImage();
	...
	public void noise_suppress_update(Object data) {
		...
		generatedCopy_buffers[0] = ModernizedCProgram.bmalloc(frames * channels * /* One speex state for each channel (limit 2) */);
		Object generatedSegment_buffers = ng.getSegment_buffers();
		generatedSegment_buffers[0] = ModernizedCProgram.bmalloc(frames * channels * );
		for ( c = 1;
		 c < channels; ++c) {
		...
		for ( i = 0;
		 i < num; i++) {
			obs_data_t file = obs_data.obs_data_array_item(array, i);
			 continue_enum = .enum_func(param, file);
	...
	public Object update_files_to_local(Object param) {
		...
		file_update_data data = new file_update_data(, );
	...
	public Object update_remote_files(Object param) {
		...
		if (generatedCallback) {
			...
			confirm = .UNRECOGNIZEDFUNCTIONNAME(generatedParam, download_data);
	...
	public void ss_update(Object data) {
		...
		byte mode;
		/* ------------------------------------- */
		.da_init(/* get settings data */new_files);
		behavior = settings.obs_data_get_string("playback_behavior");
		if (ModernizedCProgram.astrcmpi(behavior, "pause_unpause") == 0) {
		...
		obs_source obs_source = new obs_source();
		if (!generatedTr_name || .strcmp(tr_name, generatedTr_name) != 0) {
		...
		int cx_in = 0;
		int cy_in = 0;
		if (.strcmp(res_str, ModernizedCProgram.obs_module_text("SlideShow.CustomSize.Auto")) != 0) {
			...
			if (aspect_only) {
				if (.fabs(old_aspect - new_aspect) > EPSILON) {
					if (new_aspect > old_aspect) {
						cx = .UNRECOGNIZEDFUNCTIONNAME(cy_f * new_aspect);
					} else {
							cy = .UNRECOGNIZEDFUNCTIONNAME(cx_f / new_aspect);
					} 
				} 
			} else {
					cx = ()cx_in;
	...
	public void ft2_source_update(Object data) {
		...
		if (generatedFont_name != NULL) {
			if (.strcmp(font_name, generatedFont_name) == 0 && .strcmp(font_style, generatedFont_style) == 0 && font_flags == generatedFont_flags && font_size == generatedFont_size) {
				;
			} 
		...
		if (!srcdata.init_font() || generatedFont_face == NULL) {
			...
			;
				...
				.FT_Set_Pixel_Sizes(generatedFont_face, 0, generatedFont_size);
				.FT_Select_Charmap(generatedFont_face, FT_ENCODING_UNICODE);
		} 
	...
	public Object obs_qsv_update(Object data) {
		...
		if (success) {
			...
			ret = .qsv_encoder_reconfig(generatedContext, generatedParams);
			if (ret != 0) {
