...
public class obs_source {
	...
	public obs_source obs_view_get_source(obs_view view, Object channel) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/assert(channel < 64);
		if (!view) {
			return NULL;
		} 
		if (channel >= 64) {
	...
	public Object enum_sources_proc(Object param) {
		...
		if (ModernizedCProgram.libobs_to_py_("obs_source_t *", source, false, py_source, NULL, __FUNCTION__, 1051)) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Import_PyList_Append(list, py_source);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Py_DECREF(py_source);
		} 
		return true;
		 script = param;
		source.obs_source_get_ref();
		ModernizedCProgram.ls_push_libobs_obj_(script, "obs_source_t *", source, false, NULL, __FUNCTION__, 577);
		 idx = /*Error: Function owner not recognized*/lua_rawlen(script, -2);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/lua_rawseti(script, -2, (int)idx + 1);
		return true;
	}
	public void on_audio_playback(Object param, Object audio_data, Object muted) {
		...
		bytes = /*Error: Unsupported expression*/ * generatedChannels * resample_frames;
		if (muted) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(resample_data[0], 0, bytes);
		} else {
		...
		circlebuf generatedNew_data = monitor.getNew_data();
		...
		if (generatedCirclebuf >= generatedWait_size) {
			...
			if (generatedPaused) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/AudioQueueStart(generatedQueue, NULL);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedClient, pad);
		...
		HRESULT hr = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(render, resample_frames, output);
		if (((HRESULT)(hr) < 0)) {
			;
		} 
		Object generatedChannels = monitor.getChannels();
		if (!muted) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(output, resample_data[0], resample_frames * generatedChannels * /*Error: Unsupported expression*/);
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(render, resample_frames, muted ? AUDCLNT_BUFFERFLAGS_SILENT : 0);
		audio_monitor monitor = param;
		double generatedUser_volume = this.getUser_volume();
		double vol = generatedUser_volume;
		 bytes = new ();
	...
	******************************************************************************/
	...
	public void deinterlace_get_closest_frames(Object sys_time) {
		...
		if (ModernizedCProgram.s.first_frame() || ModernizedCProgram.s.ready_deinterlace_frames(sys_time)) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase(ModernizedCProgram.s.getAsync_frames(), 0);
			if (ModernizedCProgram.s.getCur_async_frame().getPrev_frame()) {
				...
				ModernizedCProgram.s.setCur_async_frame(ModernizedCProgram.s.getAsync_frames().getArray()[0]);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase(ModernizedCProgram.s.getAsync_frames(), 0);
				ModernizedCProgram.s.setDeinterlace_half_duration(/*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME((ModernizedCProgram.s.getCur_async_frame().getTimestamp() - ModernizedCProgram.s.getPrev_async_frame().getTimestamp()) / 2));
			} else {
					ModernizedCProgram.s.setDeinterlace_half_duration(/*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME((ModernizedCProgram.s.getCur_async_frame().getTimestamp() - ModernizedCProgram.s.getDeinterlace_frame_ts()) / 2));
			} 
			if (!ModernizedCProgram.s.getLast_frame_ts()) {
	...
	public void allocate_audio_output_buffer() {
		 size = /*Error: Unsupported expression*/ * AUDIO_OUTPUT_FRAMES * MAX_AUDIO_CHANNELS * MAX_AUDIO_MIXES;
	...
	public void allocate_audio_mix_buffer() {
		 size = /*Error: Unsupported expression*/ * AUDIO_OUTPUT_FRAMES * MAX_AUDIO_CHANNELS;
		double ptr = ModernizedCProgram.bzalloc(size);
	...
	public Object obs_source_init() {
		...
		this.setControl(ModernizedCProgram.bzalloc(/*Error: Unsupported expression*/));
	...
	public obs_source obs_source_create_internal(Object id, Object name, obs_data settings, obs_data hotkey_data, Object private, Object last_obs_ver) {
		obs_source source = ModernizedCProgram.bzalloc(/*Error: Unsupported expression*/);
		...
		if (info) {
			...
			if (info.getGet_defaults2()) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(info.getType_data(), generatedSettings);
			} 
		} 
		...
		if (info && info.getCreate()) {
			generatedContext.setData(/*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedSettings, source));
		} 
	...
	public void duplicate_filters(obs_source src, Object private) {
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_init(filters);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_copy(filters, generatedFilters);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(filters);
	}
	...
	public void obs_source_destroy() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(generatedAudio_cb_list);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(generatedAsync_cache);
		Object generatedAsync_frames = this.getAsync_frames();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(generatedAsync_frames);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_free(generatedFilters);
		Object generatedFilter_mutex = this.getFilter_mutex();
	...
	public void obs_source_send_mouse_click(Object event, Object type, Object mouse_up, Object click_count) {
		...
		if (generatedOutput_flags & (1 << 5)) {
			if (generatedMouse_click) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, event, type, mouse_up, click_count);
			} 
		} 
	}
	public void obs_source_send_mouse_move(Object event, Object mouse_leave) {
		...
		if (generatedOutput_flags & (1 << 5)) {
			if (generatedMouse_move) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, event, mouse_leave);
			} 
		} 
	}
	public void obs_source_send_mouse_wheel(Object event, int x_delta, int y_delta) {
		...
		if (generatedOutput_flags & (1 << 5)) {
			if (generatedMouse_wheel) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, event, x_delta, y_delta);
			} 
		} 
	}
	public void obs_source_send_focus(Object focus) {
		...
		if (generatedOutput_flags & (1 << 5)) {
			if (generatedFocus) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, focus);
			} 
		} 
	}
	public void obs_source_send_key_click(Object event, Object key_up) {
		...
		if (generatedOutput_flags & (1 << 5)) {
			if (generatedKey_click) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, event, key_up);
			} 
		} 
	}
	...
	public void activate_tree(obs_source child, Object param) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/UNUSED_PARAMETER(parent);
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/UNUSED_PARAMETER(param);
	}
	...
	public void obs_source_video_tick(double seconds) {
		...
		if (generatedData && generatedVideo_tick) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, seconds);
		} 
	...
	public void source_signal_audio_data(Object in, Object muted) {
		...
		for ( i = generatedAudio_cb_list.getNum();
		 i > 0; i--) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedParam, source, in, muted);
		}
	...
	public void source_output_audio_place(Object in) {
		 ...
		 size = in.getFrames() * /*Error: Unsupported expression*/;
		...
		buf_placement = ModernizedCProgram.get_buf_placement(audio, in.getTimestamp() - generatedAudio_ts) * /*Error: Unsupported expression*/;
		if ((buf_placement + size) > (1000 * AUDIO_OUTPUT_FRAMES * /*Error: Unsupported expression*//* do not allow the circular buffers to become too big */)) {
			return /*Error: Unsupported expression*/;
		} 
		Object generatedAudio_input_buf = this.getAudio_input_buf();
		for ( i = 0;
	...
	public void source_output_audio_push_back(Object in) {
		 ...
		 size = in.getFrames() * /*Error: Unsupported expression*/;
		Object generatedAudio_input_buf = this.getAudio_input_buf();
		if ((generatedAudio_input_buf[0].getCirclebuf() + size) > (1000 * AUDIO_OUTPUT_FRAMES * /*Error: Unsupported expression*//* do not allow the circular buffers to become too big */)) {
			return /*Error: Unsupported expression*/;
		} 
		for ( i = 0;
		 i < channels; i++) {
	...
	public Object init_gpu_conversion(Object frame) {
		...
		case convert_type.CONVERT_NONE:
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/assert(false && "No conversion requested");
				break;
	...
	public Object update_async_texrender(Object frame, Object[][] tex, Object texrender) {
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/GS_DEBUG_MARKER_BEGIN(GS_DEBUG_COLOR_CONVERT_FORMAT, "Convert Format");
		...
		if (success) {
			...
			if (!frame.getFull_range()) {
				...
				min_param.gs_effect_set_val(frame.getColor_range_min(), /*Error: Unsupported expression*/ * 3);
				 max_param = gs_effect_param.gs_effect_get_param_by_name(conv, "color_range_max");
				max_param.gs_effect_set_val(frame.getColor_range_max(), /*Error: Unsupported expression*/ * 3);
			} 
	...
	public void obs_source_default_render() {
		...
		for (i = 0; i < passes; i++) {
			...
			if (generatedData) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, effect);
			} 
			tech.gs_technique_end_pass();
		}
		tech.gs_technique_end();
	...
	public void obs_source_main_render() {
		...
		if (default_effect) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, custom_draw ? NULL : gs_effect.gs_get_effect());
		} 
	}
	public void render_video() {
		...
		Object generatedRendering_filter = this.getRendering_filter();
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/GS_DEBUG_MARKER_BEGIN_FORMAT(GS_DEBUG_COLOR_SOURCE, /*Error: Function owner not recognized*/get_type_format(generatedType), ModernizedCProgram.obs_source_get_name(source));
		Object generatedFilters = this.getFilters();
		Object generatedVideo_render = generatedInfo.getVideo_render();
		obs_source generatedFilter_target = this.getFilter_target();
	...
	public void obs_source_filter_add(obs_source filter) {
		...
		ModernizedCProgram.pthread_mutex_lock(generatedFilter_mutex);
		Object generatedFilters = this.getFilters();
		if (/*Error: Function owner not recognized*/da_find(generatedFilters, filter, 0) != DARRAY_INVALID) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_insert(generatedFilters, 0, filter);
		ModernizedCProgram.pthread_mutex_unlock(generatedFilter_mutex);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "filter_add", cd);
		Byte generatedName = generatedContext.getName();
	...
	public Object obs_source_filter_remove_refless(obs_source filter) {
		...
		ModernizedCProgram.pthread_mutex_lock(generatedFilter_mutex);
		Object generatedFilters = this.getFilters();
		idx = /*Error: Function owner not recognized*/da_find(generatedFilters, filter, 0);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase(generatedFilters, idx);
		ModernizedCProgram.pthread_mutex_unlock(generatedFilter_mutex);
		cd.calldata_init_fixed(stack, /*Error: sizeof expression not supported yet*/);
		cd.calldata_set_ptr("source", source);
		cd.calldata_set_ptr("filter", filter);
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "filter_remove", cd);
		Byte generatedName = generatedContext.getName();
		obs_source_info generatedInfo = filter.getInfo();
		Object generatedId = generatedInfo.getId();
		...
		if (generatedFilter_remove) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, generatedFilter_parent);
		} 
	...
	public Object move_filter_dir(obs_source filter, obs_order_movement movement) {
		...
		if (obs_order_movement.movement == obs_order_movement.OBS_ORDER_MOVE_UP) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_move_item(generatedFilters, idx, next_id);
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_move_item(generatedFilters, idx, prev_id);
		}  else if (obs_order_movement.movement == obs_order_movement.OBS_ORDER_MOVE_TOP) {
			if (idx == generatedFilters.getNum() - 1) {
				return false;
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_move_item(generatedFilters, idx, generatedFilters.getNum() - 1);
		}  else if (obs_order_movement.movement == obs_order_movement.OBS_ORDER_MOVE_BOTTOM) {
			if (idx == 0) {
				return false;
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_move_item(generatedFilters, idx, 0);
		} 
		for ( i = 0;
	...
	public void free_async_cache() {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_resize(generatedAsync_cache, 0);
		Object generatedAsync_frames = this.getAsync_frames();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_resize(generatedAsync_frames, 0);
		this.setCur_async_frame(NULL);
		this.setPrev_async_frame(NULL);
	}
		 ...
		 i > 0; i--) {
			...
			if (!generatedUsed) {
				if (++generatedUnused_count == 5) {
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase(generatedAsync_cache, i - 1);
				} 
			} 
		}
	}
		...
		if (output) {
			if (ModernizedCProgram.os_atomic_dec_long(generatedRefs) == 0) {
				...
				output = NULL;
					...
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back(generatedAsync_frames, output);
	...
	public void obs_source_output_video2(Object frame) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedColor_matrix, generatedColor_matrix, /*Error: sizeof expression not supported yet*/);
		Object generatedColor_range_min = new_frame.getColor_range_min();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedColor_range_min, generatedColor_range_min, /*Error: sizeof expression not supported yet*/);
		Object generatedColor_range_max = new_frame.getColor_range_max();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedColor_range_max, generatedColor_range_max, /*Error: sizeof expression not supported yet*/);
		source.obs_source_output_video_internal(new_frame);
	}
	...
	public void copy_audio_data(Object[][] data, Object frames, Object ts) {
		...
		for ( i = 0;
		 i < planes; i++) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedData[i], data[i], size);
		}
		if (resize) {
	...
	public void process_audio_balancing(Object frames, double balance, obs_balance_type type) {
		...
		case obs_balance_type.OBS_BALANCE_TYPE_SINE_LAW:
				for (uint32_t frame = 0;
				 frame < frames; frame++) {
					data[0][frame] = data[0][frame] * /*Error: Function owner not recognized*/sinf((1.0 - balance) * (3.141592653589793 / 2.0));
					data[1][frame] = data[1][frame] * /*Error: Function owner not recognized*/sinf(balance * (3.141592653589793 / 2.0));
				}
				break;
		case obs_balance_type.OBS_BALANCE_TYPE_SQUARE_LAW:
				for (uint32_t frame = 0;
				 frame < frames; frame++) {
					data[0][frame] = data[0][frame] * /*Error: Function owner not recognized*/sqrtf(1.0 - balance);
					data[1][frame] = data[1][frame] * /*Error: Function owner not recognized*/sqrtf(balance);
				}
				break;
		default:
				break;
	...
	public void process_audio(Object audio) {
		uint32_t frames = audio.getFrames();
		 mono_output = new ();
		resample_info generatedSample_info = this.getSample_info();
		Object generatedResample_info = generatedSample_info.getResample_info();
		if (generatedResample_info != audio.getSamples_per_sec() || generatedResample_info != audio.getFormat() || generatedResample_info != audio.getSpeakers()) {
			source.reset_resampler(audio);
		} 
		Object generatedAudio_failed = this.getAudio_failed();
		if (generatedAudio_failed) {
			return /*Error: Unsupported expression*/;
		} 
		Object generatedResampler = this.getResampler();
		Object generatedResample_offset = this.getResample_offset();
		if (generatedResampler) {
			uint8_t[] output = new uint8_t();
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(output, 0, /*Error: sizeof expression not supported yet*/);
			generatedResampler.audio_resampler_resample(output, frames, generatedResample_offset, audio.getData(), audio.getFrames());
			source.copy_audio_data((uint8_t)output, frames, audio.getTimestamp());
		} else {
				source.copy_audio_data(audio.getData(), audio.getFrames(), audio.getTimestamp());
		} 
		mono_output = ModernizedCProgram.audio_output_get_channels(ModernizedCProgram.obs.getAudio().getAudio()) == 1;
		double generatedBalance = this.getBalance();
		if (!mono_output && generatedResample_info == SPEAKERS_STEREO && (generatedBalance > 0.51 || generatedBalance < 0.49)) {
			source.process_audio_balancing(frames, generatedBalance, obs_balance_type.OBS_BALANCE_TYPE_SINE_LAW);
		} 
		Object generatedFlags = this.getFlags();
		if (!mono_output && (generatedFlags & (1 << 1)) != 0) {
			source.downmix_to_mono_planar(frames);
		} 
		Object generatedAudio_pending = child.getAudio_pending();
		 valid = child && !generatedAudio_pending;
		...
		Object generatedAudio_ts = child.getAudio_ts();
		ts = generatedAudio_ts;
		child_audio.obs_source_get_audio_mix(child);
		pos = ()ModernizedCProgram.ns_to_audio_frames(sample_rate, ts - min_ts);
		if (pos > AUDIO_OUTPUT_FRAMES) {
			return /*Error: Unsupported expression*/;
		} 
		Object generatedOutput = audio.getOutput();
		Object generatedAudio_output_data = output.getAudio_output_data();
		for ( mix_idx = 0;
		 mix_idx < MAX_AUDIO_MIXES; mix_idx++) {
			audio_output_data output = generatedOutput[mix_idx];
			audio_output_data input = generatedOutput[mix_idx];
			if ((mixers & (1 << mix_idx)) == 0) {
				continue;
			} 
			for ( ch = 0;
			 ch < channels; ch++) {
				double out = generatedAudio_output_data[ch];
				double in = generatedAudio_output_data[ch];
				transition.mix_child(out + pos, in, AUDIO_OUTPUT_FRAMES - pos, sample_rate, ts, mix);
			}
		}
	}
	...
	public void obs_source_set_name(Object name) {
		...
		Object generatedPrivate = generatedContext.getPrivate();
		Object generatedSignals = generatedContext.getSignals();
		if (!name || !name || !generatedName || /*Error: Function owner not recognized*/strcmp(name, generatedName) != 0) {
			...
			if (!generatedPrivate) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(ModernizedCProgram.obs.getSignals(), "source_rename", ModernizedCProgram.data);
			} 
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "rename", ModernizedCProgram.data);
	...
	public void obs_source_set_volume(double volume) {
		...
		if (ModernizedCProgram.obs_object_valid(source, "obs_source_set_volume", "source")) {
			...
			calldata data = new calldata();
			uint8_t[] stack = new uint8_t();
			ModernizedCProgram.data.calldata_init_fixed(stack, /*Error: sizeof expression not supported yet*/);
			ModernizedCProgram.data.calldata_set_ptr("source", source);
			ModernizedCProgram.data.calldata_set_float("volume", volume);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "volume", ModernizedCProgram.data);
			if (!generatedPrivate) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "source_volume", ModernizedCProgram.data);
			} 
	...
	public void obs_source_set_sync_offset(Object offset) {
		...
		if (ModernizedCProgram.obs_object_valid(source, "obs_source_set_sync_offset", "source")) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "audio_sync", ModernizedCProgram.data);
	...
	public void enum_source_active_tree_callback(obs_source child, Object param) {
		...
		if (generatedEnum_active_sources) {
			if (generatedData) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, enum_source_active_tree_callback, data);
			} 
		} 
		Object generatedParam = data.getParam();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(parent, child, generatedParam);
	}
	public void obs_source_enum_active_sources(Object enum_callback, Object param) {
		...
		Object[] generatedData = generatedContext.getData();
		if (generatedEnum_active_sources) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, enum_callback, param);
		} 
		source.obs_source_release();
	}
	...
	public void enum_source_full_tree_callback(obs_source child, Object param) {
		...
		if (generatedEnum_all_sources) {
			if (generatedData) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, enum_source_full_tree_callback, data);
			} 
	...
	public void signal_flags_updated() {
		calldata data = new calldata();
		uint8_t[] stack = new uint8_t();
		data.calldata_init_fixed(stack, /*Error: sizeof expression not supported yet*/);
		data.calldata_set_ptr("source", source);
		Object generatedFlags = this.getFlags();
		data.calldata_set_int("flags", generatedFlags);
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "update_flags", data);
	}
	...
	public void obs_source_set_audio_mixers(Object mixers) {
		...
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "audio_mixers", data);
	...
	public void obs_source_enum_filters(Object callback, Object param) {
		...
		for ( i = generatedFilters.getNum();
		 i > 0; i--) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/callback(source, filter, param);
		}
		ModernizedCProgram.pthread_mutex_unlock(generatedFilter_mutex);
	}
	...
	public void obs_source_set_enabled(Object enabled) {
		...
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "enable", data);
	}
	public void obs_source_set_muted(Object muted) {
		...
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, "mute", data);
		Object generatedAudio_actions_mutex = this.getAudio_actions_mutex();
		ModernizedCProgram.pthread_mutex_lock(generatedAudio_actions_mutex);
		Object generatedAudio_actions = this.getAudio_actions();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back(generatedAudio_actions, action);
	...
	public void source_signal_push_to_changed(Object signal, Object enabled) {
		...
		obs_context_data generatedContext = this.getContext();
		Object generatedSignals = generatedContext.getSignals();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, signal, data);
	}
	...
	public void apply_audio_actions(Object channels, Object sample_rate) {
		double vol_data = /*Error: Function owner not recognized*/malloc(/*Error: Unsupported expression*/ * AUDIO_OUTPUT_FRAMES);
		Object generatedAudio_ts = this.getAudio_ts();
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/free(vol_data);
	}
	public void apply_audio_volume(Object mixers, Object channels, Object sample_rate) {
		...
		Object generatedAudio_output_buf = this.getAudio_output_buf();
		if (vol == 0.0 || mixers == 0) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedAudio_output_buf[0][0], 0, AUDIO_OUTPUT_FRAMES * /*Error: Unsupported expression*/ * MAX_AUDIO_CHANNELS * MAX_AUDIO_MIXES);
			return /*Error: Unsupported expression*/;
		} 
		Object generatedAudio_mixers = this.getAudio_mixers();
		for ( mix = 0;
		...
		for ( mix = 0;
		 mix < MAX_AUDIO_MIXES; mix++) {
			...
			if ((generatedAudio_mixers & mixers & (1 << mix)) != 0) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedAudio_output_buf[mix][0], 0, /*Error: Unsupported expression*/ * AUDIO_OUTPUT_FRAMES * channels);
			} 
		}
		obs_context_data generatedContext = this.getContext();
		Object[] generatedData = generatedContext.getData();
		success = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, ts, audio_data, mixers, channels, sample_rate);
	...
	public void audio_submix(Object channels, Object sample_rate) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedAudio_mix_buf[0], 0, /*Error: Unsupported expression*/ * AUDIO_OUTPUT_FRAMES * channels);
		obs_context_data generatedContext = this.getContext();
		Object[] generatedData = generatedContext.getData();
		success = /*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(generatedData, ts, audio_data, channels, sample_rate);
		if (!success) {
			return /*Error: Unsupported expression*/;
		} 
		for ( i = 0;
	...
	public void process_audio_source_tick(Object mixers, Object channels, Object sample_rate, Object size) {
		...
		for ( mix = 1;
		 mix < MAX_AUDIO_MIXES; mix++) {
			...
			if ((generatedAudio_mixers & mix_and_val) == 0 || (mixers & mix_and_val) == 0) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedAudio_output_buf[mix][0], 0, size * channels);
				continue;
			} 
			for ( ch = 0;
			 ch < channels; ch++) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memcpy(generatedAudio_output_buf[mix][ch], generatedAudio_output_buf[0][ch], size);
			}
		}
		if (audio_submix) {
		...
		if ((generatedAudio_mixers & 1) == 0 || (mixers & 1) == 0) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedAudio_output_buf[0][0], 0, size * channels);
		} 
		source.apply_audio_volume(mixers, channels, sample_rate);
		this.setAudio_pending(false);
	}
	...
	public void obs_source_add_audio_capture_callback(Object callback, Object param) {
		...
		ModernizedCProgram.pthread_mutex_lock(generatedAudio_cb_mutex);
		Object generatedAudio_cb_list = this.getAudio_cb_list();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back(generatedAudio_cb_list, info);
		ModernizedCProgram.pthread_mutex_unlock(generatedAudio_cb_mutex);
	}
	public void obs_source_remove_audio_capture_callback(Object callback, Object param) {
		...
		ModernizedCProgram.pthread_mutex_lock(generatedAudio_cb_mutex);
		Object generatedAudio_cb_list = this.getAudio_cb_list();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_erase_item(generatedAudio_cb_list, info);
		ModernizedCProgram.pthread_mutex_unlock(generatedAudio_cb_mutex);
	}
	...
	public void obs_source_dosignal(Object signal_obs, Object signal_source) {
		...
		if (signal_obs && !generatedPrivate) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(ModernizedCProgram.obs.getSignals(), signal_obs, data);
		} 
		Object generatedSignals = generatedContext.getSignals();
		if (signal_source) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(generatedSignals, signal_source, data/* maximum timestamp variance in nanoseconds */);
		} 
	}
	...
	public void obs_set_output_source(Object channel) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/signal_handler_signal(ModernizedCProgram.obs.getSignals(), "channel_change", params);
	...
	public void sidechain_capture(Object param, Object audio_data, Object muted) {
		...
		if (generatedMax_sidechain_frames < audio_data.getFrames()) {
		 ...
		 expected_size = generatedMax_sidechain_frames * /*Error: Unsupported expression*/;
		if (!expected_size) {
			;
		} 
		...
		if (muted) {
			for ( i = 0;
			 i < generatedNum_channels; i++) {
				generatedSidechain_data[i].circlebuf_push_back_zero(audio_data.getFrames() * /*Error: Unsupported expression*/);
			}
		} else {
				for ( i = 0;
				 i < generatedNum_channels; i++) {
					generatedSidechain_data[i].circlebuf_push_back(audio_data.getData()[i], audio_data.getFrames() * /*Error: Unsupported expression*/);
				}
		} 
	}
	...
	public obs_source get_source(Object array, Object path) {
		...
		for ( i = 0;
		 i < files.getNum(); i++) {
			...
			if (/*Error: Function owner not recognized*/strcmp(path, cur_path) == 0) {
	...
	public void push_audio_tree(obs_source source, Object p) {
		...
		if (/*Error: Function owner not recognized*/da_find(generatedRender_order, source, 0) == DARRAY_INVALID) {
			obs_source_t s = source.obs_source_get_ref();
			if (ModernizedCProgram.s) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/da_push_back(generatedRender_order, ModernizedCProgram.s);
			} 
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/UNUSED_PARAMETER(parent);
	}
	public void ignore_audio(Object channels, Object sample_rate) {
		Object generatedAudio_input_buf = this.getAudio_input_buf();
		 num_floats = generatedAudio_input_buf[0].getCirclebuf() / /*Error: Unsupported expression*/;
		Object generatedAudio_ts = this.getAudio_ts();
		if (num_floats) {
			for ( ch = 0;
			 ch < channels; ch++) {
	...
	public Object audio_buffer_insuffient(Object sample_rate, Object min_ts) {
		...
		size = total_floats * /*Error: Unsupported expression*/;
		Object generatedAudio_input_buf = this.getAudio_input_buf();
		if (generatedAudio_input_buf[0].getCirclebuf() < size) {
			this.setAudio_pending(true);
			return true;
	...
	public void set_source(obs_transition_target target, obs_source new_child, Object callback) {
		...
		if (callback && add_success) {
			add_success = /*Error: Function owner not recognized*/callback(transition, idx, new_child);
		} 
	...
	public Object obs_transition_start(obs_transition_mode mode, Object duration_ms, obs_source dest) {
		...
		transition.recalculate_transition_size();
		transition.recalculate_transition_matrices();
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/UNUSED_PARAMETER(/* TODO: Add mode */obs_transition_mode.mode);
		return true;
	}
	...
	public void enum_child(obs_source child, Object enum_callback, Object param) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/enum_callback(tr, child, param);
	}
		...
		for ( i = 0;
		 i < 2; i++) {
			if (generatedTransition_sources[i]) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/cb(transition, generatedTransition_sources[i], param);
			} 
		}
		ModernizedCProgram.pthread_mutex_unlock(generatedTransition_mutex);
		;
	...
	public void obs_transition_video_render(Object callback) {
		...
		if (generatedTransitioning_video && locked && callback) {
			...
			if (cx && cy) {
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/callback(generatedData, tex[0], tex[1], t, cx, cy);
				ModernizedCProgram.gs_blend_state_pop();
			} 
	...
	public void mix_child(double[] out, double[] in, Object count, Object sample_rate, Object ts, Object mix) {
		...
		for ( i = 0;
		 i < count; i++) {
			double t = ModernizedCProgram.get_sample_time(transition, sample_rate, i, ts);
			out[i] += in[i] * /*Error: Function owner not recognized*/mix(context_data, t);
		}
	}
