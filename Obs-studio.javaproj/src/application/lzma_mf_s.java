...
public class lzma_mf_s {
	...
	public void move_window() {
		...
		// like LZMA use the lowest bits of mf->read_pos to know the// alignment of the uncompressed data. We also get better speed// for memmove() with aligned buffers.// like LZMA use the lowest bits of mf->read_pos to know the// alignment of the uncompressed data. We also get better speed// for memmove() with aligned buffers.((generatedRead_pos > generatedKeep_size_before) ? (Object)0 : ._assert("mf->read_pos > mf->keep_size_before", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder.c", 50));
		uint32_t move_offset = (generatedRead_pos - generatedKeep_size_before) & ~-1024;
		Object generatedWrite_pos = this.getWrite_pos();
		((generatedWrite_pos > move_offset) ? (Object)0 : ._assert("mf->write_pos > move_offset", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder.c", 54));
		size_t move_size = generatedWrite_pos - move_offset;
		Object generatedSize = this.getSize();
		((move_offset + move_size <= generatedSize) ? (Object)0 : ._assert("move_offset + move_size <= mf->size", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder.c", 57));
		Object generatedBuffer = this.getBuffer();
		.memmove(generatedBuffer, generatedBuffer + move_offset, move_size);
		...
		return ;/// \brief      Tries to fill the input window (mf->buffer)////// If we are the last encoder in the chain, our input data is in in[].
	}
	...
	public boolean lz_encoder_init(Object allocator, Object lz_options) {
		...
		// Check for integer overflow. (Huge dictionaries are not// possible on 32-bit CPU.)if (alloc_count > -1024 / ) {
			return 1;
		} 
		Object generatedHash = this.getHash();
		if (generatedHash == ((Object)0)) {
			this.setHash(ModernizedCProgram.lzma_alloc(alloc_count * , allocator));
			if (generatedHash == ((Object)0)) {
				return 1;
			} 
		} 
		...
		.memset(generatedHash, 0, (size_t)(generatedHash_size_sum) * /*
		...
		*/)// We don't need to initialize mf->son, but not doing that will// make Valgrind complain in normalization (see normalize() in// lz_encoder_mf.c).//// Skipping this initialization is *very* good when big dictionary is;// We don't need to initialize mf->son, but not doing that will// make Valgrind complain in normalization (see normalize() in// lz_encoder_mf.c).//// Skipping this initialization is *very* good when big dictionary is
		...
		// used but only small amount of data gets actually compressed: most// of the mf->hash won't get actually allocated by the kernel, so// we avoid wasting RAM and improve initialization speed a lot.//memzero(mf->son, (size_t)(mf->sons_count) * sizeof(uint32_t));// Handle preset dictionary.if (lz_options.getPreset_dict() != ((Object)0) && lz_options.getPreset_dict_size() > 0) {
			...
			.memcpy(generatedBuffer, lz_options.getPreset_dict() + lz_options.getPreset_dict_size() - generatedWrite_pos, generatedWrite_pos);
			this.setAction(LZMA_SYNC_FLUSH);
			.UNRECOGNIZEDFUNCTIONNAME(mf, generatedWrite_pos);
		} 
	...
	public void normalize() {
		Object generatedRead_pos = this.getRead_pos();
		Object generatedOffset = this.getOffset();
		((generatedRead_pos + generatedOffset == -1024) ? (Object)0 : ._assert("mf->read_pos + mf->offset == MUST_NORMALIZE_POS", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder_mf.c", 111))// In future we may not want to touch the lowest bits, because there;// In future we may not want to touch the lowest bits, because there
		Object generatedCyclic_size = this.getCyclic_size();
	...
	public void move_pos() {
		...
		++generatedRead_pos;
		Object generatedWrite_pos = this.getWrite_pos();
		((generatedRead_pos <= generatedWrite_pos) ? (Object)0 : ._assert("mf->read_pos <= mf->write_pos", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder_mf.c", 154));
		Object generatedOffset = this.getOffset();
		if (.__builtin_expect(generatedRead_pos + generatedOffset == -1024, 0)) {
			mf.normalize();
		} 
	...
	public void move_pending() {
		...
		++generatedRead_pos;
		Object generatedWrite_pos = this.getWrite_pos();
		((generatedRead_pos <= generatedWrite_pos) ? (Object)0 : ._assert("mf->read_pos <= mf->write_pos", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder_mf.c", 179));
	...
	public void mf_skip(Object amount) {
		...
		if (amount != 0) {
			.UNRECOGNIZEDFUNCTIONNAME(mf, amount);
	...
	public void mf_read(Object out, Object out_pos, Object out_size, Object left) {
		...
		Object generatedRead_ahead = this.getRead_ahead();
		((generatedRead_ahead == 0) ? (Object)0 : ._assert("mf->read_ahead == 0", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder.h", 286));
		Object generatedRead_pos = this.getRead_pos();
		((generatedRead_pos >= left) ? (Object)0 : ._assert("mf->read_pos >= *left", "E:\\Programfiles\\Eclipse\\Workspaces\\runtime-EclipseApplication\\Obs-studio\\src\\lz_encoder.h", 287));
		Object generatedBuffer = this.getBuffer();
		.memcpy(out + out_pos, generatedBuffer + generatedRead_pos - left, copy_size);
		out_pos += copy_size;
		left -= copy_size;
		return ;
	}
