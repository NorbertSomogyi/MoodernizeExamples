...
public class Curl_tree {
	...
	public Curl_tree[] Curl_splay(curltime i, Curl_tree[] t) {
		...
		Curl_tree[] generatedSmaller = t.getSmaller();
	...
	public Curl_tree[] Curl_splayinsert(curltime i, Curl_tree[] t, Curl_tree[] node) {
		curltime KEY_NOTUSED = new curltime((time_t)-1, (int)-1/* will *NEVER* appear */);
		if (node == ((Object)0)) {
			return t;
		} 
		Curl_tree curl_tree = new Curl_tree();
		Object generatedTv_sec = (i).getTv_sec();
		int generatedTv_usec = (i).getTv_usec();
		Curl_tree[] generatedSamep = t.getSamep();
		if (t != ((Object)0)) {
			t = curl_tree.Curl_splay(i, t);
		...
		Curl_tree[] generatedSmaller = t.getSmaller();
		Curl_tree[] generatedLarger = t.getLarger();
		if (t == ((Object)0)) {
	...
	public Curl_tree[] Curl_splaygetbest(curltime i, Curl_tree[] t, Curl_tree removed) {
		...
		Curl_tree curl_tree = new Curl_tree();
		t = curl_tree.Curl_splay(tv_zero, /* find smallest */t);
		Object generatedTv_sec = (i).getTv_sec();
		int generatedTv_usec = (i).getTv_usec();
		if ((((generatedTv_sec) < (generatedTv_sec)) ? -1 : (((generatedTv_sec) > (generatedTv_sec)) ? 1 : (((generatedTv_usec) < (generatedTv_usec)) ? -1 : (((generatedTv_usec) > (generatedTv_usec)) ? 1 : 0)))) < 0) {
		...
		Curl_tree[] generatedSamen = t.getSamen();
		x = generatedSamen;
		curltime generatedKey = t.getKey();
		Curl_tree[] generatedLarger = t.getLarger();
		Curl_tree[] generatedSmaller = t.getSmaller();
		Curl_tree[] generatedSamep = t.getSamep();
	...
	public int Curl_splayremovebyaddr(Curl_tree[] t, Curl_tree[] removenode, Curl_tree newroot) {
		curltime KEY_NOTUSED = new curltime((time_t)-1, (int)-1/* will *NEVER* appear */);
		Curl_tree[] x = new Curl_tree();
		if (!t || !removenode) {
			return 1;
		...
		Curl_tree curl_tree = new Curl_tree();
		t = curl_tree.Curl_splay(generatedKey, t/* First make sure that we got the same root node as the one we want
			...
			generatedSamep.setSamen(x);
				...
				if (generatedSmaller == ((Object)/* Remove the root node */0)) {
					x = generatedLarger;
						...
						x = curl_tree.Curl_splay(generatedKey, generatedSmaller);
						x.setLarger(generatedLarger);
				} 
		} 
