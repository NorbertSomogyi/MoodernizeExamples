...
public class Curl_multi {
	...
	public Curl_multi Curl_multi_handle(int hashsize, int chashsize) {
		Curl_multi multi = /*Error: Function owner not recognized*/Curl_ccalloc(1, /*Error: Unsupported expression*/);
		if (!multi) {
			return ((Object)0);
		} 
		multi.setType(-1024);
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(multi);
		return ((Object)0);
	}
	...
	public Object Curl_multi_wait(Object[] extra_fds, int extra_nfds, int timeout_ms, Integer ret,  extrawait) {
		...
		data = generatedEasyp;
		Curl_easy generatedNext = data.getNext();
		while (data) {
		...
		if (nfds > 10/* 'nfds' is a 32 bit value and 'struct pollfd' is typically 8 bytes
			...
			ufds = /*Error: Function owner not recognized*/Curl_cmalloc(nfds * /*Error: Unsupported expression*/);
			if (!ufds) {
		...
		if (ufds_malloc) {
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(ufds);
		} 
		if (ret) {
			ret = retcode;
		} 
	...
	public Object curl_multi_perform(Integer running_handles) {
	...
	public Object curl_multi_cleanup() {
		...
		Object generatedDone = generatedState.getDone();
		connectdata generatedConn = data.getConn();
		Names generatedDns = data.getDns();
		 generatedHostcachetype = generatedDns.getHostcachetype();
		...
		if (((multi) && generatedType == -1024)) {
			...
			while (data) {
				...
				if (!generatedDone && generatedConn) {
					(Object)data.multi_done(CURLE_OK, /* if DONE was never called for this handle */1);
				} 
				if (generatedHostcachetype == .HCACHE_MULTI) {
					ModernizedCProgram.Curl_hostcache_clean(data, generatedHostcache);
	...
	public Object curl_multi_info_read(Integer msgs_in_queue) {
	...
	public Object multi_socket( checkall, Object s, int ev_bitmask, Integer running_handles) {
		 result = CURLM_OK;
		...
		connectdata generatedConn = data.getConn();
		Object generatedHandler = generatedConn.getHandler();
			...
			if (!entry/* Unmatched socket, we can't act on it but we ignore this fact.  In
				...
				;
					...
					for (he = curl_hash_element.Curl_hash_next_element(iter); he; he = curl_hash_element.Curl_hash_next_element(iter)) {
						...
						if (generatedConn && !(generatedHandler.getFlags() & (1 << 3))) {
				...
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/memset(generatedTimer_lastcall, 0, /*Error: sizeof expression not supported yet*/);
		} 
		Curl_tree generatedTimetree = this.getTimetree();
		Object generatedPayload = t.getPayload();
		do {
	...
	public void Curl_update_timer() {
		...
		if (timeout_ms < 0) {
			...
			if ((((none.getTv_sec()) < (generatedTv_sec)) ? -1 : (((generatedTv_sec) > (generatedTv_sec)) ? 1 : (((none.getTv_usec()) < (generatedTv_usec)) ? -1 : (((generatedTv_usec) > (generatedTv_usec)) ? 1 : 0))))) {
				this.setTimer_lastcall(none/* there's no timeout now but there was one previously, tell the app to
				         disable it */);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(multi, -1, generatedTimer_userp);
				return /*Error: Unsupported expression*/;
			} 
			return /*Error: Unsupported expression*/;
		} 
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/ERROR_UNRECOGNIZED_FUNCTIONNAME(multi, timeout_ms, generatedTimer_userp);
	}
