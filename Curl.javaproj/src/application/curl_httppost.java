...
public class curl_httppost {
	...
	public  curl_formadd_ccsid(curl_httppost last_post) {
		...
		lforms = /*Error: Function owner not recognized*/malloc(lformlen * /*Error: sizeof expression not supported yet*/);
		if (!lforms) {
		...
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/__builtin_va_start(arg, last_post);
		 generatedOption = forms.getOption();
		Object generatedValue = forms.getValue();
		for (; /*Error: Unsupported expression*/; /*Error: Unsupported expression*/) {
			if (nargs >= lformlen) {
				lformlen += 8;
				tforms = /*Error: Function owner not recognized*/realloc(lforms, lformlen * /*Error: sizeof expression not supported yet*/);
				if (!tforms) {
	...
	public curl_httppost AddHttpPost(Byte name, Object namelength, Byte value, Object contentslength, Byte buffer, Object bufferlength, Byte contenttype, long flags, curl_slist contentHeader, Byte showfilename, Byte userp, curl_httppost httppost, curl_httppost last_post) {
		...
		post = /*Error: Function owner not recognized*/Curl_ccalloc(1, /*Error: Unsupported expression*/);
		if (post) {
			post.setCurl_httppost(name);
			post.setCurl_httppost((long)(name ? (namelength ? namelength : /*Error: Function owner not recognized*/strlen(name)) : 0));
	...
	public Object FormAdd(curl_httppost last_post, Object params) {
		...
		bool array_state = 0/*
		   ...
		   */;
		first_form = /*Error: Function owner not recognized*/Curl_ccalloc(1, /*Error: Unsupported expression*/);
		if (!first_form) {
					...
					{ 
						...
						if (generatedShowfilename) {
							return_value = CURL_FORMADD_OPTION_TWICE;
								...
								current_form.setShowfilename(/*Error: Function owner not recognized*/Curl_cstrdup(ModernizedCProgram.filename));
								if (!generatedShowfilename) {
					...
					{ 
						...
						if (generatedContenttype) {
							if (generatedFlags & CURL_HTTPPOST_FILENAME) {
								if (contenttype) {
									byte type = /*Error: Function owner not recognized*/Curl_cstrdup(contenttype);
									if (!type) {
										return_value = CURL_FORMADD_MEMORY;
									} else {
											form = current_form.AddFormInfo(((Object)0), type);
											if (!form) {
												/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(type);
												return_value = CURL_FORMADD_MEMORY;
											} else {
								...
								if (contenttype) {
									current_form.setContenttype(/*Error: Function owner not recognized*/Curl_cstrdup(contenttype));
									if (!generatedContenttype) {
										return_value = CURL_FORMADD_MEMORY;
									} else {
						...
						if (generatedValue) {
							if (generatedFlags & CURL_HTTPPOST_FILENAME) {
								if (ModernizedCProgram.filename) {
									byte fname = /*Error: Function owner not recognized*/Curl_cstrdup(ModernizedCProgram.filename);
									if (!fname) {
										return_value = CURL_FORMADD_MEMORY;
									} else {
											form = current_form.AddFormInfo(fname, ((Object)0));
											if (!form) {
												/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(fname);
												return_value = CURL_FORMADD_MEMORY;
											} else {
								...
								if (ModernizedCProgram.filename) {
									current_form.setValue(/*Error: Function owner not recognized*/Curl_cstrdup(ModernizedCProgram.filename));
									if (!generatedValue) {
										return_value = CURL_FORMADD_MEMORY;
									} else {
		...
		if (CURL_FORMADD_OK != return_value/* On error, free allocated fields for all nodes of the FormInfo linked
			...
			for (ptr = first_form; ptr != ((Object)0); ptr = generatedMore) {
				...
				if (generatedContenttype_alloc) {
					do {
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree((generatedContenttype));
				...
				if (generatedShowfilename_alloc) {
					do {
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree((generatedShowfilename));
		...
		if (CURL_FORMADD_OK == return_value/* go through the list, check for completeness and if everything is
			...
			for (form = first_form; form != ((Object)0); form = generatedMore) {
				...
				if (((generatedFlags & CURL_HTTPPOST_FILENAME) || (generatedFlags & CURL_HTTPPOST_BUFFER)) && !generatedContenttype) {
					...
					form.setContenttype(/*Error: Function owner not recognized*/Curl_cstrdup(/* our contenttype is missing */type));
					if (!generatedContenttype) {
						return_value = CURL_FORMADD_MEMORY;
						break;
					} 
				...
				if (!(generatedFlags & CURL_HTTPPOST_PTRNAME) && (form == first_form/* Note that there's small risk that form->name is NULL here if the
					...
					if (generatedName) {
						form.setName(ModernizedCProgram.Curl_memdup(generatedName, generatedNamelength ? generatedNamelength : /*Error: Function owner not recognized*/strlen(generatedName) + 1));
					} 
					if (!generatedName) {
						return_value = CURL_FORMADD_MEMORY;
						break;
				...
				if (!(generatedFlags & (CURL_HTTPPOST_FILENAME | CURL_HTTPPOST_READFILE | CURL_HTTPPOST_PTRCONTENTS | CURL_HTTPPOST_PTRBUFFER | CURL_HTTPPOST_CALLBACK)) && generatedValue) {
					...
					if (!clen) {
						clen = /*Error: Function owner not recognized*/strlen(generatedValue) + 1;
					} 
		...
		while (/* Always deallocate FormInfo linked list nodes without touching node
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(first_form);
			first_form = ptr;
		}
		return return_value/*
	...
	public int curl_formget(Object arg, Object append) {
		...
		while (!result) {
			...
			size_t nread = ModernizedCProgram.Curl_mime_read(ModernizedCProgram.buffer, 1, /*Error: sizeof expression not supported yet*/, toppart);
			if (!nread) {
				break;
			} 
			switch (nread) {
	...
	public void curl_formfree() {
		...
		do {
			...
			if (!(generatedCurl_httppost & CURL_HTTPPOST_PTRNAME)) {
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(generatedCurl_httppost);
			} 
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(generatedCurl_httppost);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(generatedCurl_httppost);
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(/* free the struct */form);
			form = next;
		} while (/* continue */form);
	}
