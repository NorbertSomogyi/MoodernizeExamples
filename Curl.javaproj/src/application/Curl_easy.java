...
public class Curl_easy {
	...
	public Curl_easy curl_easy_init() {
		...
		return data/* the next node in the list or NULL *//* socket info of what to monitor */;/* timeout, run the timeout function when reached */
	}
	...
	public Object curl_easy_getinfo(Object info) {
		...
		paramp = (int)arg;
		result = .Curl_getinfo(ModernizedCProgram.data, info, paramp);
	...
	public Object dupset(Curl_easy src) {
		 result = CURLE_OK;
		dupstring i;
		Object generatedSet = src.getSet();
		this.setSet(generatedSet);
		dst.Curl_mime_initpart(generatedSet.getMimepost());
		.memset(generatedSet.getStr(), 0, dupstring.STRING_LAST * /* clear all string pointers first */);
		for (dupstring.i = (dupstring)0; dupstring.i < dupstring.STRING_LASTZEROTERMINATED; /* duplicate all strings */dupstring.i++) {
			result = ModernizedCProgram.Curl_setstropt(generatedSet.getStr()[dupstring.i], generatedSet.getStr()[dupstring.i]);
			if (result) {
				return result;
			} 
		}
		dupstring.i = /* duplicate memory areas pointed to */dupstring.STRING_COPYPOSTFIELDS;
		if (generatedSet.getPostfieldsize() && generatedSet.getStr()[dupstring.i]) {
			generatedSet.getStr()[dupstring.i] = ModernizedCProgram.Curl_memdup(generatedSet.getStr()[/* postfieldsize is curl_off_t, Curl_memdup() takes a size_t ... */dupstring.i], ModernizedCProgram.curlx_sotouz(generatedSet.getPostfieldsize()));
			if (!generatedSet.getStr()[dupstring.i]) {
				return CURLE_OUT_OF_MEMORY;
			} 
			generatedSet.setPostfields(generatedSet.getStr()[/* point to the new copy */dupstring.i]);
		} 
		result = ModernizedCProgram.Curl_mime_duppart(generatedSet.getMimepost(), generatedSet.getMimepost());
		Object generatedChange = this.getChange();
		if (generatedSet.getResolve()) {
			generatedChange.setResolve(generatedSet.getResolve());
		} 
		return result/*
	...
	public Curl_easy curl_easy_duphandle() {
		...
		Object generatedSet = outcurl.getSet();
		generatedSet.setBuffer_size(ModernizedCProgram.data.getSet().getBuffer_size());
		Object generatedState = outcurl.getState();
		generatedState.setBuffer(.Curl_cmalloc(generatedSet.getBuffer_size() + 1));
		if (!generatedState.getBuffer()) {
			;
		} 
		generatedState.setHeaderbuff(.Curl_cmalloc(256));
		if (!generatedState.getHeaderbuff()) {
			;
		} 
		...
		Object generatedProgress = outcurl.getProgress();
		generatedProgress.setFlags(ModernizedCProgram.data.getProgress().getFlags());
		generatedProgress.setCallback(generatedProgress.getCallback());
		Object generatedCookies = outcurl.getCookies();
		if (ModernizedCProgram.data.getCookies()) {
			outcurl.setCookies(generatedCookies.Curl_cookie_init(ModernizedCProgram.data, ModernizedCProgram.data.getCookies().getFilename(), generatedSet.getCookiesession()));
			if (!generatedCookies) {
				;
			} 
		} 
		Object generatedChange = outcurl.getChange();
		if (ModernizedCProgram.data.getChange().getCookielist()) {
			...
			if (!generatedChange.getCookielist()) {
				;
			} 
		} 
		if (generatedChange.getUrl()) {
			generatedChange.setUrl(.Curl_cstrdup(generatedChange.getUrl()));
			if (!generatedChange.getUrl()) {
				;
			} 
			generatedChange.setUrl_alloc(1);
		} 
		if (generatedChange.getReferer()) {
			generatedChange.setReferer(.Curl_cstrdup(generatedChange.getReferer()));
			if (!generatedChange.getReferer()) {
				;
			} 
			generatedChange.setReferer_alloc(1);
		} 
		if (generatedSet.getStr()[/* Reinitialize an SSL engine for the new handle
		   * note: the engine name has already been copied by dupset */dupstring.STRING_SSL_ENGINE]) {
			if (.Curl_ssl_set_engine(outcurl, generatedSet.getStr()[dupstring.STRING_SSL_ENGINE])) {
				;
			} 
		} 
		if (/* Clone the resolver handle, if present, for the new handle */outcurl.Curl_resolver_duphandle(generatedState.getResolver(), generatedState.getResolver())) {
			;
		} 
		do {
		} while (0);
	...
	public void curl_easy_reset() {
		...
		.memset(ModernizedCProgram.data.getSet(), 0, );
		(Object)ModernizedCProgram.data.Curl_init_userdefined();
		.memset(ModernizedCProgram.data.getProgress(), 0, /* zero out Progress data: */);
		.Curl_initinfo(/* zero out PureInfo data: */ModernizedCProgram.data);
		...
		.memset(ModernizedCProgram.data.getState().getAuthhost(), 0, /* zero out authentication data: */);
		.memset(ModernizedCProgram.data.getState().getAuthproxy(), 0, );
		ModernizedCProgram.data.Curl_http_auth_cleanup_digest();
		if (old_buffer_size != ModernizedCProgram.data.getSet().getBuffer_size()) {
			byte newbuff = .Curl_crealloc(ModernizedCProgram.data.getState().getBuffer(), ModernizedCProgram.data.getSet().getBuffer_size() + 1);
			if (!newbuff) {
				do {
				} while (0);
	...
	public Object Curl_share_lock(Object type, Object accesstype) {
		...
		if (generatedSpecifier & (1 << type)) {
			if (generatedLockfunc) {
				.UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.data, type, accesstype, generatedClientdata);
			} 
		} 
	...
	public Object Curl_share_unlock(Object type) {
		...
		if (generatedSpecifier & (1 << type)) {
			if (generatedUnlockfunc) {
				.UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.data, type, generatedClientdata);
			} 
		} 
		return CURLSHE_OK;
	}
	public void Curl_freeset() {
		...
		for (dupstring.i = (dupstring)0; dupstring.i < dupstring.STRING_LAST; dupstring.i++) {
			do {
				.Curl_cfree((ModernizedCProgram.data.getSet().getStr()[dupstring.i]));
				(ModernizedCProgram.data.getSet().getStr()[dupstring.i]) = ((Object)0);
			} while (0);
		}
		if (ModernizedCProgram.data.getChange().getReferer_alloc()) {
			do {
				.Curl_cfree((ModernizedCProgram.data.getChange().getReferer()));
		...
		if (ModernizedCProgram.data.getChange().getUrl_alloc()) {
			do {
				.Curl_cfree((ModernizedCProgram.data.getChange().getUrl()));
	...
	public void up_free() {
		...
		.curl_url_cleanup(ModernizedCProgram.data.getState().getUh());
	...
	public Object Curl_close() {
		...
		if (ModernizedCProgram.data.getState().getRangestringalloc()) {
			.Curl_cfree(ModernizedCProgram.data.getState().getRange());
		} 
		/* freed here just in case DONE wasn't called */ModernizedCProgram.data.Curl_free_request_state();
		.Curl_ssl_close_all(/* Close down all open SSL info and sessions */ModernizedCProgram.data);
		do {
			.Curl_cfree((ModernizedCProgram.data.getState().getFirst_host()));
			(ModernizedCProgram.data.getState().getFirst_host()) = ((Object)0);
		} while (0);
		do {
			.Curl_cfree((ModernizedCProgram.data.getState().getScratch()));
			(ModernizedCProgram.data.getState().getScratch()) = ((Object)0);
		} while (0);
		.Curl_ssl_free_certinfo(ModernizedCProgram.data);
		.Curl_cfree(ModernizedCProgram.data.getReq().getNewurl());
		...
		do {
			.Curl_cfree((ModernizedCProgram.data.getState().getBuffer()));
			(ModernizedCProgram.data.getState().getBuffer()) = ((Object)0);
		} while (0);
		do {
			.Curl_cfree((ModernizedCProgram.data.getState().getHeaderbuff()));
			(ModernizedCProgram.data.getState().getHeaderbuff()) = ((Object)0);
		} while (0);
		do {
			.Curl_cfree((ModernizedCProgram.data.getState().getUlbuf()));
			(ModernizedCProgram.data.getState().getUlbuf()) = ((Object)0);
		} while (0);
		ModernizedCProgram.data.Curl_flush_cookies(1);
		ModernizedCProgram.data.Curl_http_auth_cleanup_digest();
		do {
			.Curl_cfree((ModernizedCProgram.data.getInfo().getContenttype()));
			(ModernizedCProgram.data.getInfo().getContenttype()) = ((Object)0);
		} while (0);
		do {
			.Curl_cfree((ModernizedCProgram.data.getInfo().getWouldredirect()));
		...
		.Curl_cfree(ModernizedCProgram.data.getReq().getDoh().getProbe()[0].getServerdoh().getMemory());
		.Curl_cfree(ModernizedCProgram.data.getReq().getDoh().getProbe()[1].getServerdoh().getMemory());
		...
		.Curl_cfree(ModernizedCProgram.data);
		return CURLE_OK/*
	...
	public Object Curl_open() {
		...
		result = data.Curl_resolver_init(generatedState.getResolver());
		if (result) {
			do {
			} while (0);
			.Curl_cfree(data);
		...
		generatedState.setBuffer(.Curl_cmalloc(CURL_MAX_WRITE_SIZE + /* We do some initial setup here, all those fields that can't be just 0 */1));
			...
			result = CURLE_OUT_OF_MEMORY;
				...
				if (!generatedState.getHeaderbuff()) {
					...
					result = CURLE_OUT_OF_MEMORY;
						...
						generatedProgress.getFlags() |=  (1 << 4);
						generatedState.setCurrent_speed(-/* init to negative == impossible */1);
				} 
		} 
		if (result) {
			ModernizedCProgram.Curl_resolver_cleanup(generatedState.getResolver());
			.Curl_cfree(generatedState.getBuffer());
			.Curl_cfree(generatedState.getHeaderbuff());
			data.Curl_freeset();
			.Curl_cfree(data);
			data = ((Object)0);
		} else {
	...
	public void Curl_free_request_state() {
		do {
			.Curl_cfree((ModernizedCProgram.data.getReq().getProtop()));
	...
	public Object parse_connect_to_host_port(Object host, byte hostname_result, int port_result) {
		byte host_dup;
		byte hostptr;
		byte host_portno;
		byte portptr;
		...
		if (hostptr == /* detect and extract RFC6874-style IPv6-addresses */(byte)'['/* advance beyond the initial bracket *//* There might be a zone identifier *//* Allow unreserved characters as defined in RFC 3986 */) {
			ModernizedCProgram.data.Curl_failf(/* yeps, it ended nicely with a bracket as well *//* Note that if this didn't end with a bracket, we still advanced the
	...
	public Object Curl_connect( asyncp,  protocol_done) {
		...
		.memset(ModernizedCProgram.data.getReq(), 0, );
	...
	public void Curl_speedinit() {
		.memset(ModernizedCProgram.data.getState().getKeeps_speed(), 0, /*
		 * @unittest: 1606
		 */);
	}
	...
	public void Curl_failf(Object fmt) {
		if (ModernizedCProgram.data.getSet().getVerbose() || ModernizedCProgram.data.getSet().getErrorbuffer()) {
			...
			if (ModernizedCProgram.data.getSet().getErrorbuffer() && !ModernizedCProgram.data.getState().getErrorbuf()) {
				.strcpy(ModernizedCProgram.data.getSet().getErrorbuffer(), error);
	...
	public Object pausewrite(int type, Object ptr, Object len) {
		...
		if (!newtype/* append new data to old data */) {
			...
			byte newptr = .Curl_crealloc(generatedTempwrite[i].getBuf(), /* allocate the new memory area */newlen);
			if (!newptr) {
				return CURLE_OUT_OF_MEMORY;
			} 
			.memcpy(newptr + generatedTempwrite[i].getLen(), ptr, /* copy the new data to the end of the new area */len);
	...
	public int Curl_debug(Object type, Byte ptr, Object size) {
		...
		/* Special processing is needed for this block if it
		     ...
		     */
		/* convert everything through this CRLFCRLF but no further */if (ModernizedCProgram.data.getSet().getFdebug()) {
			ModernizedCProgram.data.Curl_set_in_callback(.bool_true);
			rc = .UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.data, type, ptr, size, ModernizedCProgram.data.getSet().getDebugdata());
			ModernizedCProgram.data.Curl_set_in_callback(.bool_false);
		} else {
				switch (type) {
				case CURLINFO_HEADER_OUT:
				case CURLINFO_HEADER_IN:
						.fwrite(s_infotype[type], 2, 1, ModernizedCProgram.data.getSet().getErr());
						.fwrite(ptr, size, 1, ModernizedCProgram.data.getSet().getErr());
						break;
				case CURLINFO_TEXT:
				default:
						break;
	...
	public Object Curl_auth_create_oauth_bearer_message(Object user, Object host, Object port, Object bearer, Byte outptr, Object outlen) {
		...
		result = ModernizedCProgram.data.Curl_base64_encode(oauth, .strlen(oauth), outptr, /* Base64 encode the reply */outlen);
		.Curl_cfree(oauth);
		return result/*
	...
	public Object Curl_auth_create_xoauth_bearer_message(Object user, Object bearer, Byte outptr, Object outlen) {
		...
		result = ModernizedCProgram.data.Curl_base64_encode(xoauth, .strlen(xoauth), outptr, /* Base64 encode the reply */outlen);
		.Curl_cfree(xoauth);
	...
	public Byte curl_easy_unescape(Object string, int length, int olen) {
		byte str = ((Object)0);
		if (length >= 0) {
	...
	public Object Curl_auth_create_plain_message(Object authzid, Object authcid, Object passwd, byte outptr, Object outlen) {
		...
		outlen = 0;
		outptr = ((Object)0);
		zlen = (authzid == ((Object)0) ? 0 : .strlen(authzid));
		clen = .strlen(authcid);
		plen = .strlen(passwd);
		...
		plainauth = .Curl_cmalloc(plainlen);
		if (!plainauth) {
			return CURLE_OUT_OF_MEMORY;
		} 
	...
	public Object Curl_auth_create_login_message(Object valuep, byte outptr, Object outlen) {
		...
		if (!vlen) {
			outptr = .Curl_cstrdup(/* Calculate an empty reply */"=");
			if (outptr) {
				outlen = (size_t)1;
				return CURLE_OK;
			} 
	...
	public void mstate( state) {
		...
		if (finit[/* if this state has an init-function, run it */state]) {
			.UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.data);
		} 
	}
	public Object multi_done(Object status,  premature) {
		...
		do {
			.Curl_cfree((ModernizedCProgram.data.getReq().getLocation()));
		...
		for (i = 0; i < ModernizedCProgram.data.getState().getTempcount(); /* if the transfer was completed in a paused state there can be buffered
		     data left to free */i++) {
			.Curl_cfree(ModernizedCProgram.data.getState().getTempwrite()[i].getBuf());
		}
		...
		if ((ModernizedCProgram.data.getSet().getReuse_forbid() && !(generatedHttp_ntlm_state == .NTLMSTATE_TYPE2 || generatedProxy_ntlm_state == .NTLMSTATE_TYPE2)) || generatedClose || (premature && !(generatedHandler.getFlags() & (1 << 9)))) {
			...
			if (!result && /* If we had an error already, make sure we return that one. But
				...
				ModernizedCProgram.curl_msnprintf(ModernizedCProgram.buffer, , "Connection #%ld to host %s left intact", generatedConnection_id, generatedSocksproxy ? generatedDispname : generatedHttpproxy ? generatedDispname : generatedConn_to_host ? generatedDispname : generatedDispname);
			...
			if (multi/* this is set if this connection is part of a handle that is added to
				...
				if (entry) {
					if (generatedSocket_cb) {
						.UNRECOGNIZEDFUNCTIONNAME(ModernizedCProgram.data, s, CURL_POLL_REMOVE, generatedSocket_userp, generatedSocketp);
					} 
	...
	public  checkrtspprefix(Object s, Object len) {
		...
		statusline onmatch = len >= 5 ? .STATUS_DONE : .STATUS_UNKNOWN/* convert from the network encoding using a scratch area *//* can't return CURLE_OUT_OF_MEMORY so return FALSE */;
		(Object)/* Curl_convert_from_network calls failf if unsuccessful *//* can't return CURLE_foobar so return FALSE *//* unused */ModernizedCProgram.data;
	...
	public Object Curl_auth_create_cram_md5_message(Object chlg, Object userp, Object passwdp, Byte outptr, Object outlen) {
		...
		ctxt = ModernizedCProgram.Curl_HMAC_init(/* Compute the digest using the password as the key */ModernizedCProgram.Curl_HMAC_MD5, (byte)passwdp, ModernizedCProgram.curlx_uztoui(.strlen(passwdp)));
		if (!ctxt) {
			return CURLE_OUT_OF_MEMORY;
		} 
	...
	public Object Curl_auth_create_digest_md5_message(Object chlg64, Object userp, Object passwdp, Object service, Byte outptr, Object outlen) {
		...
		byte[] qop = "auth";
		byte spn = ((Object)0);
		 result = ModernizedCProgram.auth_decode_digest_md5_message(chlg64, /* Decode the challenge message */nonce, , realm, , algorithm, , qop_options, );
		if (result) {
			return result;
		} 
		if (.strcmp(algorithm, "md5-sess") != /* We only support md5 sessions */0) {
		...
		result = ModernizedCProgram.Curl_rand_hex(ModernizedCProgram.data, (byte)cnonce, );
		if (result) {
			return result;
		} 
		...
		ctxt.Curl_MD5_update((byte)userp, ModernizedCProgram.curlx_uztoui(.strlen(userp)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)realm, ModernizedCProgram.curlx_uztoui(.strlen(realm)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)passwdp, ModernizedCProgram.curlx_uztoui(.strlen(passwdp)));
		...
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)nonce, ModernizedCProgram.curlx_uztoui(.strlen(nonce)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)cnonce, ModernizedCProgram.curlx_uztoui(.strlen(cnonce)));
		ctxt.Curl_MD5_final(digest);
		...
		ctxt.Curl_MD5_update((byte)method, ModernizedCProgram.curlx_uztoui(.strlen(method)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)spn, ModernizedCProgram.curlx_uztoui(.strlen(spn)));
		ctxt.Curl_MD5_final(digest);
		for (i = 0; i < 16; i++) {
		...
		ctxt.Curl_MD5_update((byte)nonce, ModernizedCProgram.curlx_uztoui(.strlen(nonce)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)nonceCount, ModernizedCProgram.curlx_uztoui(.strlen(nonceCount)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)cnonce, ModernizedCProgram.curlx_uztoui(.strlen(cnonce)));
		ctxt.Curl_MD5_update((byte)":", 1);
		ctxt.Curl_MD5_update((byte)qop, ModernizedCProgram.curlx_uztoui(.strlen(qop)));
		ctxt.Curl_MD5_update((byte)":", 1);
	...
	public Object Curl_vsetopt(Object option, Object param) {
		     ...
		     */:
				...
				if (!argptr || ModernizedCProgram.data.getSet().getPostfieldsize() == -1) {
					result = ModernizedCProgram.Curl_setstropt(ModernizedCProgram.data.getSet().getStr()[dupstring.STRING_COPYPOSTFIELDS], argptr/*
						...
						if ((ModernizedCProgram.data.getSet().getPostfieldsize() < 0) || (( != ) && (ModernizedCProgram.data.getSet().getPostfieldsize() > .UNRECOGNIZEDFUNCTIONNAME((size_t)-1)))) {
							result = CURLE_OUT_OF_MEMORY;
						} else {
								byte p;
								(Object)ModernizedCProgram.Curl_setstropt(ModernizedCProgram.data.getSet().getStr()[dupstring.STRING_COPYPOSTFIELDS], ((Object)0/* Allocate even when size == 0. This satisfies the need of possible
								           ...
								           form data.
								        */));
								p = .Curl_cmalloc((size_t)(ModernizedCProgram.data.getSet().getPostfieldsize() ? ModernizedCProgram.data.getSet().getPostfieldsize() : 1));
								if (!p) {
									result = CURLE_OUT_OF_MEMORY;
								} else {
										if (ModernizedCProgram.data.getSet().getPostfieldsize()) {
											.memcpy(p, argptr, (size_t)ModernizedCProgram.data.getSet().getPostfieldsize());
										} 
		...
		case CURLOPT_BUFFERSIZE/*
		     ...
		     */:
				...
				if (arg != ModernizedCProgram.data.getSet().getBuffer_size()) {
					byte newbuff = .Curl_crealloc(ModernizedCProgram.data.getState().getBuffer(), arg + 1);
					if (!newbuff) {
						do {
						} while (0);
						result = CURLE_OUT_OF_MEMORY;
		...
		case CURLOPT_SSLENGINE_DEFAULT/*
		     ...
		     */:
				...
				result = .Curl_ssl_set_engine_default(ModernizedCProgram.data);
				break;
		case CURLOPT_CRLF/*
		...
		case CURLOPT_SSLENGINE/*
		     ...
		     */:
				...
				if (argptr && argptr[0]) {
					...
					if (!result) {
						result = .Curl_ssl_set_engine(ModernizedCProgram.data, argptr);
					} 
				} 
				break;
	...
	public void expect_timer_seconds(int seconds) {
		...
		ModernizedCProgram.curl_msnprintf(msg, , "about %d seconds should have passed", seconds);
	...
	public int Curl_doh_done(Object result) {
		Object generatedSet = this.getSet();
		Curl_easy data = generatedSet.getDohfor();
		...
		if (!generatedReq.getDoh().getPending()) {
			generatedReq.getDoh().getHeaders().curl_slist_free_all();
	...
	public Object Curl_get_upload_buffer() {
		if (!ModernizedCProgram.data.getState().getUlbuf()) {
			ModernizedCProgram.data.getState().setUlbuf(.Curl_cmalloc(ModernizedCProgram.data.getSet().getUpload_buffer_size()));
			if (!ModernizedCProgram.data.getState().getUlbuf()) {
				return CURLE_OUT_OF_MEMORY;
			} 
		} 
	...
	public Object Curl_pretransfer() {
		...
		if (!ModernizedCProgram.data.getChange().getUrl() && ModernizedCProgram.data.getSet().getUh()) {
			 uc = new ();
			uc = .curl_url_get(ModernizedCProgram.data.getSet().getUh(), CURLUPART_URL, ModernizedCProgram.data.getSet().getStr()[dupstring.STRING_SET_URL], 0);
			if (uc) {
		...
		result = .Curl_ssl_initsessions(ModernizedCProgram.data, ModernizedCProgram.data.getSet().getGeneral_ssl().getMax_ssl_sessions());
		if (result) {
			return result;
		} 
		...
		if (ModernizedCProgram.data.getSet().getHttpreq() == .HTTPREQ_PUT) {
			...
			if (ModernizedCProgram.data.getSet().getPostfields() && (ModernizedCProgram.data.getState().getInfilesize() == -1)) {
				ModernizedCProgram.data.getState().setInfilesize(().strlen(ModernizedCProgram.data.getSet().getPostfields()));
			} 
		} else {
		...
		if (!result/* Allow data->set.use_port to set which port to use. This needs to be
			...
			ModernizedCProgram.data.getState().setAllow_port(1/*************************************************************
			     ...
			     *************************************************************/);
			.Curl_initinfo(/* reset session-specific information "variables" */ModernizedCProgram.data);
	...
	public Object Curl_follow(Byte newurl,  type) {
			...
			if ((ModernizedCProgram.data.getSet().getMaxredirs() != -1) && (ModernizedCProgram.data.getSet().getFollowlocation() >= ModernizedCProgram.data.getSet().getMaxredirs())) {
				...
				type = /* switch to fake to store the would-be-redirected
					...
					if (ModernizedCProgram.data.getSet().getHttp_auto_referer()) {
						...
						ModernizedCProgram.data.getChange().setReferer(.Curl_cstrdup(ModernizedCProgram.data.getChange().getUrl()));
		...
		if (.Curl_is_absolute_url(newurl, ((Object)0), 40)) {
			disallowport = /* This is an absolute URL, don't allow the custom port number */1;
		} 
		do {
		} while (0);
		uc = .curl_url_set(ModernizedCProgram.data.getState().getUh(), CURLUPART_URL, newurl, (type == .FOLLOW_FAKE) ? CURLU_NON_SUPPORT_SCHEME : ((type == .FOLLOW_REDIR) ? CURLU_URLENCODE : 0));
		if (uc) {
			if (type != .FOLLOW_FAKE) {
			...
			if (!newurl) {
				...
				uc = .curl_url_get(ModernizedCProgram.data.getState().getUh(), CURLUPART_URL, newurl, 0);
				if (uc) {
					return ModernizedCProgram.Curl_uc_to_curlcode(uc);
				} 
		} 
	...
	public void setFake(int newFake) {
		fake = newFake;
	}
}
