...
public class CookieInfo {
	...
	public CookieInfo Curl_cookie_init(Curl_easy data, Object file,  newsession) {
		...
		if (((Object)0) == inc) {
			c = /*Error: Function owner not recognized*/Curl_ccalloc(1, /*Error: Unsupported expression*//* we didn't get a struct, create one */);
			if (!c) {
				return ((Object)/* failed to get memory */0);
			} 
			c.setFilename(/*Error: Function owner not recognized*/Curl_cstrdup(file ? file : /* copy the name just in case */"none"));
		...
		if (file && !/*Error: Function owner not recognized*/strcmp(file, "-")) {
			fp = (_iob[0]);
			fromfile = 0;
		}  else if (file && !file) {
			fp = ((Object)/* points to a "" string */0);
		} else {
				fp = file ? /*Error: Function owner not recognized*/fopen(file, "rt") : ((Object)0);
		} 
		...
		if (fp) {
			...
			line = /*Error: Function owner not recognized*/Curl_cmalloc(5000);
			if (!line) {
				;
			} 
			while (fp.Curl_get_line(line, 5000)) {
				if (ModernizedCProgram.curl_strnequal("Set-Cookie:", line, /*Error: Function owner not recognized*/strlen("Set-Cookie:"))) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(/* free the line buffer */line);
	...
	public void Curl_cookie_cleanup() {
		...
		if (c) {
			int i;
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(generatedFilename);
			for (i = 0; i < 256; i++) {
				generatedCookies[i].Curl_cookie_freelist();
			}
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(/* free the base struct as well */c/* get_netscape_format()
			 *
	...
	public int cookie_output(Object dumphere) {
		...
		if (!/*Error: Function owner not recognized*/strcmp("-", dumphere)) {
			out = (_iob[/* use stdout */1]);
			use_stdout = 1;
		} else {
				out = /*Error: Function owner not recognized*/fopen(dumphere, "wt");
				if (!out) {
					return /* failure */1;
				} 
		} 
		/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fputs("# Netscape HTTP Cookie File\n# https://curl.haxx.se/docs/http-cookies.html\n# This file was generated by libcurl! Edit at your own risk.\n\n", out);
		long generatedNumcookies = this.getNumcookies();
		...
		if (generatedNumcookies) {
			...
			array = /*Error: Function owner not recognized*/Curl_ccalloc(1, /*Error: Unsupported expression*/ * generatedNumcookies);
			if (!array) {
				if (!use_stdout) {
			...
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/qsort(array, nvalid, /*Error: Unsupported expression*/, cookie_sort_ct);
			for (i = 0; i < nvalid; i++) {
				byte format_ptr = ModernizedCProgram.get_netscape_format(array[i]);
				if (format_ptr == ((Object)0)) {
					out.curl_mfprintf("#\n# Fatal libcurl error\n");
					/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(array);
					if (!use_stdout) {
						/*Error: Function owner not recognized*//*Error: Function owner not recognized*/fclose(out);
					} 
					return 1;
				...
				out.curl_mfprintf("%s\n", format_ptr);
				/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(format_ptr);
			}
			/*Error: Function owner not recognized*//*Error: Function owner not recognized*/Curl_cfree(array);
		} 
		if (!use_stdout) {
