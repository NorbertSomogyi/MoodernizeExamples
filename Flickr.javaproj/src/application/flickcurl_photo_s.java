...
public class flickcurl_photo_s {
	...
	public flickcurl_photo_s[] flickcurl_photos_getInfo2(flickcurl_s[] fc, Object[] photo_id, Object[] secret) {
		...
		flickcurl_s flickcurl_s = new flickcurl_s();
		if (flickcurl_s.flickcurl_prepare(fc, "flickr.photos.getInfo")) {
			;
		} 
		flickcurl_s flickcurl_s = new flickcurl_s();
	...
	public flickcurl_photo_s[][] flickcurl_panda_getPhotos(flickcurl_s[] fc, Object[] panda_name) {
		...
		flickcurl_s flickcurl_s = new flickcurl_s();
		flickcurl_s.flickcurl_add_param(fc, "panda_name", panda_name);
		flickcurl_s flickcurl_s = new flickcurl_s();
		flickcurl_s.flickcurl_end_params(fc);
		flickcurl_s flickcurl_s = new flickcurl_s();
		if (flickcurl_s.flickcurl_prepare(fc, "flickr.panda.getPhotos")) {
			;
		} 
		flickcurl_s flickcurl_s = new flickcurl_s();
	...
	public flickcurl_photo_s[][] flickcurl_stats_getPopularPhotos(flickcurl_s[] fc, Object[] date, Object[] sort, int per_page, int page, Object[] extras) {
		...
		if (sort) {
			flickcurl_s.flickcurl_add_param(fc, "sort", sort);
		} 
		ModernizedCProgram.flickcurl_append_photos_list_params(fc, list_params, /* Photos List parameters */format);
		flickcurl_s flickcurl_s = new flickcurl_s();
		flickcurl_s.flickcurl_end_params(fc);
		flickcurl_s flickcurl_s = new flickcurl_s();
		if (flickcurl_s.flickcurl_prepare(fc, "flickr.stats.getPopularPhotos")) {
			;
		} 
		flickcurl_s flickcurl_s = new flickcurl_s();
		photos_list = flickcurl_s.flickcurl_invoke_photos_list(fc, ()"/rsp/photos", format);
	...
	public void command_print_photo(flickcurl_photo_s[] photo) {
		int i;
		...
		flickcurl_tag_s.command_print_tags(generatedTags, ((Object)0), ((Object)0));
		flickcurl_note_s[][] generatedNotes = photo.getNotes();
		flickcurl_note_s flickcurl_note_s = new flickcurl_note_s();
		if (generatedNotes) {
			flickcurl_note_s.command_print_notes(generatedNotes, ((Object)0), ((Object)0));
		} 
		[] generatedPlace = photo.getPlace();
		if (generatedPlace) {
			ModernizedCProgram.command_print_place(generatedPlace, ((Object)0), ((Object)0), 1);
		} 
		[] generatedVideo = photo.getVideo();
		if (generatedVideo) {
			ModernizedCProgram.command_print_video(generatedVideo);
		} 
	}
	public void flickcurl_free_photo(flickcurl_photo_s[] photo) {
		int i;
		do {
			if (!photo) {
		...
		flickcurl_tag_s[][] generatedTags = photo.getTags();
		flickcurl_tag_s flickcurl_tag_s = new flickcurl_tag_s();
		flickcurl_tag_s.flickcurl_free_tags(generatedTags);
		int generatedNotes_count = photo.getNotes_count();
		flickcurl_note_s[][] generatedNotes = photo.getNotes();
		flickcurl_note_s flickcurl_note_s = new flickcurl_note_s();
		for (i = 0; i < generatedNotes_count; i++) {
			flickcurl_note_s.flickcurl_free_note(generatedNotes[i]);
		}
		.free(generatedNotes);
		byte[] generatedId = photo.getId();
		if (generatedId) {
		...
		[] generatedPlace = photo.getPlace();
		if (generatedPlace) {
			ModernizedCProgram.flickcurl_free_place(generatedPlace);
		} 
		[] generatedVideo = photo.getVideo();
		if (generatedVideo) {
			ModernizedCProgram.flickcurl_free_video(generatedVideo);
		} 
		.free(photo/**
	...
	public byte[] flickcurl_photo_as_source_uri(flickcurl_photo_s[] photo, Object c) {
	...
	public byte[] flickcurl_photo_as_page_uri(flickcurl_photo_s[] photo) {
		byte[] buf = new byte[512];
		byte[] result;
		size_t len = new size_t();
		Object[] generatedFields = photo.getFields();
	...
	public byte[] flickcurl_photo_as_short_uri(flickcurl_photo_s[] photo) {
		byte[] generatedId = photo.getId();
		return ModernizedCProgram.flickcurl_photo_id_as_short_uri(generatedId);
	}
	public byte[] flickcurl_photo_as_user_icon_uri(flickcurl_photo_s[] photo) {
		Object[] generatedFields = photo.getFields();
	...
	public flickcurl_photo_s[][] flickcurl_build_photos(flickcurl_s[] fc, Object xpathCtx, Object[] xpathExpr, Integer photo_count_p) {
		...
		for (; i < nodes_count; i++) {
			...
			for (expri = 0; photo_fields_table[expri].getXpath(); expri++) {
				...
				int special = 0;
				string_value = flickcurl_s.flickcurl_xpath_eval(fc, xpathNodeCtx, photo_fields_table[expri].getXpath());
				if (!string_value) {
					continue;
				} 
				switch (datatype) {
			...
			if (!generatedPlace) {
				photo.setPlace(flickcurl_s.flickcurl_build_place(fc, xpathNodeCtx, ()"./location"));
			} 
			photo.setVideo(flickcurl_s.flickcurl_build_video(fc, xpathNodeCtx, ()"./video"));
	...
	public void flickcurl_free_photos(flickcurl_photo_s[][] photos) {
		int i;
		do {
			if (!photos) {
		...
		flickcurl_photo_s flickcurl_photo_s = new flickcurl_photo_s();
		for (i = 0; photos[i]; i++) {
			flickcurl_photo_s.flickcurl_free_photo(photos[i]);
		}
		.free(photos/*
